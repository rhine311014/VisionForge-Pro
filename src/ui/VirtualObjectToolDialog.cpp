...
void VirtualObjectToolDialog::connectSignals()
{
    ...
    // 图像操作按钮
    connect(loadImageBtn_, &QPushButton::clicked, this, &VirtualObjectToolDialog::onLoadImageClicked);
    connect(captureImageBtn_, &QPushButton::clicked, this, &VirtualObjectToolDialog::onCaptureImageClicked);
}
...
void VirtualObjectToolDialog::onLoadImageClicked()
{
    QString filePath = QFileDialog::getOpenFileName(
        this, tr("选择图片文件"), QString(),
    ...
    try {
        ...
        if (!mat.empty()) {
            ...
            QMessageBox::warning(this, tr("加载失败"),
                tr("无法加载图片文件: %1").arg(filePath));
        }
    } catch (const std::exception& e) {
        QMessageBox::warning(this, tr("加载失败"),
            tr("加载图片时发生错误: %1").arg(e.what()));
        LOG_ERROR(QString("加载图片失败: %1").arg(e.what()));
    }
}

void VirtualObjectToolDialog::onCaptureImageClicked()
{
