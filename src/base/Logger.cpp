 ...
 *
 * ### 单例实现
 * 使用C++11的静态局部变量特性实现线程安全的单例模式（Meyers' Singleton）。
 * 静态局部变量的初始化在C++11中保证是线程安全的。
 *
 * ### 日志格式
 * 格式：[时间戳] [级别] 消息 (文件:行号)
 * 示例：[2025-12-14 10:30:00.123] [INFO ] 应用程序启动 (main.cpp:42)
 *
 * ### 批量写入优化
 * 为避免频繁的磁盘I/O，使用unflushedCount_计数器：
 * - 每次写入日志增加计数
 * - 达到FLUSH_THRESHOLD(10条)时执行flush
 * - 关闭文件前确保所有日志已写入
 *
 * ### 导出格式实现
 * - PlainText：人类可读的纯文本格式
 * - CSV：Excel兼容的逗号分隔格式
 * - JSON：结构化数据格式，便于程序解析
 * - HTML：带样式的网页格式，便于分享和查看
 */
