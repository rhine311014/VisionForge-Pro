// CrossSearchTool.h: interface for the CCrossSearchTool class.
//
//////////////////////////////////////////////////////////////////////

#if !defined(AFX_CROSSSEARCHTOOL_H__8CF0FDDA_2D35_45FE_A3EF_327D8D73244F__INCLUDED_)
#define AFX_CROSSSEARCHTOOL_H__8CF0FDDA_2D35_45FE_A3EF_327D8D73244F__INCLUDED_
...
#define CROSS_LINE_NUM  2

// CrossSearch定位工具训练Data参数类
typedef struct tagCrossSearchTrainDataParam : public CBaseTrainDataParam
{
	double	m_dGrau;									// 颗粒度
	long	m_lNoiseThre;								// 噪声阈值
	long	m_lTotalMag;								// 幅值和
	bool	m_bMaskEnable;								// 是否设置掩模图像
	tagCrossSearchTrainDataParam()
	{
		m_dGrau			= 2.0;
		m_lNoiseThre	= 10;
		m_lTotalMag		= 200;
...
}CCrossSearchTrainDataParam;

// CrossSearch定位工具训练Gui参数类
typedef struct tagCrossSearchTrainGuiParam : public CBaseTrainGuiParam
{
...
}CCrossSearchTrainGuiParam;

// CrossSearch定位工具搜索Data参数类
typedef struct tagCrossSearchSearchDataParam : public CBaseSearchDataParam
{
	int		m_nSearchNum;								// 搜索个数
	double	m_dAcceptThre;								// 接收阈值
	double  m_dAngleLow;								// 低角度
	double	m_dAngleHigh;								// 高角度
	tagCrossSearchSearchDataParam()
	{
		m_nSearchNum	= 1;
		m_dAcceptThre	= 0.8;
		m_dAngleLow		= -10.0;
...
}CCrossSearchSearchDataParam;

// CrossSearch定位工具搜索Gui参数类
typedef struct tagCrossSearchSearchGuiParam : public CBaseSearchGuiParam
{
	COLORREF        m_LineColor;						// 图形颜色
	int				m_nLineWidth;						// 图形线宽
	bool			m_bAffineRectVisible;				// 搜索结果矩形可见
	bool			m_bModelPointVisible;				// 参考点是够可见
	tagCrossSearchSearchGuiParam()
	{
...
}CCrossSearchSearchGuiParam;

// CrossSearch训练结果
typedef struct tagCrossSearchTrainResult
{
	double m_dModelX; // 参考点X					
	double m_dModelY; // 参考点Y

	tagCrossSearchTrainResult()
	{
		m_dModelX = 0;
		m_dModelY = 0;
	}
...
}CCrossSearchTrainResult;

//
//	CrossSearch定位工具
//
class /*SV_SEARCHTOOL_API*/ CCrossSearchTool : public CBaseSearchTool  
{
public:
	CCrossSearchTool();
	CCrossSearchTool(CCrossSearchTool &cCrossSearchTool);
	CCrossSearchTool& operator=(CCrossSearchTool &cCrossSearchTool);
	virtual ~CCrossSearchTool();
...
private:
	scQuickSearchTool                   m_QuickSearchTool;				// CrossSearch搜索工具
	...
	CCrossSearchTrainDataParam			m_tmpTrainDataParam;			// 当前训练Data参数
	CCrossSearchTrainGuiParam			m_tmpTrainGuiParam;				// 当前训练Gui参数	
	CCrossSearchSearchDataParam			m_tmpSearchDataParam;			// 当前搜索Data参数	
	CCrossSearchSearchGuiParam			m_tmpSearchGuiParam;			// 当前搜索Gui参数
	CCrossSearchTrainResult				m_tmpTrainResult;				// 当前训练结果
	cpImage							m_tmpImageTrain;				// 当前训练图像
	cpImage							m_tmpImageMask;					// 当前掩模图像
	CCrossSearchTrainDataParam			m_TrainDataParam;				// 训练Data参数
	CCrossSearchTrainGuiParam			m_TrainGuiParam;				// 训练Gui参数	
	CCrossSearchSearchDataParam			m_SearchDataParam;				// 搜索Data参数	
	CCrossSearchSearchGuiParam			m_SearchGuiParam;				// 搜索Gui参数
	CCrossSearchTrainResult				m_TrainResult;					// 训练结果
...
#endif // !defined(AFX_CROSSSEARCHTOOL_H__8CF0FDDA_2D35_45FE_A3EF_327D8D73244F__INCLUDED_)
