// CircleSearchTool.cpp: implementation of the CPatternCornerMulCircleSearchTool class.
//
//////////////////////////////////////////////////////////////////////
...
CPatternCornerMulCircleSearchTool::CPatternCornerMulCircleSearchTool()
{
	m_pGuiDisplay								= NULL;
	m_hDispWnd									= NULL;
	...
	for(int i =0; i< LINE_NUM; i++)
	{
		...
		m_tmpTrainDataParam.m_bEnableLineDualEdge[i] = false;
	}
...
CPatternCornerMulCircleSearchTool::CPatternCornerMulCircleSearchTool(CPatternCornerMulCircleSearchTool &cCircleSearchTool)
{
	if (cCircleSearchTool.GetSearchToolType() != ePatternCornerMulCircleSearchTool)
	{
		return;
	}
...
CPatternCornerMulCircleSearchTool& CPatternCornerMulCircleSearchTool::operator=(CPatternCornerMulCircleSearchTool &cCircleSearchTool)
{
...
// 获取工具类型
CSearchToolType CPatternCornerMulCircleSearchTool::GetSearchToolType()
{
	return ePatternCornerMulCircleSearchTool;
}										   

// 设置显示控件指针		
bool CPatternCornerMulCircleSearchTool::SetDisplay(scGuiDisplay *pGuiDisplay)
{	
	ClearAllGui();
...
// 获取显示控件指针										
scGuiDisplay* CPatternCornerMulCircleSearchTool::GetDisplay()
{
	return m_pGuiDisplay;
}

// 设置Gui图形的关键字														
bool CPatternCornerMulCircleSearchTool::SetGuiKeyText(CString strGuiKeyText)									
{
...
// 获取Gui图形的关键字
bool CPatternCornerMulCircleSearchTool::GetGuiKeyText(CString &strGuiKeyText)
{
...
// 更新训练界面显示					
bool CPatternCornerMulCircleSearchTool::ShowTrainGui(bool bShow)
{
...
// 是否显示训练结果界面
bool CPatternCornerMulCircleSearchTool::ShowTrainResultGui(bool bShow)
{
...
// 更新搜索界面显示									
bool CPatternCornerMulCircleSearchTool::ShowSearchResultGui(bool bShow)							
{
...
// 更新训练Gui控件显示
bool CPatternCornerMulCircleSearchTool::UpdateTrainGuiDisplay()
{
...
// 更新训练结果Gui控件显示
bool CPatternCornerMulCircleSearchTool::UpdateTrainResultGuiDisplay()
{
	// 训练结果
	for (int i = 0; i < PATTERN_CIRCLE_NUM; i++)
	{
...
// 更新搜索Gui控件显示														
bool CPatternCornerMulCircleSearchTool::UpdateSearchResultGuiDisplay()
{
	int i=0;
...
// 更新界面显示
bool CPatternCornerMulCircleSearchTool::UpdateGuiDisplay()
{
	UpdateTrainGuiDisplay();
...
// 设置训练Data参数	
bool CPatternCornerMulCircleSearchTool::SetTrainDataParam(CBaseTrainDataParam *pTrainDataParam)
{
	if (!pTrainDataParam)
	{
		return false;
	...
	if (pTrainDataParam->GetSearchToolType() != ePatternCornerMulCircleSearchTool)
	{
		return false;
	}

	m_tmpTrainDataParam = *((CPatternCornerMulCircleSearchTrainDataParam*)pTrainDataParam);

	return true;
}

// 获取训练Data参数									
bool CPatternCornerMulCircleSearchTool::GetTrainDataParam(CBaseTrainDataParam *pTrainDataParam)
{
	if (!pTrainDataParam)
	{
		return false;
	...
	*((CPatternCornerMulCircleSearchTrainDataParam*)pTrainDataParam) = m_tmpTrainDataParam;

	return true;
}

// 获取搜索Gui参数						
bool CPatternCornerMulCircleSearchTool::SetTrainGuiParam(CBaseTrainGuiParam *pTrainGuiParam)
{
	if (!pTrainGuiParam)
	{
		return false;
	...
	if (pTrainGuiParam->GetSearchToolType() != ePatternCornerMulCircleSearchTool)
	{
		return false;
	}

	m_tmpTrainGuiParam = *((CPatternCornerMulCircleSearchTrainGuiParam*)pTrainGuiParam);
...
// 获取搜索Gui参数							
bool CPatternCornerMulCircleSearchTool::GetTrainGuiParam(CBaseTrainGuiParam *pTrainGuiParam)
{
	if (!pTrainGuiParam)
	{
		return false;
	...
	*((CPatternCornerMulCircleSearchTrainGuiParam*)pTrainGuiParam) = m_tmpTrainGuiParam;

	return true;
}

// 设置训练原始图像				
bool CPatternCornerMulCircleSearchTool::SetTrainImage(const cpImage &ImageTrain)
{
...
// 获取训练原始图像									
bool CPatternCornerMulCircleSearchTool::GetTrainImage(cpImage &ImageTrain)
{
...
// 设置训练原始图像									
bool CPatternCornerMulCircleSearchTool::SetMaskImage(const cpImage &ImageMask)
{
...
// 获取训练原始图像								
bool CPatternCornerMulCircleSearchTool::GetMaskImage(cpImage &ImageMask)
{
...
// 执行训练											
bool CPatternCornerMulCircleSearchTool::Train()
{
	m_bTrainOK = false;
	...
	if (m_tmpTrainDataParam.m_bMaskEnable)
	{
		...
		bool bFindCircle[PATTERN_CIRCLE_NUM];
		...
		for (int i = 0; i < PATTERN_CIRCLE_NUM; i++)
		{
			...
			m_FindCircleTool_1[i].SetCenterRadiusAngleStartAngleSpan(
				...
				m_tmpTrainGuiParam.m_CircleArcStart[i], m_tmpTrainGuiParam.m_CircleArcSpan[i]);
			//掩膜与双极性
			m_FindCircleTool_1[i].SetCaliperMasked(m_tmpTrainGuiParam.m_vbCircleCaliperMask[i]);
			if (m_tmpTrainDataParam.m_bEnableCircleDualEdge[i])
			{
				...
				m_FindCircleTool_1[i].SetCaliperProperty(m_tmpTrainDataParam.m_dCircleContrastThre[i], 
					m_tmpTrainDataParam.m_edgeCircleProperty[i],tmpEdgePolarity, 
					m_tmpTrainDataParam.m_lCircleFilterHalfWidth[i]);
			} 
			else
			{
			...
			if(bIsProcessImage)
			{
				bFindCircle[i] = m_FindCircleTool_1[i].Execute(tmpImage, cCircle[i], dRMS[i]);
			}
			else
			{
				bFindCircle[i] = m_FindCircleTool_1[i].Execute(m_tmpImageTrain, cCircle[i], dRMS[i]);
			}
			if (!bFindCircle[i])
			{
				continue;
		...
		if(!bFindCircle[0]&&!bFindCircle[1])
		{
			m_nResultErrNumber = -4;
			return false;
		}
		else if(bFindCircle[0]&&!bFindCircle[1])
		{
...
// 获取训练结果图像														
bool CPatternCornerMulCircleSearchTool::GetTrainResultImage(cpImage &ImageTrainResult)
{
...
// 获取训练结果
bool CPatternCornerMulCircleSearchTool::GetTrainResult(CBaseTrainResult *pTrainResult)
{
	if (!pTrainResult)
	{
		return false;
	...
	if (pTrainResult->GetSearchToolType() != ePatternCornerMulCircleSearchTool)
	{
		return false;
	}

	*((CPatternCornerMulCircleSearchTrainResult*)pTrainResult) = m_tmpTrainResult;

	return true;
}	

// 是否训练成功
bool CPatternCornerMulCircleSearchTool::IsTrained()
{
	return m_bTrainOK;
}

// 设置搜索Data参数	
bool CPatternCornerMulCircleSearchTool::SetSearchDataParam(CBaseSearchDataParam *pSearchDataParam)
{	
	if (!pSearchDataParam)
	{
		return false;
	...
	if (pSearchDataParam->GetSearchToolType() != ePatternCornerMulCircleSearchTool)
	{
		return false;
	}

	m_tmpSearchDataParam = *((CPatternCornerMulCircleSearchSearchDataParam*)pSearchDataParam);

	return true;
}

// 获取搜索Data参数				
bool CPatternCornerMulCircleSearchTool::GetSearchDataParam(CBaseSearchDataParam *pSearchDataParam)
{
	if (!pSearchDataParam)
	{
		return false;
	...
	*((CPatternCornerMulCircleSearchSearchDataParam*)pSearchDataParam) = m_tmpSearchDataParam;

	return true;
}

// 设置搜索Gui参数					
bool CPatternCornerMulCircleSearchTool::SetSearchGuiParam(CBaseSearchGuiParam *pSearchGuiParam)
{
	if (!pSearchGuiParam)
	{
		return false;
	...
	if (pSearchGuiParam->GetSearchToolType() != ePatternCornerMulCircleSearchTool)
	{
		return false;
	}

	m_tmpSearchGuiParam = *((CPatternCornerMulCircleSearchSearchGuiParam*)pSearchGuiParam);
...
// 获取搜索Gui参数			
bool CPatternCornerMulCircleSearchTool::GetSearchGuiParam(CBaseSearchGuiParam *pSearchGuiParam)
{
	if (!pSearchGuiParam)
	{
		return false;
	...
	*((CPatternCornerMulCircleSearchSearchGuiParam*)pSearchGuiParam) = m_tmpSearchGuiParam;

	return true;
}

bool CPatternCornerMulCircleSearchTool::CalcLineCross(scLineSeg cLineSeg1, scLineSeg cLineSeg2, sc2Vector& centerLine1, sc2Vector& centerLine2, std::vector<sc2Vector>& vtCrossArray)
{
	bool bIntersect;
...
// 执行搜索 					
bool CPatternCornerMulCircleSearchTool::Search(cpImage &ImageSearch)
{
	if (!m_bTrainOK)
	{
	...
	for (int i = 0; i < PATTERN_CIRCLE_NUM; i++)
	{
		...
		if (m_tmpTrainDataParam.m_bEnableLineDualEdge[i])
		{
			...
			m_FindCircleTool_1[i].SetCaliperProperty(m_TrainDataParam.m_dCircleContrastThre[i], 
				m_TrainDataParam.m_edgeCircleProperty[i],tmpEdgePolarity, 
				m_TrainDataParam.m_lCircleFilterHalfWidth[i]);
		} 
		else
		{
	...
	if (bRet)
	{
		...
		for (int i=0;  i<nResultNum && i<1; i++)
		{
			...
			for (int j = 0; j < PATTERN_CIRCLE_NUM; j++)
			{
				// 初始的
				dOriCenterX_1[j] = m_TrainGuiParam.m_CircleCenter_1[j].GetX();
				dOriCenterY_1[j] = m_TrainGuiParam.m_CircleCenter_1[j].GetY();
				// 变换后
				dTransCenterX_1[j] = cos(dRotation) * dOriCenterX_1[j] - sin(dRotation) * dOriCenterY_1[j] + dTransX;
				dTransCenterY_1[j] = sin(dRotation) * dOriCenterX_1[j] + cos(dRotation) * dOriCenterY_1[j] + dTransY;

				m_FindCircleTool_1[j].SetCaliperParams(m_TrainGuiParam.m_nCircleCaliperNum[j], 
					m_TrainGuiParam.m_CircleCaliperSearchDirection[j], 
					m_TrainGuiParam.m_dCircleCaliperSearchLen[j], 
					m_TrainGuiParam.m_dCircleCaliperProjectLen[j]);
				m_FindCircleTool_1[j].SetCenterRadiusAngleStartAngleSpan(
					sc2Vector(dTransCenterX_1[j], dTransCenterY_1[j]), 
					m_TrainGuiParam.m_dCircleRadius[j], 
					scRadian(dRotation)+m_TrainGuiParam.m_CircleArcStart[j], 
					m_TrainGuiParam.m_CircleArcSpan[j]);
			...
			for (int k = 0; k < PATTERN_CIRCLE_NUM; k++)
			{
				if(bIsProcessImage)
				{
					bFindCircle[k] = m_FindCircleTool_1[k].Execute(tmpImage, ResCircle_1[k], dRMS_1[k]);
				}
				else
				{
					bFindCircle[k] = m_FindCircleTool_1[k].Execute(ImageSearch, ResCircle_1[k], dRMS_1[k]);
				}
			...
			searchResult.m_bFindLine1 = true;			searchResult.m_bFindLine2 = true;

			if (m_TrainDataParam.m_nLineAngleType == 0)	// 任意角度
			{
				searchResult.m_dAngle = scDegree(ResLineSeg.GetRotation().Norm()).ToDouble();
				//辅助点3-圆1点
				searchResult.m_vdAuxiliaryPosX.at(2)  = ResLineSeg.GetP1().GetX();
				searchResult.m_vdAuxiliaryPosY.at(2)  = ResLineSeg.GetP1().GetY();
				searchResult.m_vdAuxiliaryAngle.at(2) = searchResult.m_dAngle;
				//辅助点4-圆2点
				searchResult.m_vdAuxiliaryPosX.at(3)  = ResLineSeg.GetP2().GetX();
				searchResult.m_vdAuxiliaryPosY.at(3)  = ResLineSeg.GetP2().GetY();
				searchResult.m_vdAuxiliaryAngle.at(3) = searchResult.m_dAngle;
			}
			else if (m_TrainDataParam.m_nLineAngleType == 1)	//水平线
			{
				...
				{
					//辅助点3
					searchResult.m_vdAuxiliaryPosX.at(2) = ResLineSeg.GetP2().GetX();
					searchResult.m_vdAuxiliaryPosY.at(2) = ResLineSeg.GetP2().GetY();
					searchResult.m_vdAuxiliaryAngle.at(2) = searchResult.m_dAngle;
					//辅助点4
					searchResult.m_vdAuxiliaryPosX.at(3) = ResLineSeg.GetP1().GetX();
					searchResult.m_vdAuxiliaryPosY.at(3) = ResLineSeg.GetP1().GetY();
					searchResult.m_vdAuxiliaryAngle.at(3) = searchResult.m_dAngle;
				}
			}
			...
			for (int k = 0; k < PATTERN_CIRCLE_NUM; k++)
			{
				strTmpKey.Format(_T("%s_ResultCircle%d_%d"), m_strKeyText, k, i);
				scGuiCircle *pTemCircle = m_GuiSearchCircle+k + i*2;
				...
				scGuiCross *pTempCircleCenter = m_GuiSearchCircleCenter+k+i*2;
			...
			pTemLineSeg->SetStartXYEndXY(sc2Vector(searchResult.m_vdAuxiliaryPosX.at(2),searchResult.m_vdAuxiliaryPosY.at(2)), 
				sc2Vector(searchResult.m_vdAuxiliaryPosX.at(3),searchResult.m_vdAuxiliaryPosY.at(3)));
			...
			// 			st.Format(_T("%d X:%.1f;Y:%.1f;D:%.3f"), i, 
			// 				(ResLineSeg.GetP1().GetX() + ResLineSeg.GetP2().GetX())/2,
			// 				(ResLineSeg.GetP1().GetY() + ResLineSeg.GetP2().GetY())/2, searchResult.m_dAngle);
			...
			// 暂时用中点---iyo
			//辅助点3-圆1点
			searchResult.m_vdAuxiliaryPosX.at(2)  = (ResLineSeg.GetP1().GetX()+ResLineSeg.GetP2().GetX())/2.0;
			searchResult.m_vdAuxiliaryPosY.at(2)  = (ResLineSeg.GetP1().GetY()+ResLineSeg.GetP2().GetY())/2.0;
			searchResult.m_vdAuxiliaryAngle.at(2) = searchResult.m_dAngle;
			...
			// 圆
			for (int nCircle = 0; nCircle < PATTERN_CIRCLE_NUM; nCircle++)
			{
				CResultCircleSearchTool circle;
				circle.m_scCircleCircle = ResCircle_1[nCircle].GetCenter();
				circle.m_dRadius = ResCircle_1[nCircle].GetRadius();
			...
			pTempCross->SetCenterExtent(sc2Vector(searchResult.m_vdAuxiliaryPosX.at(2),searchResult.m_vdAuxiliaryPosY.at(2)), sc2Vector(40,40));
			pTempCross->SetLineColor(m_tmpSearchGuiParam.m_LineColor);
			pTempCross->SetLineWidth(m_tmpSearchGuiParam.m_nLineWidth);
			pTempCross->SetTipTextEnabled(true);
			pTempCross->SetTipText(strTmpKey);
			pTempCross->SetLabel(_T("中点"));
			pTempCross->SetLabelVisible(TRUE);
...
// 获取搜索结果个数										
int CPatternCornerMulCircleSearchTool::GetResultNum()
{
	return m_SearchResultArray.size();
}

// 获取搜索结果														
bool CPatternCornerMulCircleSearchTool::GetResult(int nIndex, CSearchResult &searchResult)
{
...
// 加载模板训练图像
bool CPatternCornerMulCircleSearchTool::LoadModelTrainPatternImage(CString strDir ,cpImage &ImagePattern)
{
	// strDir为文件夹路径
	// 仅配置ModelImage.bmp
	if (strDir == _T(""))
	...
	if (cSearchToolType != ePatternCornerMulCircleSearchTool)
	{
		return false;
	}
...
// 加载模板文件
bool CPatternCornerMulCircleSearchTool::LoadModelFromFile(CString strDir)
{
	// strDir为文件夹路径
	// 配置文件包括以下部分:
	//    (1) 配置文件Model.xml
	...
	sc2Vector vPos;
	//	BYTE byR, byG, byB;
	CPatternCornerMulCircleSearchTrainDataParam tmpTrainDataParam;
	CPatternCornerMulCircleSearchTrainGuiParam tmpTrainGuiParam;
	CPatternCornerMulCircleSearchSearchDataParam tmpSearchDataParam;
	CPatternCornerMulCircleSearchSearchGuiParam tmpSearchGuiParam;
	CPatternCornerMulCircleSearchTrainResult  tmpTrainResult;
	if (!cXMLConfigurator.Load(strXmlPath))
	{
		return false;
	}
	...
	for(int i = 0; i < LINE_NUM; i++)
	{
		...
		if (!bRet)
		{
			tmpTrainDataParam.m_bEnableLineDualEdge[i] = false;
		}
		else
		{
			tmpTrainDataParam.m_bEnableLineDualEdge[i] = _ttoi(cXMLConfigurator.GetElemData());
		}
	}
	...
	for (int i = 0; i < PATTERN_CIRCLE_NUM; i++)
	{
		...
		// DualEdge
		strTemp.Format(_T("CircleDualEdge%d"),i);
		bRet = cXMLConfigurator.FindElem(strTemp);
		if (!bRet)
		{
			tmpTrainDataParam.m_bEnableCircleDualEdge[i] = false;
		}
		else
		{
			tmpTrainDataParam.m_bEnableCircleDualEdge[i] = _ttoi(cXMLConfigurator.GetElemData());
		}
	...
	for (int i = 0; i < LINE_NUM; i++)
	{
		...
		if (bRet)
		{
			tmpTrainGuiParam.m_bEnableLineMask[i] = _ttol(cXMLConfigurator.GetElemData());
		}
		else
		{
			tmpTrainGuiParam.m_bEnableLineMask[i] = false;
		}
		...
		if (bRet)
		{
			...
			tmpTrainGuiParam.m_vdLineCaliperMaskData[i].resize(_ttoi(cXMLConfigurator.GetElemData()));

			for (int j=0; j<tmpTrainGuiParam.m_vdLineCaliperMaskData[i].size(); j++)
			{
				...
				tmpTrainGuiParam.m_vdLineCaliperMaskData[i].at(j) = _tcstod(cXMLConfigurator.GetElemData(), NULL);
			}
			bRet = cXMLConfigurator.FindElem(_T("EnableMaskDataNum"));
			if (!bRet) return false;
			tmpTrainGuiParam.m_vbLineCaliperMask[i].resize(_ttoi(cXMLConfigurator.GetElemData()));
			for (int j=0; j<tmpTrainGuiParam.m_vbLineCaliperMask[i].size(); j++)
			{
				...
				tmpTrainGuiParam.m_vbLineCaliperMask[i].at(j) = _ttoi(cXMLConfigurator.GetElemData());
			}
			cXMLConfigurator.LeaveElem();
		}
	}
	...
	for (int i = 0; i < PATTERN_CIRCLE_NUM; i++)
	{
		...
		strTemp.Format(_T("CircleCaliperMaskEnable%d"),i);
		bRet = cXMLConfigurator.FindElem(strTemp);
		if (bRet)
		{
		...
		strTemp.Format(_T("CircleMaskParam%d"),i);
		bRet = cXMLConfigurator.FindElem(strTemp);
		if (bRet)
		{
			CString strTmp;
			...
			bRet = cXMLConfigurator.FindElem(_T("MaskDataNum"));
			if (!bRet) return false;
			tmpTrainGuiParam.m_vdCircleCaliperMaskData[i].resize(_ttoi(cXMLConfigurator.GetElemData()));

			for (int j=0; j<tmpTrainGuiParam.m_vdCircleCaliperMaskData[i].size(); j++)
			{
				strTmp.Format(_T("MaskData%d"), j);
				bRet = cXMLConfigurator.FindElem(strTmp);
				if (!bRet)
				...
				tmpTrainGuiParam.m_vdCircleCaliperMaskData[i].at(j) = _tcstod(cXMLConfigurator.GetElemData(), NULL);
			}

			bRet = cXMLConfigurator.FindElem(_T("EnableMaskDataNum"));
			if (!bRet) return false;
			tmpTrainGuiParam.m_vbCircleCaliperMask[i].resize(_ttoi(cXMLConfigurator.GetElemData()));
			for (int j=0; j<tmpTrainGuiParam.m_vbCircleCaliperMask[i].size(); j++)
			{
				strTmp.Format(_T("EnableMaskData%d"), j);
				bRet = cXMLConfigurator.FindElem(strTmp);
				if (!bRet)
				...
				tmpTrainGuiParam.m_vbCircleCaliperMask[i].at(j) = _ttoi(cXMLConfigurator.GetElemData());
			}
			cXMLConfigurator.LeaveElem();
		}
...
// 保存模板文件							
bool CPatternCornerMulCircleSearchTool::SaveModelToFile(CString strDir)
{
	...
	for (int i = 0; i < LINE_NUM; i++)
	{
		...
		cXMLConfigurator.AddElem(strTemp, strInfo);
		// DualEdge1
		strInfo.Format(_T("%d"), m_TrainDataParam.m_bEnableLineDualEdge[i]);
		strTemp.Format(_T("LineDualEdge%d"),i);
		cXMLConfigurator.AddElem(strTemp, strInfo);
	}
	...
	for (int i = 0; i < PATTERN_CIRCLE_NUM; i++)
	{
		...
		// DualEdge
		strInfo.Format(_T("%d"), m_TrainDataParam.m_bEnableCircleDualEdge[i]);
		strTemp.Format(_T("CircleDualEdge%d"),i);
		cXMLConfigurator.AddElem(strTemp, strInfo);
	...
	for (int i = 0; i < LINE_NUM; i++)
	{
		...
		strInfo.Format(_T("%d"), m_TrainGuiParam.m_vdLineCaliperMaskData[i].size());
		cXMLConfigurator.AddElem(_T("MaskDataNum"), strInfo);

		for (int j=0; j<m_TrainGuiParam.m_vdLineCaliperMaskData[i].size(); j++)
		{
			strTmp.Format(_T("MaskData%d"), j);
			strInfo.Format(_T("%f"), m_TrainGuiParam.m_vdLineCaliperMaskData[i].at(j));
			cXMLConfigurator.AddElem(strTmp, strInfo);
		}

		strInfo.Format(_T("%d"), m_TrainGuiParam.m_vbLineCaliperMask[i].size());
		cXMLConfigurator.AddElem(_T("EnableMaskDataNum"), strInfo);

		for (int j=0; j<m_TrainGuiParam.m_vbLineCaliperMask[i].size(); j++)
		{
			strTmp.Format(_T("EnableMaskData%d"), j);
			strInfo.Format(_T("%d"), (int)m_TrainGuiParam.m_vbLineCaliperMask[i].at(j));
			cXMLConfigurator.AddElem(strTmp, strInfo);
		}
		cXMLConfigurator.LeaveElem();
	}
	...
	for (int i = 0; i < PATTERN_CIRCLE_NUM; i++)
	{
		...
		cXMLConfigurator.AddElem(strTemp);
		cXMLConfigurator.EnterElem();
		strInfo.Format(_T("%d"), m_TrainGuiParam.m_vdCircleCaliperMaskData[i].size());
		cXMLConfigurator.AddElem(_T("MaskDataNum"), strInfo);
		for (int j=0; j<m_TrainGuiParam.m_vdCircleCaliperMaskData[i].size(); j++)
		{
			strTemp.Format(_T("MaskData%d"), j);
			strInfo.Format(_T("%f"), m_TrainGuiParam.m_vdCircleCaliperMaskData[i].at(j));
			cXMLConfigurator.AddElem(strTemp, strInfo);
		}

		strInfo.Format(_T("%d"), m_TrainGuiParam.m_vbCircleCaliperMask[i].size());
		cXMLConfigurator.AddElem(_T("EnableMaskDataNum"), strInfo);

		for (int j=0; j<m_TrainGuiParam.m_vbCircleCaliperMask[i].size(); j++)
		{
			strTemp.Format(_T("EnableMaskData%d"), j);
			strInfo.Format(_T("%d"), (int)m_TrainGuiParam.m_vbCircleCaliperMask[i].at(j));
			cXMLConfigurator.AddElem(strTemp, strInfo);
		}
		cXMLConfigurator.LeaveElem();
...
// 更新训练控件数据信息	
bool CPatternCornerMulCircleSearchTool::UpdateTrainGuiData(bool bSaveAndValidate)
{
	if (bSaveAndValidate)	// 从Gui控件获取数据
	{
		...
		for (int i = 0; i < PATTERN_CIRCLE_NUM; i++)
		{
			...
			m_tmpTrainGuiParam.m_CircleCaliperSearchDirection[i] = (CaliperSearchDirection)m_GuiFindCircle_1[i].GetCaliperSearchDirection();
			m_tmpTrainGuiParam.m_bCircleCaliperVisible[i] = m_GuiFindCircle_1[i].IsCaliperVisible();
			m_GuiFindCircle_1[i].GetMaskData(m_tmpTrainGuiParam.m_vdCircleCaliperMaskData[i]);
			m_GuiFindCircle_1[i].GetCaliperMasked(m_tmpTrainGuiParam.m_vbCircleCaliperMask[i]);
		}
	}
	else // 将数据传递到Gui控件上
	{
		...
		for (int i = 0; i < PATTERN_CIRCLE_NUM; i++)
		{
			...
			m_GuiFindCircle_1[i].SetMaskData(m_tmpTrainGuiParam.m_vdCircleCaliperMaskData[i]);
		}
	}
...
bool CPatternCornerMulCircleSearchTool::UpdateSearchGuiData(bool bSaveAndValidate)
{
	if (bSaveAndValidate)	// 从Gui控件获取数据
	{
...
// 清除控件及其显示					
bool CPatternCornerMulCircleSearchTool::ClearGraphicsArray(std::vector<scGuiGraphic*> &pGraphicsArray)
{
...
bool CPatternCornerMulCircleSearchTool::ClearAllGui()
{
	ClearTrainGui();
...
// 清除搜索结果，需先清除结果GUI
bool CPatternCornerMulCircleSearchTool::ClearResult()
{
	ClearResultGui();
...
bool CPatternCornerMulCircleSearchTool::ClearTrainGui()
{
	if (m_pGuiDisplay && ::IsWindow(m_hDispWnd))
	{
		m_pGuiInteractiveContainer = m_pGuiDisplay->GetInteractiveContainer();
...
bool CPatternCornerMulCircleSearchTool::ClearResultGui()
{
	if (m_pGuiDisplay && ::IsWindow(m_hDispWnd))
	{
		int j=0;
...
// 获取训练模板原始图像
bool CPatternCornerMulCircleSearchTool::GetPatternImage(cpImage &ImagePattern)
{
...
// 恢复训练参数
bool CPatternCornerMulCircleSearchTool::RestoreTrainParam()
{ 
...
// 恢复搜索参数		
bool CPatternCornerMulCircleSearchTool::RestoreSearchParam()
{
...
void CPatternCornerMulCircleSearchTool::SetLanguage(int nLanguage)
{
...
bool CPatternCornerMulCircleSearchTool::SetGuiByCam(double nWidth,double nHeight)
{
...
CString CPatternCornerMulCircleSearchTool::GetResultErrInfo()
{
	CString strErrInfo;
	...
	case 0:
		{
			...
			case -3:			//找圆2失败				
				...
				break;
			case -4:			//找圆1 2失败				
				strErrInfo += _T("Find circle1 2 failed");
				break;
			case -5:			//找线1失败				
