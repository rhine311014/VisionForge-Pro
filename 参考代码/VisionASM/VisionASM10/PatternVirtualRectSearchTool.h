// PatternVirtualRectSearchTool.h: interface for the CPatternVirtualRectSearchTool class.
//
//////////////////////////////////////////////////////////////////////
...
#define RECT_LINE_NUM 4

// PatternVirtualRectSearch定位工具训练Data参数类
typedef struct tagPatternVirtualRectSearchTrainDataParam : public CBaseTrainDataParam
{
	...
	double			m_dContrastThre[RECT_LINE_NUM] ;			// 对比度阈值
	long			m_lFilterHalfWidth[RECT_LINE_NUM];		// 滤波器半宽
	int				m_nOutLineNum[RECT_LINE_NUM];			// 局外点
	EdgePolarity	m_edgeProperty[RECT_LINE_NUM];			// 边缘属性
	bool			m_bSortByScore[RECT_LINE_NUM];			// 按分数排序

	tagPatternVirtualRectSearchTrainDataParam()
	{
...
}CPatternVirtualRectSearchTrainDataParam;

// PatternVirtualRectSearch定位工具训练Gui参数类
typedef struct tagPatternVirtualRectSearchTrainGuiParam : public CBaseTrainGuiParam
{
	...
	// 找线工具1
	int				m_nCaliperNum[RECT_LINE_NUM];			// 卡尺个数
	double			m_dStartX[RECT_LINE_NUM];				// 起始点X
	double			m_dStartY[RECT_LINE_NUM];				// 起始点Y
	double			m_dEndX[RECT_LINE_NUM];					// 终止点X
	double			m_dEndY[RECT_LINE_NUM];					// 终止点Y
	double			m_dCaliperProjectLen[RECT_LINE_NUM];		// 投影长度
	double			m_dCaliperSearchLen[RECT_LINE_NUM];		// 搜索长度
	scRadian		m_CaliperSearchRotation[RECT_LINE_NUM];	// 搜索角度
	...
	// 找线工具掩摸
	svStd vector<double> m_vdCaliperMaskData[RECT_LINE_NUM];			// 掩膜数据
	svStd vector<bool>   m_vbCaliperMask[RECT_LINE_NUM];				// 卡尺是否被掩膜
	bool			     m_bEnableLineMask[RECT_LINE_NUM];			// 是否启用掩膜
	...
	tagPatternVirtualRectSearchTrainGuiParam()
	{
		m_nCaliperNum[0]				= 2;
		m_dStartX[0]					= 100.0;
		m_dStartY[0]					= 100.0;
...
}CPatternVirtualRectSearchTrainGuiParam;

//PatternnVirtualRectSearch工具修正模式类
typedef enum tagRiviseModeType
{
	r12to34				= 0,	// 1、2修正3、4
	r12to1234			= 1,	// 1、2修正1、2、3、4
	r34to12				= 2,	// 3、4修正1、2
	r34to1234			= 3,	// 3、4修正1、2、3、4
}CRiviseMode;


// PatternnRectSearch定位工具搜索Data参数类
typedef struct tagPatternRectSearchSearchDataParam : public CBaseSearchDataParam
{
	int		m_nSearchNum;								// 搜索个数
	double	m_dAcceptThre;								// 接收阈值
	double  m_dConfusionThre;							// 混淆阈值。混淆阈值不小于接收阈值

	int		m_nRiviseMode;								//	1--1、2修正3、4
														//	2--1、2修正1、2、3、4
														//	3--3、4修正1、2
														//	4--3、4修正1、2、3、4

	tagPatternRectSearchSearchDataParam()
	{
		m_nSearchNum	= 1;
		m_dAcceptThre	= 0.6;
...
}CPatternVirtualRectSearchSearchDataParam;

// PatternVirtualRectSearch定位工具搜索Gui参数类
typedef struct tagPatternVirtualRectSearchGuiParam : public CBaseSearchGuiParam
{
	COLORREF        m_LineColor;						// 图形颜色
	int				m_nLineWidth;						// 图形线宽
	...
	int				m_nResultOutMode;					//输出结果模式


	tagPatternVirtualRectSearchGuiParam()
	{
...
}CPatternVirtualRectSearchSearchGuiParam;

// PatternSearch训练结果
typedef struct tagPatternVirtualRectSearchTrainResult
{
	double m_dModelX; // 参考点X					
	double m_dModelY; // 参考点Y

	double m_dStartX[RECT_LINE_NUM]; // 线段起点X
	double m_dStartY[RECT_LINE_NUM]; // 线段起点Y
	double m_dEndX[RECT_LINE_NUM];	  // 线段终点X
	double m_dEndY[RECT_LINE_NUM];	  // 线段终点Y

	tagPatternVirtualRectSearchTrainResult()
	{
		m_dModelX = 0;
		m_dModelY = 0;
...
}CPatternVirtualRectSearchTrainResult;

class CPatternVirtualRectSearchTool :public CBaseSearchTool
{
public:
	CPatternVirtualRectSearchTool(void);
	CPatternVirtualRectSearchTool(CPatternVirtualRectSearchTool &cPatternVirtualRectSearchTool);
	CPatternVirtualRectSearchTool& operator=(CPatternVirtualRectSearchTool &cPatternVirtualRectSearchTool);
	~CPatternVirtualRectSearchTool(void);
...
private:
	...
	scFindLine							m_FindLineTool[RECT_LINE_NUM];		// 找线工具
	...
	CPatternVirtualRectSearchTrainDataParam		m_tmpTrainDataParam;			// 当前训练Data参数
	CPatternVirtualRectSearchTrainGuiParam			m_tmpTrainGuiParam;				// 当前训练Gui参数	
	CPatternVirtualRectSearchSearchDataParam		m_tmpSearchDataParam;			// 当前搜索Data参数	
	CPatternVirtualRectSearchSearchGuiParam		m_tmpSearchGuiParam;			// 当前搜索Gui参数
	CPatternVirtualRectSearchTrainResult			m_tmpTrainResult;				// 当前训练结果
	cpImage							m_tmpImageTrain;				// 当前训练图像
	cpImage							m_tmpImageMask;					// 当前掩模图像
	CPatternVirtualRectSearchTrainDataParam		m_TrainDataParam;				// 训练Data参数
	CPatternVirtualRectSearchTrainGuiParam			m_TrainGuiParam;				// 训练Gui参数	
	CPatternVirtualRectSearchSearchDataParam		m_SearchDataParam;				// 搜索Data参数	
	CPatternVirtualRectSearchSearchGuiParam		m_SearchGuiParam;				// 搜索Gui参数	
	CPatternVirtualRectSearchTrainResult			m_TrainResult;					// 训练结果
	...
	scGuiFindLine                       m_GuiFindLine[RECT_LINE_NUM];		// 两个找线工具
	scGuiLine                           m_GuiTrainResultLine[RECT_LINE_NUM];	// 训练结果线段
	...
	scGuiLine							m_GuiSearchLine[MAXRESNUM*RECT_LINE_NUM];
	scGuiCross							m_GuiSearchLineCross[MAXRESNUM*RECT_LINE_NUM];
