...
void CCornerCheckSearchTool::Initialize_Member_Variables()
{
	...
	case 1:
		...
		m_GuiTrainRect1.SetLabel(_T("检查工具训练区域"));
		m_GuiSearchRect1.SetLabel(_T("检查工具搜索区域"));
		m_GuiModelPoint1.SetLabel(_T("检查工具参考点"));
		m_GuiTrainAffineRect1.SetLabel(_T("几何检查工具训练区域"));
	...
	// 线距检测工具初始化属性
	m_GuiInspectFindLine[0].SetStartXYEndXY(340, 350, 340, 200);
	m_GuiInspectFindLine[0].SetCaliperParams(20,scDegree(90), 50, 5, TRUE);

	m_GuiInspectFindLine[1].SetStartXYEndXY(440, 350, 440, 200);
	m_GuiInspectFindLine[1].SetCaliperParams(20,scDegree(90), 50, 5, TRUE);

	m_GuiInspectFindLine[2].SetStartXYEndXY(430, 150, 580, 150);
	m_GuiInspectFindLine[2].SetCaliperParams(20,scDegree(90), 50, 5, TRUE);

	m_GuiInspectFindLine[3].SetStartXYEndXY(430, 250, 580, 250);
	m_GuiInspectFindLine[3].SetCaliperParams(20,scDegree(90), 50, 5, TRUE);

	//卡尺不可见，矩形区域可见
	m_GuiInspectFindLine[0].SetCaliperVisible(FALSE);
	m_GuiInspectFindLine[0].SetAffineRectFindLineVisible(TRUE);
	m_GuiInspectFindLine[1].SetCaliperVisible(FALSE);
	m_GuiInspectFindLine[1].SetAffineRectFindLineVisible(TRUE);
	m_GuiInspectFindLine[2].SetCaliperVisible(FALSE);
	m_GuiInspectFindLine[2].SetAffineRectFindLineVisible(TRUE);
	m_GuiInspectFindLine[3].SetCaliperVisible(FALSE);
	m_GuiInspectFindLine[3].SetAffineRectFindLineVisible(TRUE);
	int i=0;
	...
	m_GuiInspectFindLine[0].SetLabel(str);
	m_GuiInspectFindLine[0].SetLabelVisible(TRUE);

	m_GuiInspectFindLine[1].SetLabel(str1);
	m_GuiInspectFindLine[1].SetLabelVisible(TRUE);

	m_GuiInspectFindLine[2].SetLabel(str2);
	m_GuiInspectFindLine[2].SetLabelVisible(TRUE);

	m_GuiInspectFindLine[3].SetLabel(str3);
	m_GuiInspectFindLine[3].SetLabelVisible(TRUE);

	m_GuiInspectFindLine[4].SetLabel(str4);
	m_GuiInspectFindLine[4].SetLabelVisible(TRUE);

	m_GuiFindCircle.SetLabel(str5);
	m_GuiFindCircle.SetLabelVisible(TRUE);

	CreateGUID(m_strKeyText);
}

CCornerCheckSearchTool::CCornerCheckSearchTool(void)
{
	...
	for(int i =0;i<INSPCET_TOOL_LINE_NUMS+1;i++)
	{
		...
		{
			m_GuiInspectFindLine[i].EnableGuiDOF(scGuiFindLine::eGuiDOFStart|scGuiFindLine::eGuiDOFEnd|scGuiFindLine::eGuiDOFMarker|scGuiFindLine::eGuiDOFPosition|scGuiFindLine::eGuiDOFMask);
		}
		else
		{
			m_GuiInspectFindLine[i].EnableGuiDOF(scGuiFindLine::eGuiDOFStart|scGuiFindLine::eGuiDOFEnd|scGuiFindLine::eGuiDOFMarker|scGuiFindLine::eGuiDOFPosition);
		}
	}
	//训练参数与GUI一致
	// 的粗定位训练参数
	...
	// Line
	for (i=0; i<INSPCET_TOOL_LINE_NUMS+1; i++)
	{
		m_InspectFindLineTool[i].GetCaliperProperty(m_tmpTrainDataParam.m_dContrastThre1[i], m_tmpTrainDataParam.m_edgeProperty1[i], 
			m_tmpTrainDataParam.m_lFilterHalfWidth1[i]);
		m_tmpTrainDataParam.m_nOutLineNum1[i]			= 20;
		m_tmpTrainGuiParam.m_nCaliperNum1[i]			= m_GuiInspectFindLine[i].GetCaliperNum();
		m_GuiInspectFindLine[i].GetStartXYEndXY(m_tmpTrainGuiParam.m_dStartX1[i], m_tmpTrainGuiParam.m_dStartY1[i], m_tmpTrainGuiParam.m_dEndX1[i],
			m_tmpTrainGuiParam.m_dEndY1[i]);
		m_tmpTrainGuiParam.m_dCaliperProjectLen1[i]		= m_GuiInspectFindLine[i].GetCaliperProjectionLength();
		m_tmpTrainGuiParam.m_dCaliperSearchLen1[i]		= m_GuiInspectFindLine[i].GetCaliperSearchLength();
		m_tmpTrainGuiParam.m_CaliperSearchRotation1[i]	= m_GuiInspectFindLine[i].GetCaliperSearchDirection();
		m_tmpTrainGuiParam.m_bEnableLineMask[i]			= false;
		m_tmpTrainDataParam.m_bEnableDualEdge1[i]		= false;
	...
	m_tmpSearchDataParam.m_bCheckAfterSearch = true;
...
CCornerCheckSearchTool::CCornerCheckSearchTool(CCornerCheckSearchTool &cCornerCheckSearchTool)
{
	if (cCornerCheckSearchTool.GetSearchToolType() != eCornerSearchCheckInspectTool)
	{
		return;
	}
	...
	for(int i =0;i<INSPCET_TOOL_LINE_NUMS+1;i++)
	{
		if (cCornerCheckSearchTool.m_TrainGuiParam.m_bEnableLineMask[i])
		{
			m_GuiInspectFindLine[i].EnableGuiDOF(scGuiFindLine::eGuiDOFStart|scGuiFindLine::eGuiDOFEnd|scGuiFindLine::eGuiDOFMarker|scGuiFindLine::eGuiDOFPosition|scGuiFindLine::eGuiDOFMask);
		}
		else
	...
	// 赋值
    m_CamHeight								= cCornerCheckSearchTool.m_CamHeight;
    m_CamWidth								= cCornerCheckSearchTool.m_CamWidth;
	m_TrainDataParam						= cCornerCheckSearchTool.m_TrainDataParam;
	m_TrainGuiParam							= cCornerCheckSearchTool.m_TrainGuiParam;
	m_SearchDataParam						= cCornerCheckSearchTool.m_SearchDataParam;
	m_SearchGuiParam						= cCornerCheckSearchTool.m_SearchGuiParam;
	m_TrainResult							= cCornerCheckSearchTool.m_TrainResult;
	m_ImageTrain							= cCornerCheckSearchTool.m_ImageTrain;
	m_ImageMask								= cCornerCheckSearchTool.m_ImageMask;
	m_ImageMask1							= cCornerCheckSearchTool.m_ImageMask1;
	m_bTrainGuiShow							= cCornerCheckSearchTool.m_bTrainGuiShow;
	m_bTrainResultGuiShow					= cCornerCheckSearchTool.m_bTrainResultGuiShow;
	m_bSearchResultGuiShow					= cCornerCheckSearchTool.m_bSearchResultGuiShow;
	m_bTrainOK								= cCornerCheckSearchTool.m_bTrainOK;
	m_pGuiDisplay							= cCornerCheckSearchTool.m_pGuiDisplay;
	...
	cCornerCheckSearchTool.UpdateSearchGuiData(true);
	cCornerCheckSearchTool.UpdateTrainGuiData(true);
    

	SetTrainDataParam(&cCornerCheckSearchTool.m_tmpTrainDataParam);
	SetTrainGuiParam(&cCornerCheckSearchTool.m_tmpTrainGuiParam);
	SetSearchDataParam(&cCornerCheckSearchTool.m_tmpSearchDataParam);
	SetSearchGuiParam(&cCornerCheckSearchTool.m_tmpSearchGuiParam);
	SetTrainImage(cCornerCheckSearchTool.m_tmpImageTrain);
	SetMaskImage(cCornerCheckSearchTool.m_tmpImageMask);
	SetMaskImage2(cCornerCheckSearchTool.m_tmpImageMask1);
	...
	if (m_bTrainOK)
	{
		...
		{
			m_PatternSearchTool =cCornerCheckSearchTool.m_PatternSearchTool;
			m_bTrainOK = cCornerCheckSearchTool.m_bTrainOK;
		}
		...
		if (m_tmpTrainDataParam.m_bEnableCheck && m_tmpTrainDataParam.GetInspectToolType() == ePatternSearchTool)
		{
			m_PatternSearchTool1 = cCornerCheckSearchTool.m_PatternSearchTool1;
			m_bTrainOK = cCornerCheckSearchTool.m_bTrainOK;
		}

		if (m_tmpTrainDataParam.m_bEnableCheck && m_tmpTrainDataParam.GetInspectToolType() == eQuickSearchTool)
		{
			// 设置训练参数
...
CCornerCheckSearchTool& CCornerCheckSearchTool::operator=(CCornerCheckSearchTool &cCornerCheckSearchTool)
{
	if (this == &cCornerCheckSearchTool)
	{
		return *this;	
	}
...
CSearchToolType CCornerCheckSearchTool::GetSearchToolType()
{
	return eCornerSearchCheckInspectTool;
}

// 设置显示控件指针
bool CCornerCheckSearchTool::SetDisplay(scGuiDisplay *pGuiDisplay)
{
	ClearAllGui();
...
// 设置训练结果显示控件指针
bool CCornerCheckSearchTool::SetTrainResultDisplay(scGuiDisplay *pGuiDisplay)
{
	ClearAllGui();
...
// 设置训练结果显示控件指针
bool CCornerCheckSearchTool::SetTrainResultDisplay2(scGuiDisplay *pGuiDisplay)
{
	ClearAllGui();
...
// 获取显示控件指针										
scGuiDisplay* CCornerCheckSearchTool::GetDisplay()
{
	return m_pGuiDisplay;
}

// 获取训练结果显示控件指针
scGuiDisplay* CCornerCheckSearchTool::GetTrainResultDisplay()
{
	return m_pGuiTrainResultDisplay;
}

// 获取训练结果显示控件指针
scGuiDisplay* CCornerCheckSearchTool::GetTrainResultDisplay2()
{
	return m_pGuiTrainResultDisplay2;
}

// 设置Gui图形的关键字														
bool CCornerCheckSearchTool::SetGuiKeyText(CString strGuiKeyText)									
{
...
// 获取Gui图形的关键字
bool CCornerCheckSearchTool::GetGuiKeyText(CString &strGuiKeyText)
{
...
// 更新训练界面显示					
bool CCornerCheckSearchTool::ShowTrainGui(bool bShow)
{
...
// 是否显示训练结果界面
bool CCornerCheckSearchTool::ShowTrainResultGui(bool bShow)
{
	return false;
}

// 更新搜索界面显示									
bool CCornerCheckSearchTool::ShowSearchResultGui(bool bShow)							
{
...
// 更新训练界面显示					
bool CCornerCheckSearchTool::UpdateTrainGuiDisplay()
{	
	if (m_tmpTrainDataParam.GetSubSearchToolType() == ePatternCornerSearchTool)
	{
	...
	{
		m_GuiTrainRect1.SetVisible(m_bTrainGuiShow && m_tmpTrainGuiParam.m_bTrainRectVisible1 && m_tmpTrainDataParam.m_bEnableCheck);
		m_GuiModelPoint1.SetVisible(m_bTrainGuiShow && m_tmpTrainGuiParam.m_bModelPointVisible1 && m_tmpTrainDataParam.m_bEnableCheck);
		m_GuiSearchRect1.SetVisible(m_bTrainGuiShow && m_tmpTrainGuiParam.m_bSearchRectVisible1 && m_tmpTrainDataParam.m_bEnableCheck);
		m_GuiTrainAffineRect1.SetVisible(FALSE);
	} 
	if ( m_tmpTrainDataParam.GetInspectToolType() == eQuickSearchTool)
	{
		m_GuiTrainAffineRect1.SetVisible(m_bTrainGuiShow && m_tmpTrainGuiParam.m_bTrainRectVisible1 && m_tmpTrainDataParam.m_bEnableCheck);
		m_GuiModelPoint1.SetVisible(m_bTrainGuiShow && m_tmpTrainGuiParam.m_bModelPointVisible1 && m_tmpTrainDataParam.m_bEnableCheck);
		m_GuiSearchRect1.SetVisible(m_bTrainGuiShow && m_tmpTrainGuiParam.m_bSearchRectVisible1 && m_tmpTrainDataParam.m_bEnableCheck);
	...
	{
		int nMark = i/2;
		m_GuiInspectFindLine[i].SetVisible(m_bTrainGuiShow && m_tmpTrainGuiParam.m_bLineVisible && m_tmpTrainDataParam.m_bEnableInspect[nMark] && nMark != 4);
		if (i == 4)
		{
			m_GuiInspectFindLine[i].SetVisible(m_bTrainGuiShow && m_tmpTrainGuiParam.m_bLineVisible && m_tmpTrainDataParam.m_bEnableInspect[nMark] && !m_tmpTrainDataParam.m_bEnableInspect[3]);
			m_GuiFindCircle.SetVisible(m_bTrainGuiShow && m_tmpTrainGuiParam.m_bLineVisible && m_tmpTrainDataParam.m_bEnableInspect[nMark] || m_bTrainGuiShow && m_tmpTrainGuiParam.m_bLineVisible && m_tmpTrainDataParam.m_bEnableInspect[3]);
		}
	}
	...
	//	if (m_tmpTrainDataParam.m_bEnableCheck && m_tmpTrainDataParam.GetInspectToolType() == eQuickSearchTool)
		...
		for (int i=0;i<INSPCET_TOOL_LINE_NUMS/2;i++)
		{
			...
			m_pGuiInteractiveContainer->AddItem(&m_GuiInspectFindLine[i], strKey);
			m_pGuiInteractiveContainer->AddItem(&m_GuiInspectFindLine[i+2], strKey1);
		}
		{
			CString strKey;
			...
			m_pGuiInteractiveContainer->AddItem(&m_GuiInspectFindLine[4], strKey);
			m_pGuiInteractiveContainer->AddItem(&m_GuiFindCircle, strKey1);
		}
...
bool CCornerCheckSearchTool::UpdateTrainResultGuiDisplay()
{
	// 训练结果
	...
	for (int i=0;i<INSPCET_TOOL_LINE_NUMS+1;i++)
	{
		m_GuiInspectTrainResultLine[i].SetVisible(m_bTrainResultGuiShow && m_tmpTrainDataParam.m_bEnableInspect[(int)((i+1)/2)] && i != 4);
		if(i == 4)
		{
			m_GuiInspectTrainResultLine[i].SetVisible(m_bTrainResultGuiShow && m_tmpTrainDataParam.m_bEnableInspect[(int)((i+1)/2)] && !m_tmpTrainDataParam.m_bEnableInspect[3]);
			m_GuiTrainResultCircle.SetVisible(m_bTrainResultGuiShow && m_tmpTrainDataParam.m_bEnableInspect[(int)((i+1)/2)] || m_bTrainResultGuiShow && m_tmpTrainDataParam.m_bEnableInspect[3]);
		}
	}
	...
	if (m_pGuiDisplay && ::IsWindow(m_hDispWnd))
	{
		...
		for (int i=0;i<INSPCET_TOOL_LINE_NUMS+1;i++)
		{
			...
			m_pGuiStaticContainer->AddItem(&m_GuiInspectTrainResultLine[i],str);
			if(i == 4)
			{
...
// 更新搜索界面显示									
bool CCornerCheckSearchTool::UpdateSearchResultGuiDisplay()							
{
	int i=0;
	...
	{
		m_pGuiSearchCircleArray[i]->SetVisible(m_bSearchResultGuiShow && m_tmpSearchGuiParam.m_bLineVisible);
	}

	for (i=0; i<m_pGuiSearchCircleCenterArray.size(); i++)
	{
		m_pGuiSearchCircleCenterArray[i]->SetVisible(m_bSearchResultGuiShow && m_tmpSearchGuiParam.m_bLineVisible);
	}
	...
	for (i=0; i<m_pGuiInspectFitPoint1Array.size(); i++)
	{
		m_pGuiInspectFitPoint1Array[i]->SetVisible(m_bSearchResultGuiShow && m_tmpSearchGuiParam.m_bInspectVisibleFitPoint[0] && m_tmpTrainDataParam.m_bEnableInspect[0]);
	}

	for (i=0; i<m_pGuiInspectFitPoint2Array.size(); i++)
	{
		m_pGuiInspectFitPoint2Array[i]->SetVisible(m_bSearchResultGuiShow && m_tmpSearchGuiParam.m_bInspectVisibleFitPoint[1] && m_tmpTrainDataParam.m_bEnableInspect[0]);
	}

	for (i=0; i<m_pGuiInspectFitPoint3Array.size(); i++)
	{
		m_pGuiInspectFitPoint3Array[i]->SetVisible(m_bSearchResultGuiShow && m_tmpSearchGuiParam.m_bInspectVisibleFitPoint[2] && m_tmpTrainDataParam.m_bEnableInspect[1]);
	}

	for (i=0; i<m_pGuiInspectFitPoint4Array.size(); i++)
	{
		m_pGuiInspectFitPoint4Array[i]->SetVisible(m_bSearchResultGuiShow && m_tmpSearchGuiParam.m_bInspectVisibleFitPoint[3] && m_tmpTrainDataParam.m_bEnableInspect[1]);
	}

	for (i=0; i<m_pGuiInspectFitPoint5Array.size(); i++)
	{
		m_pGuiInspectFitPoint5Array[i]->SetVisible(m_bSearchResultGuiShow && m_tmpSearchGuiParam.m_bInspectVisibleFitPoint[4] && m_tmpTrainDataParam.m_bEnableInspect[2]);
	}
	for (i=0; i<m_pGuiInspectFitPoint6Array.size(); i++)
	{
		m_pGuiInspectFitPoint6Array[i]->SetVisible(m_bSearchResultGuiShow && m_tmpSearchGuiParam.m_bEnableShowFitCirclePoint && m_tmpTrainDataParam.m_bEnableInspect[2] || m_bSearchResultGuiShow && m_tmpSearchGuiParam.m_bEnableShowFitCirclePoint && m_tmpTrainDataParam.m_bEnableInspect[3]);
	}
	if (m_pGuiDisplay && ::IsWindow(m_hDispWnd))
	{	
		m_pGuiStaticContainer = m_pGuiDisplay->GetStaticContainer();
		...
		if (m_bSearchResultGuiShow) // 立即更新
		{
			...
			for (i=0;i<m_pGuiInspectFitPoint1Array.size();i++)
			{
				m_pGuiStaticContainer->AddItem(m_pGuiInspectFitPoint1Array[i],m_pGuiInspectFitPoint1Array[i]->GetTipText());
			}

			for (i=0;i<m_pGuiInspectFitPoint2Array.size();i++)
			{
				m_pGuiStaticContainer->AddItem(m_pGuiInspectFitPoint2Array[i],m_pGuiInspectFitPoint2Array[i]->GetTipText());
			}

			for (i=0;i<m_pGuiInspectFitPoint3Array.size();i++)
			{
				m_pGuiStaticContainer->AddItem(m_pGuiInspectFitPoint3Array[i],m_pGuiInspectFitPoint3Array[i]->GetTipText());
			}

			for (i=0;i<m_pGuiInspectFitPoint4Array.size();i++)
			{
				m_pGuiStaticContainer->AddItem(m_pGuiInspectFitPoint4Array[i],m_pGuiInspectFitPoint4Array[i]->GetTipText());
			}
			for (i=0;i<m_pGuiInspectFitPoint5Array.size();i++)
			{
				m_pGuiStaticContainer->AddItem(m_pGuiInspectFitPoint5Array[i],m_pGuiInspectFitPoint5Array[i]->GetTipText());
			}
			for (i=0;i<m_pGuiInspectFitPoint6Array.size();i++)
			{
				m_pGuiStaticContainer->AddItem(m_pGuiInspectFitPoint6Array[i],m_pGuiInspectFitPoint6Array[i]->GetTipText());
			}
		}
...
// 更新界面显示
bool CCornerCheckSearchTool::UpdateGuiDisplay()
{
	UpdateTrainGuiDisplay();
...
// 设置训练Data参数	
bool CCornerCheckSearchTool::SetTrainDataParam(CBaseTrainDataParam *pTrainDataParam)
{
	if (!pTrainDataParam)
	{
		return false;
	...
	if (pTrainDataParam->GetSearchToolType() != eCornerSearchCheckInspectTool)
	{
		return false;
	}

	m_tmpTrainDataParam = *((CCornerCheckSearchTrainDataParam*)pTrainDataParam);

	return true;
}

bool CCornerCheckSearchTool::SetTrainDataParamForce(CBaseTrainDataParam *pTrainDataParam)
{
	if (!pTrainDataParam)
	{
		return false;
	...
	m_TrainDataParam = *((CCornerCheckSearchTrainDataParam*)pTrainDataParam);

	return true;
}

// 获取训练Data参数									
bool CCornerCheckSearchTool::GetTrainDataParam(CBaseTrainDataParam *pTrainDataParam)
{
	if (!pTrainDataParam)
	{
		return false;
	...
	*((CCornerCheckSearchTrainDataParam*)pTrainDataParam) = m_tmpTrainDataParam;

	return true;
}

// 获取搜索Gui参数						
bool CCornerCheckSearchTool::SetTrainGuiParam(CBaseTrainGuiParam *pTrainGuiParam)
{
	if (!pTrainGuiParam)
	{
		return false;
	...
	if (pTrainGuiParam->GetSearchToolType() != eCornerSearchCheckInspectTool)
	{
		return false;
	}

	m_tmpTrainGuiParam = *((CCornerCheckSearchTrainGuiParam*)pTrainGuiParam);
...
// 获取搜索Gui参数							
bool CCornerCheckSearchTool::GetTrainGuiParam(CBaseTrainGuiParam *pTrainGuiParam)
{
	if (!pTrainGuiParam)
	{
		return false;
	...
	*((CCornerCheckSearchTrainGuiParam*)pTrainGuiParam) = m_tmpTrainGuiParam;

	return true;
}

// 设置训练原始图像				
bool CCornerCheckSearchTool::SetTrainImage(const cpImage &ImageTrain)
{
...
// 获取训练原始图像									
bool CCornerCheckSearchTool::GetTrainImage(cpImage &ImageTrain)
{
...
// 设置训练原始图像									
bool CCornerCheckSearchTool::SetMaskImage(const cpImage &ImageMask)
{
	m_tmpImageMask = ImageMask;

	return true;
}
bool CCornerCheckSearchTool::SetMaskImage2(const cpImage &ImageMask)
{
...
// 获取训练原始图像								
bool CCornerCheckSearchTool::GetMaskImage(cpImage &ImageMask)
{
...
bool CCornerCheckSearchTool::GetMaskImage2(cpImage &ImageMask)
{
...
// 执行训练											
bool CCornerCheckSearchTool::Train()
{
	...
	if (m_tmpTrainDataParam.GetSubSearchToolType() == ePatternCornerSearchTool)
	{
		...
		if (m_tmpTrainDataParam.m_bMaskEnable)
		{
			...
			if(m_tmpTrainDataParam.m_bEnableCorner)
			{
				...
				//执行线距检测工具的找线训练
				if(!ExecuteInspectTrain()) return false;
...
// 执行检测训练	
bool CCornerCheckSearchTool::ExecuteInspectTrain()
{
	scLineSeg InspectLineSeg[INSPCET_TOOL_LINE_NUMS+1];
	bool bISEnable =false;

	// 如果都没有启用线距工具,就不用执行 线距的训练 直接返回true
	for(int i=0;i<INSPCET_TOOL_LINE_NUMS+1;i++)
	{
		if (i == 4)
		{
			bISEnable = (bISEnable || m_tmpTrainDataParam.m_bEnableInspect[(int)(i/2)] || m_tmpTrainDataParam.m_bEnableInspect[3]);
		}
		else
		{
			bISEnable = (bISEnable || m_tmpTrainDataParam.m_bEnableInspect[(int)(i/2)]);
		}
	...
	for (int i=0;i<INSPCET_TOOL_LINE_NUMS+1;i++)
	{
		if (i == 4)
		{
			if(!m_tmpTrainDataParam.m_bEnableInspect[(int)(i/2)] && !m_tmpTrainDataParam.m_bEnableInspect[3])
			{
				continue;
			}
		}
		...
		{
			if(!m_tmpTrainDataParam.m_bEnableInspect[(int)(i/2)])
			{
				continue;
			}
		}

		if (i == 4 && m_tmpTrainDataParam.m_bEnableInspect[3])
		{
			if (i == 4)
			{
				///找圆
				...
				m_FindCircleTool.EnableCaliperSortByScore(m_tmpTrainDataParam.m_bSortByScore2);
				...
				if (m_tmpTrainDataParam.m_bEnableCircleDualEdge)
				{
					...
					m_FindCircleTool.SetCaliperProperty(m_tmpTrainDataParam.m_dCircleContrastThre, m_tmpTrainDataParam.m_CircleedgeProperty,
				...
				if(bIsProcessImage)
				{
					bExecute = m_FindCircleTool.Execute(tmpImage, cCircle, dRMS);
				}
				else
				{
					bExecute = m_FindCircleTool.Execute(m_tmpImageTrain, cCircle, dRMS);
				}

				if (!bExecute)
				{
					m_nResultErrNumber=-17;       // 找圆失败
					return false;
				}
		...
		{
			m_InspectFindLineTool[i].SetCaliperNum(m_tmpTrainGuiParam.m_nCaliperNum1[i]);
			m_InspectFindLineTool[i].SetCaliperParams(m_tmpTrainGuiParam.m_nCaliperNum1[i], m_tmpTrainGuiParam.m_CaliperSearchRotation1[i], m_tmpTrainGuiParam.m_dCaliperSearchLen1[i],m_tmpTrainGuiParam.m_dCaliperProjectLen1[i]);
			//m_InspectFindLineTool[i].SetCaliperProperty(m_tmpTrainDataParam.m_dContrastThre1[i], m_tmpTrainDataParam.m_edgeProperty1[i], m_tmpTrainDataParam.m_lFilterHalfWidth1[i]);

			if (m_tmpTrainDataParam.m_bEnableDualEdge1[i])
			{
				EdgePolarity tmpEdgePolarity = eDontCare;
				if (m_tmpTrainDataParam.m_edgeProperty1[i] == eDarkToLight)
				{
					tmpEdgePolarity = eLightToDark;
				} 
				else if (m_tmpTrainDataParam.m_edgeProperty1[i] == eLightToDark)
				{
					tmpEdgePolarity = eDarkToLight;
				}

				m_InspectFindLineTool[i].SetCaliperProperty(m_tmpTrainDataParam.m_dContrastThre1[i], m_tmpTrainDataParam.m_edgeProperty1[i],tmpEdgePolarity, m_tmpTrainDataParam.m_lFilterHalfWidth1[i]);
			}
			else
			{
				m_InspectFindLineTool[i].SetCaliperProperty(m_tmpTrainDataParam.m_dContrastThre1[i], m_tmpTrainDataParam.m_edgeProperty1[i], m_tmpTrainDataParam.m_lFilterHalfWidth1[i]);
			...
			m_InspectFindLineTool[i].SetCaliperMasked(m_tmpTrainGuiParam.m_vbCaliperMask_FindLine1[i]);
			m_InspectFindLineTool[i].SetOutLineNum(m_tmpTrainDataParam.m_nOutLineNum1[i], eRansacRobust);
			m_InspectFindLineTool[i].SetStartXYEndXY(m_tmpTrainGuiParam.m_dStartX1[i], m_tmpTrainGuiParam.m_dStartY1[i], 
				m_tmpTrainGuiParam.m_dEndX1[i], m_tmpTrainGuiParam.m_dEndY1[i]);

			double dInspectRMS;
			bool bExecute = false;
			if(bIsProcessImage)
			{
				bExecute = m_InspectFindLineTool[i].Execute(tmpImage, InspectLineSeg[i], dInspectRMS);
			}
			else
			{
				bExecute = m_InspectFindLineTool[i].Execute(m_tmpImageTrain, InspectLineSeg[i], dInspectRMS);
			}
			if (!bExecute)
			{	
				m_nResultErrNumber=-12-i;       //训练找线错误 
				return false;
			}
			if (i == 4)
			{
			...
			m_tmpTrainResult.m_dStartX1[i] = InspectLineSeg[i].GetP1().GetX();
			m_tmpTrainResult.m_dStartY1[i] = InspectLineSeg[i].GetP1().GetY();
			m_tmpTrainResult.m_dEndX1[i] = InspectLineSeg[i].GetP2().GetX();
			m_tmpTrainResult.m_dEndY1[i] = InspectLineSeg[i].GetP2().GetY();

			m_GuiInspectTrainResultLine[i].SetLine(InspectLineSeg[i].GetLine());
		}
...
// 获取训练结果图像														
bool CCornerCheckSearchTool::GetTrainResultImage(cpImage &ImageTrainResult)
{
	bool bRet = false;
	if (m_tmpTrainDataParam.GetSubSearchToolType() == eCornerSearchTool)
	{
...
bool CCornerCheckSearchTool::GetTrainResultImage2(cpImage &ImageTrainResult)
{
	bool bRet = false;
	if (m_tmpTrainDataParam.GetInspectToolType() == eQuickSearchTool && m_tmpTrainDataParam.m_bEnableCheck)
	{
		bRet = m_QuickSearchTool1.GetTrainModelImage(m_ImageTrainResult1);
	}
	if (m_tmpTrainDataParam.GetInspectToolType() == ePatternSearchTool && m_tmpTrainDataParam.m_bEnableCheck)
	{
...
// 获取训练结果
bool CCornerCheckSearchTool::GetTrainResult(CBaseTrainResult *pTrainResult)
{
	if (!pTrainResult)
	{
		return false;
	...
	if (pTrainResult->GetSearchToolType() != eCornerSearchCheckInspectTool)
	{
		return false;
	}

	*((CCornerCheckSearchTrainResult*)pTrainResult) = m_tmpTrainResult;

	return true;
}	

// 是否训练成功
bool CCornerCheckSearchTool::IsTrained()
{
	return m_bTrainOK;
}

// 设置搜索Data参数	
bool CCornerCheckSearchTool::SetSearchDataParam(CBaseSearchDataParam *pSearchDataParam)
{	
	if (!pSearchDataParam)
	{
		return false;
	...
	if (pSearchDataParam->GetSearchToolType() != eCornerSearchCheckInspectTool)
	{
		return false;
	}

	m_tmpSearchDataParam = *((CCornerCheckSearchSearchDataParam*)pSearchDataParam);

	return true;
}

// 获取搜索Data参数				
bool CCornerCheckSearchTool::GetSearchDataParam(CBaseSearchDataParam *pSearchDataParam)
{
	if (!pSearchDataParam)
	{
		return false;
	...
	*((CCornerCheckSearchSearchDataParam*)pSearchDataParam) = m_tmpSearchDataParam;

	return true;
}

// 设置搜索Gui参数					
bool CCornerCheckSearchTool::SetSearchGuiParam(CBaseSearchGuiParam *pSearchGuiParam)
{
	if (!pSearchGuiParam)
	{
		return false;
	...
	if (pSearchGuiParam->GetSearchToolType() != eCornerSearchCheckInspectTool)
	{
		return false;
	}

	m_tmpSearchGuiParam = *((CCornerCheckSearchSearchGuiParam*)pSearchGuiParam);
...
// 获取搜索Gui参数			
bool CCornerCheckSearchTool::GetSearchGuiParam(CBaseSearchGuiParam *pSearchGuiParam)
{
	if (!pSearchGuiParam)
	{
		return false;
	...
	*((CCornerCheckSearchSearchGuiParam*)pSearchGuiParam) = m_tmpSearchGuiParam;

	return true;
}

//
bool CCornerCheckSearchTool::ExecuteInspectSearch(const double dTransX,const double dTransY,const double dRotation, cpImage &ImageSearch, CResultL2LInspectDistance &ResultL2LInspectDistance)
{
	scLineSeg InspectResLine[INSPCET_TOOL_LINE_NUMS+1];
	vector<bool> bInspectOK;
	bInspectOK.resize((INSPCET_TOOL_LINE_NUMS/2)+1,false);
	vector<double>dInspectDistance;
	dInspectDistance.resize((INSPCET_TOOL_LINE_NUMS/2)+1,0.0);
	vector<double>dInspectDistancePixel;
	dInspectDistancePixel.resize((INSPCET_TOOL_LINE_NUMS/2)+1,0.0);

	vector<int> nInspect;
	nInspect.resize((INSPCET_TOOL_LINE_NUMS/2)+1,0);
	scCircle ResCircle;

	for (int i=0;i<(INSPCET_TOOL_LINE_NUMS/2)+1;i++)
	{
		if(!m_tmpTrainDataParam.m_bEnableInspect[i])
		{
			if (i == 2 && m_tmpTrainDataParam.m_bEnableInspect[3])
			{
			}
			else
			{
		...
		/*if (i == 2)
		{
			if(!m_tmpTrainDataParam.m_bEnableInspect[i] && !m_tmpTrainDataParam.m_bEnableInspect[3])
			{
				continue;
			}
		}
		...
		bool   bFindLine[2]={false,false};
		if (i == 2)
		{
			if (m_tmpTrainDataParam.m_bEnableInspect[3])
			{
				bFindLine[0] = true;
				// 求找圆工具变换后的位置
				...
				double dCircleRMS;
				bool bCircle = false;
				bFindLine[1] = m_FindCircleTool.Execute(ImageSearch, ResCircle, dCircleRMS);
			...
			else
			{
				...
				dTransStartX = cos(dRotation) * m_TrainGuiParam.m_dStartX1[4]-sin(dRotation) * m_TrainGuiParam.m_dStartY1[4] + dTransX;
				dTransStartY = sin(dRotation) * m_TrainGuiParam.m_dStartX1[4]  + cos(dRotation) * m_TrainGuiParam.m_dStartY1[4] + dTransY;
				dTransEndX = cos(dRotation) * m_TrainGuiParam.m_dEndX1[4]  - sin(dRotation) * m_TrainGuiParam.m_dEndY1[4] + dTransX;
				dTransEndY = sin(dRotation) * m_TrainGuiParam.m_dEndX1[4]  + cos(dRotation) * m_TrainGuiParam.m_dEndY1[4] + dTransY;

				m_InspectFindLineTool[4].SetCaliperParams(m_TrainGuiParam.m_nCaliperNum1[4], m_TrainGuiParam.m_CaliperSearchRotation1[4], m_TrainGuiParam.m_dCaliperSearchLen1[4]
				,m_TrainGuiParam.m_dCaliperProjectLen1[4]);
				m_InspectFindLineTool[4].SetStartXYEndXY(dTransStartX, dTransStartY, dTransEndX, dTransEndY);
				bFindLine[0] = m_InspectFindLineTool[4].Execute(ImageSearch,tmpscLineSeg,dRMS);
				InspectResLine[4] = tmpscLineSeg;
				// 求找圆工具变换后的位置
				double dTransCenterX, dTransCenterY;
				double dOriCenterX, dOriCenterY;
				dOriCenterX = m_TrainGuiParam.m_CircleCenter.GetX();
		...
		else
		{
			for (int j=0;j<2;j++) //两个找线
			{
				...
				double dRMS;
				scLineSeg tmpscLineSeg;
				dTransStartX = cos(dRotation) * m_TrainGuiParam.m_dStartX1[i*2+j]-sin(dRotation) * m_TrainGuiParam.m_dStartY1[i*2+j] + dTransX;
				dTransStartY = sin(dRotation) * m_TrainGuiParam.m_dStartX1[i*2+j]  + cos(dRotation) * m_TrainGuiParam.m_dStartY1[i*2+j] + dTransY;
				dTransEndX = cos(dRotation) * m_TrainGuiParam.m_dEndX1[i*2+j]  - sin(dRotation) * m_TrainGuiParam.m_dEndY1[i*2+j] + dTransX;
				dTransEndY = sin(dRotation) * m_TrainGuiParam.m_dEndX1[i*2+j]  + cos(dRotation) * m_TrainGuiParam.m_dEndY1[i*2+j] + dTransY;

				m_InspectFindLineTool[i*2+j].SetCaliperParams(m_TrainGuiParam.m_nCaliperNum1[i*2+j], m_TrainGuiParam.m_CaliperSearchRotation1[i*2+j], m_TrainGuiParam.m_dCaliperSearchLen1[i*2+j]
				,m_TrainGuiParam.m_dCaliperProjectLen1[i*2+j]);
				m_InspectFindLineTool[i*2+j].SetStartXYEndXY(dTransStartX, dTransStartY, dTransEndX, dTransEndY);
				//bFindLine[j] = m_InspectFindLineTool[i*2+j].Execute(ImageSearch, &tmpscLineSeg,dRMS);
				bFindLine[j] = m_InspectFindLineTool[i*2+j].Execute(ImageSearch,tmpscLineSeg,dRMS);
				InspectResLine[i*2+j] = tmpscLineSeg;
			}
		}
		...
		case 0:
			{
				...
				{
					CString str = _T("");
					str.Format(_T("_Inspect%d_FitPoint_%d"),i+1,0);
					ShowFitPoint(m_pGuiInspectFitPoint1Array,m_GuiInspectFitPoint1,&m_InspectFindLineTool[i*2],str);
				}
				if (bFindLine[1])
				{
					CString str = _T("");
					str.Format(_T("_Inspect%d_FitPoint_%d"),i+1,1);
					ShowFitPoint(m_pGuiInspectFitPoint2Array,m_GuiInspectFitPoint2,&m_InspectFindLineTool[i*2+1],str);
				}
		...
		case 1:
			{
				if (bFindLine[0])
				{
					CString str = _T("");
					str.Format(_T("_Inspect%d_FitPoint_%d"),i+1,0);
					ShowFitPoint(m_pGuiInspectFitPoint3Array,m_GuiInspectFitPoint3,&m_InspectFindLineTool[i*2],str);
				}
				...
				if (bFindLine[1])
				{
					CString str = _T("");
					str.Format(_T("_Inspect%d_FitPoint_%d"),i+1,1);
					ShowFitPoint(m_pGuiInspectFitPoint4Array,m_GuiInspectFitPoint4,&m_InspectFindLineTool[i*2+1],str);
				}
			}
			break;
		case 2:
			{
				...
				if (m_tmpTrainDataParam.m_bEnableInspect[3])
				{
					if (bFindLine[1])
					{
						CString str = _T("");
						str.Format(_T("_Inspect%d_FitPoint_%d"),i+1,1);
						ShowFitPoint(m_pGuiInspectFitPoint6Array,m_GuiInspectFitPoint6,&m_FindCircleTool,str);
					}
				} 
				else
				{
					if (bFindLine[0])
					{
						CString str = _T("");
						str.Format(_T("_Inspect%d_FitPoint_%d"),i+1,0);
						ShowFitPoint(m_pGuiInspectFitPoint5Array,m_GuiInspectFitPoint5,&m_InspectFindLineTool[i*2],str);
					}
					if (bFindLine[1])
					{
						CString str = _T("");
		...
		if (bFindLine[0] && bFindLine[1])
		{
			if (i == 2)
			{
				if (m_tmpTrainDataParam.m_bEnableInspect[3])
				{
					ResultL2LInspectDistance.m_dCircleCenterX = ResCircle.GetCenter().GetX();
					ResultL2LInspectDistance.m_dCircleCenterY = ResCircle.GetCenter().GetY();
					ResultL2LInspectDistance.m_dCircleRadius = ResCircle.GetRadius();
				} 
				else
				{
					ResultL2LInspectDistance.m_dStartPosX[4] =  InspectResLine[4].GetP1().GetX();
					ResultL2LInspectDistance.m_dStartPosY[4] =  InspectResLine[4].GetP1().GetY();
					ResultL2LInspectDistance.m_dEndPosX[4] =  InspectResLine[4].GetP2().GetX();
					ResultL2LInspectDistance.m_dEndPosY[4] =  InspectResLine[4].GetP2().GetY();
					ResultL2LInspectDistance.m_dCircleCenterX = ResCircle.GetCenter().GetX();
					ResultL2LInspectDistance.m_dCircleCenterY = ResCircle.GetCenter().GetY();
					ResultL2LInspectDistance.m_dCircleRadius = ResCircle.GetRadius();
				}
			...
			else
			{
				...
				for (int j=0; j<2; j++)
				{
					ResultL2LInspectDistance.m_dStartPosX[i*2+j] =  InspectResLine[i*2+j].GetP1().GetX();
					ResultL2LInspectDistance.m_dStartPosY[i*2+j] =  InspectResLine[i*2+j].GetP1().GetY();
					ResultL2LInspectDistance.m_dEndPosX[i*2+j] =  InspectResLine[i*2+j].GetP2().GetX();
					ResultL2LInspectDistance.m_dEndPosY[i*2+j] =  InspectResLine[i*2+j].GetP2().GetY();
					
					//CString strTmpKey;
					//strTmpKey.Format(_T("%s_InspectResultLine%d_%d"), m_strKeyText, j, i*2+j);

					//// 显示直线
					////scGuiLine *pTemLine = m_GuiSearchLine+i*2+j+10;
					////pTemLine->SetLine(InspectResLine[2*i+j].GetLine());
					...
					////显示线段
					//scGuiLineSeg *pTemLine = m_GuiSearchLineSeg +i*2+j;
					//pTemLine->SetLineSeg(InspectResLine[2*i+j]);
					////pTemLine->SetLineColor((m_tmpSearchGuiParam.m_LineColor));
					//pTemLine->SetLineColor(RGB(255,0,255));
					//pTemLine->SetLineWidth(m_tmpSearchGuiParam.m_nLineWidth);
					//pTemLine->SetTipTextEnabled(true);
					//pTemLine->SetTipText(strTmpKey);
					//pTemLine->SetLineSegArrowVisible(FALSE);
					//m_pGuiSearchLineArray.push_back(pTemLine);
				}
			}
			
			scGuiLineSeg tmpLineSeg[3];
			bInspectOK.at(i) = true;

			sc2Vector pt1,pt2,ptMid;
			sc2Vector pt1Project,pt2Project,ptMidProject;
			double d1,d2,dm,d1pixel,d2pixel,dmpixel;
			double dx,dy,dxpixel,dypixel;
			d1 = d2 = dm = d1pixel = d2pixel = dmpixel =0;
			dx = dy = dxpixel = dypixel = 0;
			if (i == 2)
			{
				...
				if (m_tmpTrainDataParam.m_bEnableInspect[3])
				{
					...
					d1pixel = ResultL2LInspectDistance.m_dCircleRadius;
					d1 = d1pixel * m_tmpSearchDataParam.m_dPixelRatio[i];
					tmpLineSeg[i].SetStartXYEndXY(ResCircle.GetCenter(),ResCircle.GetCenter());
					dInspectDistance.at(i) = d1;

					CString tmpd1 = _T("");
					tmpd1.Format(_T("半径:%.3fmm,"),dInspectDistance.at(i));
					m_strShowInspectInfo +=tmpd1;

					dInspectDistancePixel.at(i) = d1pixel;
				} 
				else
				{
					scLineSeg LineSeg; 
					LineSeg.SetP1(InspectResLine[4].GetP1());
					LineSeg.SetP2(InspectResLine[4].GetP2());
					pt1 = ResCircle.GetCenter();
					pt2 = LineSeg.GetLine().Project(pt1);
					//pt2 = LineSeg.NearestPoint(pt1);
					d1pixel = pt1.Distance(pt2);
					d1 = d1pixel * m_tmpSearchDataParam.m_dPixelRatio[i];
					tmpLineSeg[i].SetStartXYEndXY(pt1,pt2);
					dInspectDistance.at(i) = d1;

					CString tmpd1 = _T("");
					tmpd1.Format(_T("圆线距:%.3fmm,"),dInspectDistance.at(i));
					m_strShowInspectInfo +=tmpd1;
			...
			{
				if (m_tmpSearchDataParam.m_nDistanceMode[i]==0)
				{
					// L1 to L2; 从L1上点投影到L2;
					pt1 = InspectResLine[2*i].GetP1();
					pt2 = InspectResLine[2*i].GetP2();
					ptMid = (pt1 + pt2)/2;
					pt1Project = InspectResLine[2*i+1].GetLine().Project(pt1);
					pt2Project = InspectResLine[2*i+1].GetLine().Project(pt2);
					ptMidProject = InspectResLine[2*i+1].GetLine().Project(ptMid);

				}
				else if (m_tmpSearchDataParam.m_nDistanceMode[i] == 1)
				{
					// L2 to L1; 从L2上点投影到L1;

					pt1 = InspectResLine[2*i+1].GetP1();
					pt2 = InspectResLine[2*i+1].GetP2();
					ptMid = (pt1 + pt2)/2;

					pt1Project = InspectResLine[2*i].GetLine().Project(pt1);
					pt2Project = InspectResLine[2*i].GetLine().Project(pt2);
					ptMidProject = InspectResLine[2*i].GetLine().Project(ptMid);
				}

				d1pixel = pt1.Distance(pt1Project);
				d2pixel = pt2.Distance(pt2Project);
				dmpixel = ptMid.Distance(ptMidProject);

				d1 = d1pixel * m_tmpSearchDataParam.m_dPixelRatio[i];
				d2 = d2pixel * m_tmpSearchDataParam.m_dPixelRatio[i];
				dm = dmpixel * m_tmpSearchDataParam.m_dPixelRatio[i];

				if (m_tmpSearchDataParam.m_nDistanceType[i] == 0)
				{
					// 平均值
					dInspectDistance.at(i) = dm;
					dInspectDistancePixel.at(i) = dmpixel;
					tmpLineSeg[i].SetStartXYEndXY(ptMid,ptMidProject);

				}
				else if(m_tmpSearchDataParam.m_nDistanceType[i] == 1)
				{
					// 最大值
					dInspectDistance.at(i) = max(d1,d2);
					dInspectDistancePixel.at(i) =max(d1pixel,d2pixel);
					if(d1>d2)
					{
						tmpLineSeg[i].SetStartXYEndXY(pt1,pt1Project);

					}
					else
					{
						tmpLineSeg[i].SetStartXYEndXY(pt2,pt2Project);
					}
				}
				else if(m_tmpSearchDataParam.m_nDistanceType[i] == 2)
				{
					// 最小值
					dInspectDistance.at(i) = min(d1,d2);
					dInspectDistancePixel.at(i) =min(d1pixel,d2pixel);
					if(d1<d2)
					{
						tmpLineSeg[i].SetStartXYEndXY(pt1,pt1Project);
					}
				...
				else if(m_tmpSearchDataParam.m_nDistanceType[i] == 3)
				{
					// 水平距离
					//sc2Vector pt1,pt2,ptMid;
					sc2Vector ptx,pty,ptMid1x,ptMid2x;
					// L1  从L1上中点
					ptx = InspectResLine[2*i].GetP1();
					pty = InspectResLine[2*i].GetP2();
					ptMid1x = (ptx + pty)/2;

					// L2 从L2上中点;

					ptx = InspectResLine[2*i+1].GetP1();
					pty = InspectResLine[2*i+1].GetP2();
					ptMid2x = (ptx + pty)/2;

					// 水平距离
					//ptMid1x.SetY(0);
					ptMid2x.SetY(ptMid1x.GetY());
					dxpixel = ptMid1x.Distance(ptMid2x);
					dx = dxpixel * m_tmpSearchDataParam.m_dPixelRatio[i];

					// 平均值
					dInspectDistance.at(i) = dx;
					dInspectDistancePixel.at(i) = dxpixel;
					tmpLineSeg[i].SetStartXYEndXY(ptMid1x,ptMid2x);

					
					
				}
				else if(m_tmpSearchDataParam.m_nDistanceType[i] == 4)
				{
					// 垂直距离
					//sc2Vector pt1,pt2,ptMid;
					sc2Vector ptx,pty,ptMid1x,ptMid2x;
					...
					// 水平距离
					//ptMid1x.SetY(0);
					ptMid2x.SetX(ptMid1x.GetX());
					dypixel = ptMid1x.Distance(ptMid2x);
					dy = dypixel * m_tmpSearchDataParam.m_dPixelRatio[i];

					// 平均值
					dInspectDistance.at(i) = dy;
					dInspectDistancePixel.at(i) = dypixel;
					tmpLineSeg[i].SetStartXYEndXY(ptMid1x,ptMid2x);
				}
			...
			if (i == 2)
			{
				if (m_tmpSearchDataParam.m_bEnableMax1[i])
				{
					bInspectOK.at(i) = bInspectOK.at(i) && (dInspectDistance.at(i) < m_tmpSearchDataParam.m_dDistanceMax1[i]);
				} 

				if (m_tmpSearchDataParam.m_bEnableMin1[i])
				{
					bInspectOK.at(i) = bInspectOK.at(i) &&(dInspectDistance.at(i) > m_tmpSearchDataParam.m_dDistanceMin1[i]);
				} 
			...
			//	if (!m_tmpTrainDataParam.m_bEnableInspect[3])
			//	{
			//		tmpLineSeg[i].GetStartXY(ResultL2LInspectDistance.m_dMeasurePosX[i],ResultL2LInspectDistance.m_dMeasurePosY[i]);
			//		tmpLineSeg[i].GetEndXY(ResultL2LInspectDistance.m_dProjectPosX[i],ResultL2LInspectDistance.m_dProjectPosY[i]);

			//		CString tmpdDistanceMM = _T("");
			//		tmpdDistanceMM.Format(_T("线距%d:%.3fmm,"),i+1,dInspectDistance.at(i));
			//		m_strShowInspectInfo +=tmpdDistanceMM;
			//	}

			//		if (bInspectOK.at(i))
			//		{
			//			nInspect[i] = 1; //  1：找线OK，结果OK  
			//			ResultL2LInspectDistance.m_nResult[i] = nInspect[i];
			//		}else
			//		{
			//			nInspect[i] = -1;  // -1：找线OK，结果NG
			//			ResultL2LInspectDistance.m_nResult[i] = nInspect[i];
			//		}
			//	if (!m_tmpTrainDataParam.m_bEnableInspect[3])
			//	{
			//		// 显示测量距离线段
			//		CString strTmpKey;
			//		strTmpKey.Format(_T("%s_InspectResultDistanceLineSeg%d"), m_strKeyText, i);
			//		scGuiLineSeg *pTemLine = m_GuiSearchLineSeg +i*2+10;
			//		pTemLine->SetStartXYEndXY(tmpLineSeg[i].GetStartXY(),tmpLineSeg[i].GetEndXY());
			//		//pTemLine->SetLineSeg(tmpLineSeg[i]);
			//		if (bInspectOK.at(i))
			//		{
			//			pTemLine->SetLineColor(RGB(0,255,0));
			//		}
			//		else
			//		{
			//			pTemLine->SetLineColor(RGB(255,0,0));
			//		}

			//		pTemLine->SetLineWidth(m_tmpSearchGuiParam.m_nLineWidth);
			//		pTemLine->SetTipTextEnabled(true);
			//		pTemLine->SetTipText(strTmpKey);
			//		pTemLine->SetLineSegArrowVisible(TRUE);
			//		m_pGuiSearchLineArray.push_back(pTemLine);

			//		// GUI 显示测量结果
			//		CString StrTmpDis1 = _T("");
			//		StrTmpDis1.Format(_T("%.3fmm"),dInspectDistance.at(i));
			//		StrTmpDis1 = m_tmpSearchDataParam.m_strInspectDistanceValueLabel[i] + StrTmpDis1;

			//		scGuiText* pGuiText = m_GuiSearchLineText + i;
			...
			//		}
			//		strTmpKey.Format(_T("%s_InspectResultDistanceText_%d"), m_strKeyText, i);
			//		pGuiText->SetVisible(TRUE);
			//		pGuiText->SetXYRotation(sc2Vector(ResultL2LInspectDistance.m_dProjectPosX[i],ResultL2LInspectDistance.m_dProjectPosY[i]),scRadian(0));
			//		pGuiText->SetText(StrTmpDis1);
			//		pGuiText->SetTipText(strTmpKey);
			//		pGuiText->SetTipTextEnabled(TRUE);
			//		m_pGuiSearchLineTextArray.push_back(pGuiText);
			...
			//else
			{
				...
				tmpdDistanceMM.Format(_T("线距%d:%.3fmm,"),i+1,dInspectDistance.at(i));
				m_strShowInspectInfo +=tmpdDistanceMM;
				ResultL2LInspectDistance.m_dDistanceMM[i] = dInspectDistance.at(i);
				ResultL2LInspectDistance.m_dDistancePixel[i] = dInspectDistancePixel.at(i);
				...
				//pTemLine->SetTipTextEnabled(FALSE);
				pTemLine->SetTipText(strTmpKey);
				pTemLine->SetLineSegArrowVisible(TRUE);
				m_pGuiSearchLineArray.push_back(pTemLine);
			...
			if (i == 2)
			{
				...
				{
					CString strTmpKey;
					strTmpKey.Format(_T("%s_InspectResultLine%d_%d"), m_strKeyText, 0, 4);
					// 搜索到的圆形
					strTmpKey.Format(_T("%s_ResultCircle%d"), m_strKeyText, 0);
					//scGuiCircle *pTemCircle = new scGuiCircle;
					scGuiCircle *pTemCircle = m_GuiSearchCircle+0;
					...
					CString st;
					st.Format(_T("%d X:%.1f;Y:%.1f;R:%.1f"), 0, ResCircle.GetCenter().GetX(), ResCircle.GetCenter().GetY(), ResCircle.GetRadius());
					pTemCircle->SetLabel(st);
					pTemCircle->SetLabelVisible(TRUE);
					m_pGuiSearchCircleArray.push_back(pTemCircle);

					// 圆心
					strTmpKey.Format(_T("%s_ResultCircleCenter%d"), m_strKeyText, 0);
					scGuiCross *pTempCircleCenter = m_GuiSearchCircleCenter+0;
				...
				else
				{
					...
					//显示线段
					scGuiLineSeg *pTemLine = m_GuiSearchLineSeg +4;
					pTemLine->SetLineSeg(InspectResLine[4]);
					pTemLine->SetLineColor(RGB(255,0,255));
					pTemLine->SetLineWidth(m_tmpSearchGuiParam.m_nLineWidth);
					pTemLine->SetTipTextEnabled(true);
	...
	for (int i=0;i<(INSPCET_TOOL_LINE_NUMS/2)+1;i++)
	{
		...
		m_tmpSearchDataParam.m_dInspectDistanceValue[i] = ResultL2LInspectDistance.m_dDistanceMM[i];
		m_tmpSearchDataParam.m_nInspectOKNG[i] = ResultL2LInspectDistance.m_nResult[i];
		m_tmpSearchDataParam.InspectOKNG_AndResultInfo();

		if(m_tmpSearchDataParam.m_bEnableInspectOKNG[i] && (ResultL2LInspectDistance.m_nResult[i] != 1) && m_TrainDataParam.m_bEnableInspect[i])
		{
			if (i == 2)
			{
				if (m_tmpSearchDataParam.m_nInspectOKNG[i] == 0)
				{
					m_nResultErrNumber = -18;
					return false;
				}
				else if (m_tmpSearchDataParam.m_nInspectOKNG[i] == -1)
				{
					m_nResultErrNumber = -19;
					return false;
				}
			}
			else
			{
				if (m_tmpSearchDataParam.m_nInspectOKNG[i] == 0)
				{
					m_nResultErrNumber = -8-2*i;
					return false;
				}
				else if (m_tmpSearchDataParam.m_nInspectOKNG[i] == -1)
				{
					m_nResultErrNumber = -9-2*i;
					return false;
				}
			}
		}

		if(m_tmpSearchDataParam.m_bEnableInspectOKNG[i] && (ResultL2LInspectDistance.m_nResult[i] != 1) && m_TrainDataParam.m_bEnableInspect[3])
			{
			if (i == 2)
				{
				if (m_tmpSearchDataParam.m_nInspectOKNG[i] == 0)
					{
					m_nResultErrNumber = -17;
					return false;
					}
				else if (m_tmpSearchDataParam.m_nInspectOKNG[i] == -1)
					{
...
// 执行搜索 					
bool CCornerCheckSearchTool::Search(cpImage &ImageSearch)
{
	if (!m_bTrainOK)
	{
	...
	for (int i=0; i<INSPCET_TOOL_LINE_NUMS+1; i++)
	{
		//20230808	wm	判定线距和只检测圆
		if (i == 4)
		{
			if(!m_TrainDataParam.m_bEnableInspect[2] && !m_TrainDataParam.m_bEnableInspect[3]) continue;
		}

		//if(!m_TrainDataParam.m_bEnableInspect[(int)(i/2)]) continue;

		//m_InspectFindLineTool[i].SetCaliperNum(m_TrainGuiParam.m_nCaliperNum1[i]);
		m_InspectFindLineTool[i].SetCaliperParams(m_TrainGuiParam.m_nCaliperNum1[i], m_TrainGuiParam.m_CaliperSearchRotation1[i], m_TrainGuiParam.m_dCaliperSearchLen1[i],m_TrainGuiParam.m_dCaliperProjectLen1[i]);
		//m_InspectFindLineTool[i].SetCaliperProperty(m_TrainDataParam.m_dContrastThre1[i], m_TrainDataParam.m_edgeProperty1[i], m_TrainDataParam.m_lFilterHalfWidth1[i]);
		if (m_tmpTrainDataParam.m_bEnableDualEdge1[i])
		{
			EdgePolarity tmpEdgePolarity = eDontCare;
			if (m_tmpTrainDataParam.m_edgeProperty1[i] == eDarkToLight)
		...
		m_InspectFindLineTool[i].SetOutLineNum(m_TrainDataParam.m_nOutLineNum1[i], eRansacRobust);
		m_InspectFindLineTool[i].EnableCaliperSortByScore(m_TrainDataParam.m_bSortByScore1[i]);

		m_InspectFindLineTool[i].SetCaliperMasked(m_TrainGuiParam.m_vbCaliperMask_FindLine1[i]);
		if (i == 4)
		{
			// Circle
			...
			m_FindCircleTool.EnableCaliperSortByScore(m_TrainDataParam.m_CircleedgeProperty);
			//掩膜与双极性
			m_FindCircleTool.SetCaliperMasked(m_tmpTrainGuiParam.m_vbCircleCaliperMask);
			if (m_tmpTrainDataParam.m_bEnableCircleDualEdge)
			{
	...
	if (!m_tmpSearchDataParam.m_bCheckAfterSearch)
	{
		// 先防呆检查 再定位
		if (m_tmpTrainDataParam.m_bEnableCheck && m_tmpTrainDataParam.GetInspectToolType() == ePatternSearchTool)
		{
		...
		if (m_tmpTrainDataParam.m_bEnableCheck && m_TrainDataParam.GetInspectToolType() == eQuickSearchTool)
		{
			if(bIsProcessImage)
			{
				...
				for (int i=0; i<nResultNum && i<MAXRESNUM; i++)
				{
					...
					if(m_tmpTrainDataParam.m_bEnableCorner)
					{
						...
						if (bFindLine[0] && bFindLine[1])
						{
							...
							// 执行线距检测搜索
							CResultL2LInspectDistance tmpL2LReslut;
							bool bInspectSearch = false;
							if(bIsProcessImage)
							{
								bInspectSearch = ExecuteInspectSearch(dTransX,dTransY,dRotation,tmpImage,tmpL2LReslut);
							}
							else
							{
								bInspectSearch = ExecuteInspectSearch(dTransX,dTransY,dRotation,ImageSearch,tmpL2LReslut);
							}
							if(bInspectSearch)
							{
								searchResult.m_ResultL2LInspectDistance = tmpL2LReslut;
							}
							else
							{
								searchResult.m_ResultL2LInspectDistance = tmpL2LReslut;
								for (int i =0;i<(INSPCET_TOOL_LINE_NUMS/2)+1;i++)
								{
									if(m_tmpSearchDataParam.m_bEnableInspectOKNG[i])
									{
										bRet= false;
									}
								}
						   ...
						   SetSearchResult(tmpL2LReslut, searchResult);
	...
	else
	{
		if (m_tmpTrainDataParam.GetSubSearchToolType() == ePatternCornerSearchTool)
		{
			...
			if(bIsProcessImage)
			{
				bRet = ExecutePatternSerach(tmpImage,&m_PatternSearchTool,&(m_TrainGuiParam.m_SearchRect),m_tmpSearchDataParam.m_nSearchNum,
					m_tmpSearchDataParam.m_dAcceptThre,m_tmpSearchDataParam.m_dConfusionThre,0,0,0);
			}
			else
			{
				bRet = ExecutePatternSerach(ImageSearch,&m_PatternSearchTool,&(m_TrainGuiParam.m_SearchRect),m_tmpSearchDataParam.m_nSearchNum,
					m_tmpSearchDataParam.m_dAcceptThre,m_tmpSearchDataParam.m_dConfusionThre,0,0,0);
			}
			if (bRet)
			{
				...
				for (int i=0; i<nResultNum && i<MAXRESNUM; i++)
				{
					...
					if (bFindLine[0] && bFindLine[1])
					{
						...
						//	CSearchResult searchResult;
							...
							// 执行防呆检测
							//区域工具防呆 根据角定位结果进行搜索区调整
							if (m_tmpTrainDataParam.m_bEnableCheck && m_TrainDataParam.GetInspectToolType() == ePatternSearchTool)
							{
								...
								if(bIsProcessImage)
								{
									bRetIns = ExecutePatternSerach(tmpImage,&m_PatternSearchTool1,&(m_TrainGuiParam.m_SearchRect1),m_tmpSearchDataParam.m_nSearchNum,m_tmpSearchDataParam.m_dAcceptThre,
										m_tmpSearchDataParam.m_dConfusionThre,dTransX,dTransY,dRotation);
								}
								else
								{
									bRetIns = ExecutePatternSerach(ImageSearch,&m_PatternSearchTool1,&(m_TrainGuiParam.m_SearchRect1),m_tmpSearchDataParam.m_nSearchNum,m_tmpSearchDataParam.m_dAcceptThre,
										m_tmpSearchDataParam.m_dConfusionThre,dTransX,dTransY,dRotation);
								}
								...
								{
									// 获取结果
									int nInsResultNum = m_PatternSearchTool1.GetResultNum();
									if (nInsResultNum <= 0 || nInsResultNum != m_tmpSearchDataParam.m_nSearchNum1)
									{
										m_nResultErrNumber=-1;    //检查区域定位失败
										return false;
									}
									for (int iIns=0; iIns<nInsResultNum && iIns<MAXRESNUM; iIns++)
									{
										scPatternSearchRotResult tmpPatternResult = m_PatternSearchTool1.GetResult(iIns);
										...
										//显示矩形
										ShowResultRect(m_pGuiSearchRectArray,m_GuiSearchAffineRect1,iIns,&(tmpPatternResult.MatchRegion()),_T("_InspectResultRect_"),m_tmpSearchGuiParam.m_LineColor,m_tmpSearchGuiParam.m_nLineWidth);

										//显示参考点
										ShowResultCross(m_pGuiSearchCrossArray,m_GuiSearchCross3,iIns,tmpPatternResult.GetPosition(),_T("_InspectResultCross_"),m_tmpSearchGuiParam.m_LineColor,m_tmpSearchGuiParam.m_nLineWidth);
							...
							if (m_tmpTrainDataParam.m_bEnableCheck && m_TrainDataParam.GetInspectToolType() == eQuickSearchTool)
							{
								...
								if(bIsProcessImage)
								{
									bRetIns = ExecuteQuickSearch(tmpImage,&m_QuickSearchTool1,&(m_TrainGuiParam.m_SearchRect1),m_tmpSearchDataParam.m_nSearchNum1, \
										m_tmpSearchDataParam.m_dAcceptThre1,m_tmpSearchDataParam.m_dAngleLow1,m_tmpSearchDataParam.m_dAngleHigh1,m_tmpSearchDataParam.m_dSizeLow1, \
										m_tmpSearchDataParam.m_dSizeHigh1,dTransX,dTransY,dRotation);
								}
								else
								{
									bRetIns = ExecuteQuickSearch(ImageSearch,&m_QuickSearchTool1,&(m_TrainGuiParam.m_SearchRect1),m_tmpSearchDataParam.m_nSearchNum1, \
										m_tmpSearchDataParam.m_dAcceptThre1,m_tmpSearchDataParam.m_dAngleLow1,m_tmpSearchDataParam.m_dAngleHigh1,m_tmpSearchDataParam.m_dSizeLow1, \
										m_tmpSearchDataParam.m_dSizeHigh1,dTransX,dTransY,dRotation);
								}
								if (bRetIns)
								{
									// 获取结果
									int nInsResultNum = m_QuickSearchTool1.GetResultNum();
									if (nInsResultNum <= 0 || nInsResultNum != m_tmpSearchDataParam.m_nSearchNum1)
									{
										m_nResultErrNumber=-2;
										return false;
									...
									for (int iIns=0; iIns<nInsResultNum && iIns<MAXRESNUM; iIns++)
									{
										scQuickSearchResult tmpQuickResult = m_QuickSearchTool1.GetResult(iIns);
										CResultSearchRect resRect;
										resRect.m_searchRect = tmpQuickResult.MatchRegion();
										searchResult.m_vACFSearchRect.push_back(resRect);
										//显示矩形
										ShowResultRect(m_pGuiSearchRectArray,m_GuiSearchAffineRect1,iIns,&(tmpQuickResult.MatchRegion()),_T("_InspectResultRect_"),m_tmpSearchGuiParam.m_LineColor,m_tmpSearchGuiParam.m_nLineWidth);

										//显示参考点
										ShowResultCross(m_pGuiSearchCrossArray,m_GuiSearchCross3,iIns,tmpQuickResult.GetPosition(),_T("_InspectResultCross_"),m_tmpSearchGuiParam.m_LineColor,m_tmpSearchGuiParam.m_nLineWidth);
...
// 获取搜索结果个数										
int CCornerCheckSearchTool::GetResultNum()
{
	return m_SearchResultArray.size();
}

// 获取搜索结果														
bool CCornerCheckSearchTool::GetResult(int nIndex, CSearchResult &searchResult)
{
...
// 加载模板训练图像
bool CCornerCheckSearchTool::LoadModelTrainPatternImage(CString strDir ,cpImage &ImagePattern)
{
	// strDir为文件夹路径
	// 仅配置ModelImage.bmp
	if (strDir == _T(""))
	...
	if (cSearchToolType != eCornerSearchCheckInspectTool)
	{
		return false;
	}
...
// 加载模板文件
bool CCornerCheckSearchTool::LoadModelFromFile(CString strDir)
{
	// strDir为文件夹路径
	// 配置文件包括以下部分:
	//    (1) 配置文件Model.xml
	...
	//	BYTE byR, byG, byB;
	CCornerCheckSearchTrainDataParam tmpTrainDataParam;
	CCornerCheckSearchTrainGuiParam tmpTrainGuiParam;
	CCornerCheckSearchSearchDataParam tmpSearchDataParam;
	CCornerCheckSearchSearchGuiParam tmpSearchGuiParam;	
	CCornerCheckSearchTrainResult    tmpTrainResult;
	...
	bRet = cXMLConfigurator.FindElem(_T("InspectTrainParam1"));
	if(bRet)
	{
		cXMLConfigurator.EnterElem();

																																																																																		{
		bRet = cXMLConfigurator.FindElem(_T("EnableInspect1"));
		if(!bRet)
		{
			tmpTrainDataParam.m_bEnableInspect[0] = false;
		}
		else
		{
			tmpTrainDataParam.m_bEnableInspect[0] = _ttoi(cXMLConfigurator.GetElemData());
		}
		...
		if (bRet)
		{
			tmpTrainDataParam.m_dContrastThre1[0] = _tcstod(cXMLConfigurator.GetElemData(), NULL);
		}
		...
		if (bRet)
		{
			tmpTrainDataParam.m_lFilterHalfWidth1[0] = _ttol(cXMLConfigurator.GetElemData());
		}
		...
		if (bRet)
		{
			tmpTrainDataParam.m_nOutLineNum1[0] = _ttol(cXMLConfigurator.GetElemData());
		}
		...
		if (bRet)
		{
			tmpTrainDataParam.m_edgeProperty1[0] = (EdgePolarity)_ttol(cXMLConfigurator.GetElemData());
		}
		...
		if (bRet)
		{
			tmpTrainDataParam.m_bSortByScore1[0] = _ttoi(cXMLConfigurator.GetElemData());
		}
		...
		if (!bRet)
		{
			tmpTrainDataParam.m_bEnableDualEdge1[0] = false;
		}
		tmpTrainDataParam.m_bEnableDualEdge1[0] = _ttoi(cXMLConfigurator.GetElemData());

		// contrastthre2
		bRet = cXMLConfigurator.FindElem(_T("ContrastThre2"));
		if (bRet)
		{
			tmpTrainDataParam.m_dContrastThre1[1] = _tcstod(cXMLConfigurator.GetElemData(), NULL);
		}
		...
		if (bRet)
		{
			tmpTrainDataParam.m_lFilterHalfWidth1[1] = _ttol(cXMLConfigurator.GetElemData());
		}
		...
		if (bRet)
		{
			tmpTrainDataParam.m_nOutLineNum1[1] = _ttol(cXMLConfigurator.GetElemData());
		}
		...
		if (bRet)
		{
			tmpTrainDataParam.m_edgeProperty1[1] = (EdgePolarity)_ttol(cXMLConfigurator.GetElemData());
		}
		...
		if (bRet)
		{
			tmpTrainDataParam.m_bSortByScore1[1] = _ttoi(cXMLConfigurator.GetElemData());
		}
		...
		if (!bRet)
		{
			tmpTrainDataParam.m_bEnableDualEdge1[1] = false;
		}
		tmpTrainDataParam.m_bEnableDualEdge1[1] = _ttoi(cXMLConfigurator.GetElemData());
		
	}

		cXMLConfigurator.LeaveElem();
	}
	bRet = cXMLConfigurator.FindElem(_T("InspectTrainParam2"));
	if(bRet)
	{
		...
		if(!bRet)
		{
			tmpTrainDataParam.m_bEnableInspect[1] = false;
		}
		else
		{
			tmpTrainDataParam.m_bEnableInspect[1] = _ttoi(cXMLConfigurator.GetElemData());
		}
		...
		if (bRet)
		{
			tmpTrainDataParam.m_dContrastThre1[2] = _tcstod(cXMLConfigurator.GetElemData(), NULL);
		}
		...
		if (bRet)
		{
			tmpTrainDataParam.m_lFilterHalfWidth1[2] = _ttol(cXMLConfigurator.GetElemData());
		}
		...
		if (bRet)
		{
			tmpTrainDataParam.m_nOutLineNum1[2] = _ttol(cXMLConfigurator.GetElemData());
		}
		...
		if (bRet)
		{
			tmpTrainDataParam.m_edgeProperty1[2] = (EdgePolarity)_ttol(cXMLConfigurator.GetElemData());
		}
		...
		if (bRet)
		{
			tmpTrainDataParam.m_bSortByScore1[2] = _ttoi(cXMLConfigurator.GetElemData());
		}
		...
		if (!bRet)
		{
			tmpTrainDataParam.m_bEnableDualEdge1[2] = false;
		}
		tmpTrainDataParam.m_bEnableDualEdge1[2] = _ttoi(cXMLConfigurator.GetElemData());

		// contrastthre2
		bRet = cXMLConfigurator.FindElem(_T("ContrastThre2"));
		if (bRet)
		{
			tmpTrainDataParam.m_dContrastThre1[3] = _tcstod(cXMLConfigurator.GetElemData(), NULL);
		}
		...
		if (bRet)
		{
			tmpTrainDataParam.m_lFilterHalfWidth1[3] = _ttol(cXMLConfigurator.GetElemData());
		}
		...
		if (bRet)
		{
			tmpTrainDataParam.m_nOutLineNum1[3] = _ttol(cXMLConfigurator.GetElemData());
		}
		...
		if (bRet)
		{
			tmpTrainDataParam.m_edgeProperty1[3] = (EdgePolarity)_ttol(cXMLConfigurator.GetElemData());
		}
		...
		if (bRet)
		{
			tmpTrainDataParam.m_bSortByScore1[3] = _ttoi(cXMLConfigurator.GetElemData());
		}
		// DualEdge2
		bRet = cXMLConfigurator.FindElem(_T("DualEdge2"));
		if (!bRet)
		{
			tmpTrainDataParam.m_bEnableDualEdge1[3] = false;
		}
		tmpTrainDataParam.m_bEnableDualEdge1[3] = _ttoi(cXMLConfigurator.GetElemData());
	}
	...
	bRet = cXMLConfigurator.FindElem(_T("InspectTrainParam3"));
	if(bRet)
	{
...

		{
			...
			if(!bRet)
			{
				tmpTrainDataParam.m_bEnableInspect[2] = false;
			}
			else
			{
				tmpTrainDataParam.m_bEnableInspect[2] = _ttoi(cXMLConfigurator.GetElemData());
			}

			bRet = cXMLConfigurator.FindElem(_T("EnableInspect2"));
			if(!bRet)
			{
				tmpTrainDataParam.m_bEnableInspect[3] = false;
			}
			else
			{
				tmpTrainDataParam.m_bEnableInspect[3] = _ttoi(cXMLConfigurator.GetElemData());
			}

			bRet = cXMLConfigurator.FindElem(_T("ContrastThre1"));
			if (bRet)
			{
				tmpTrainDataParam.m_dContrastThre1[4] = _tcstod(cXMLConfigurator.GetElemData(), NULL);
			}
			...
			if (bRet)
			{
				tmpTrainDataParam.m_lFilterHalfWidth1[4] = _ttol(cXMLConfigurator.GetElemData());
			}
			...
			if (bRet)
			{
				tmpTrainDataParam.m_nOutLineNum1[4] = _ttol(cXMLConfigurator.GetElemData());
			}
			...
			if (bRet)
			{
				tmpTrainDataParam.m_edgeProperty1[4] = (EdgePolarity)_ttol(cXMLConfigurator.GetElemData());
			}
			...
			if (bRet)
			{
				tmpTrainDataParam.m_bSortByScore1[4] = _ttoi(cXMLConfigurator.GetElemData());
			}
			...
			if (!bRet)
			{
				tmpTrainDataParam.m_bEnableDualEdge1[4] = false;
			}
			tmpTrainDataParam.m_bEnableDualEdge1[4] = _ttoi(cXMLConfigurator.GetElemData());
	...
	bRet = cXMLConfigurator.FindElem(_T("InspectTrainCircle"));
	if(bRet)
	{
		cXMLConfigurator.EnterElem();
		{
			...
			if (!bRet)
			{
				tmpTrainDataParam.m_bSortByScore2 = false;
			}
			tmpTrainDataParam.m_bSortByScore2 = _ttoi(cXMLConfigurator.GetElemData());
			///CircleOutLineNum
			bRet = cXMLConfigurator.FindElem(_T("CircleOutLineNum"));
			if (bRet)
			{
				tmpTrainDataParam.m_nCircleOutLineNum = _ttoi(cXMLConfigurator.GetElemData());
			}
			///CircleContrastThre
			...
			if (bRet)
			{
				tmpTrainDataParam.m_CircleedgeProperty = (EdgePolarity)_ttoi(cXMLConfigurator.GetElemData());
			}
			///EnableCircleDualEdge
			bRet = cXMLConfigurator.FindElem(_T("EnableCircleDualEdge"));
			if (!bRet)
			{
	...
	//线距检测工具找线
	bRet = cXMLConfigurator.FindElem(_T("InspectTrainGui_FindLine"));
	if (bRet)
	{
		cXMLConfigurator.EnterElem(); 

		{
			...
			// CircleCaliperSearchDirection
			bRet = cXMLConfigurator.FindElem(_T("CircleCaliperSearchDirection"));
			if (bRet)
			{
				tmpTrainGuiParam.m_CircleCaliperSearchDirection = (CaliperSearchDirection)_ttoi(cXMLConfigurator.GetElemData());
			}
			// CircleCaliperVisible
			...
			// EnableCircleMask
			bRet = cXMLConfigurator.FindElem(_T("EnableCircleMask"));
			if (bRet)
			{
				tmpTrainGuiParam.m_bEnableCircleMask = _ttoi(cXMLConfigurator.GetElemData());
			}
			
		}
		//CenterX
		bRet = cXMLConfigurator.FindElem(_T("CircleResultCenterX"));
		if (bRet)
		{
			vPos.SetX(_tcstod(cXMLConfigurator.GetElemData(), NULL));
		}
		...
		bRet = cXMLConfigurator.FindElem(_T("CircleResultCenterY"));
		if (bRet)
		{
			vPos.SetY(_tcstod(cXMLConfigurator.GetElemData(), NULL));
		}
		...
		//Radius
		bRet = cXMLConfigurator.FindElem(_T("CircleResultRadius"));
		if (bRet)
		{
		...
		for (int i=0;i<(INSPCET_TOOL_LINE_NUMS/2)+1;i++)
		{
			...
			if (bRet)
			{
				cXMLConfigurator.EnterElem();
				{
					...
					if (bRet)
					{
						tmpTrainGuiParam.m_nCaliperNum1[i] = _ttoi(cXMLConfigurator.GetElemData());
					}
					...
					if (bRet)
					{
						tmpTrainGuiParam.m_dStartX1[i] = _tcstod(cXMLConfigurator.GetElemData(), NULL);
					}
					...
					if (bRet)
					{
						tmpTrainGuiParam.m_dStartY1[i] = _tcstod(cXMLConfigurator.GetElemData(), NULL);
					}
					...
					if (bRet)
					{
						tmpTrainGuiParam.m_dEndX1[i] = _tcstod(cXMLConfigurator.GetElemData(), NULL);
					}
					...
					if (bRet)
					{
						tmpTrainGuiParam.m_dEndY1[i] = _tcstod(cXMLConfigurator.GetElemData(), NULL);
					}
					...
					if (bRet)
					{
						tmpTrainGuiParam.m_dCaliperProjectLen1[i] = _tcstod(cXMLConfigurator.GetElemData(), NULL);
					}
					...
					if (bRet)
					{
						tmpTrainGuiParam.m_dCaliperSearchLen1[i] = _tcstod(cXMLConfigurator.GetElemData(), NULL);
					}
					...
					if (bRet)
					{
						tmpTrainGuiParam.m_CaliperSearchRotation1[i] = scRadian(_tcstod(cXMLConfigurator.GetElemData(), NULL));
					}

					// Caliper Visible
					bRet = cXMLConfigurator.FindElem(_T("InspectCaliperVisible1"));
					if (!bRet)
					{
						tmpTrainGuiParam.m_bInspectCaliperVisible[i]=true;
					}
					else
					{
						tmpTrainGuiParam.m_bInspectCaliperVisible[i] = _ttoi(cXMLConfigurator.GetElemData());
					}
					...
					if (bRet)
					{
						tmpTrainGuiParam.m_nCaliperNum1[i+2] = _ttoi(cXMLConfigurator.GetElemData());
					}
					...
					if (bRet)
					{
						tmpTrainGuiParam.m_dStartX1[i+2] = _tcstod(cXMLConfigurator.GetElemData(), NULL);
					}
					...
					if (bRet)
					{
						tmpTrainGuiParam.m_dStartY1[i+2] = _tcstod(cXMLConfigurator.GetElemData(), NULL);
					}
					...
					if (bRet)
					{
						tmpTrainGuiParam.m_dEndX1[i+2] = _tcstod(cXMLConfigurator.GetElemData(), NULL);
					}
					...
					if (bRet)
					{
						tmpTrainGuiParam.m_dEndY1[i+2] = _tcstod(cXMLConfigurator.GetElemData(), NULL);
					}
					...
					if (bRet)
					{
						tmpTrainGuiParam.m_dCaliperProjectLen1[i+2] = _tcstod(cXMLConfigurator.GetElemData(), NULL);
					}
					...
					if (bRet)
					{
						tmpTrainGuiParam.m_dCaliperSearchLen1[i+2] = _tcstod(cXMLConfigurator.GetElemData(), NULL);
					}
					...
					if (bRet)
					{
						tmpTrainGuiParam.m_CaliperSearchRotation1[i+2] = scRadian(_tcstod(cXMLConfigurator.GetElemData(), NULL));
					}

					// Caliper Visible2
					bRet = cXMLConfigurator.FindElem(_T("InspectCaliperVisible2"));
					if (!bRet)
					{
						tmpTrainGuiParam.m_bInspectCaliperVisible[i+2]=true;
					}
					else
					{
						tmpTrainGuiParam.m_bInspectCaliperVisible[i+2] = _ttoi(cXMLConfigurator.GetElemData());
					}
	...
	bRet = cXMLConfigurator.FindElem(_T("CheckAfterSearch"));
	if (bRet)
	{
		tmpSearchDataParam.m_bCheckAfterSearch = _ttoi(cXMLConfigurator.GetElemData());
	}
	...
	bRet = cXMLConfigurator.FindElem(_T("InspectSearchDataParam"));
	if (bRet)
	{
		cXMLConfigurator.EnterElem();
		bRet = cXMLConfigurator.FindElem(_T("EnableShowFitCirclePoint"));
		if (bRet)
		{
			tmpSearchGuiParam.m_bEnableShowFitCirclePoint = _ttoi(cXMLConfigurator.GetElemData());
		}

		bRet = cXMLConfigurator.FindElem(_T("InspectVisibleFitPoint"));
		if (bRet)
		{
			tmpSearchGuiParam.m_bInspectVisibleFitPoint[4] = _ttoi(cXMLConfigurator.GetElemData());
		}	
		...
		for (int i=0;i<(INSPCET_TOOL_LINE_NUMS/2)+1;i++)
		{
			...
			if(bRet)
			{
				...
				if (bRet)
				{
					tmpSearchDataParam.m_dPixelRatio[i] = _tcstod(cXMLConfigurator.GetElemData(), NULL);
				}

				bRet = cXMLConfigurator.FindElem(_T("DistanceMode"));
				if (bRet)
				{
					tmpSearchDataParam.m_nDistanceMode[i] = _ttoi(cXMLConfigurator.GetElemData());
				}
				
				bRet = cXMLConfigurator.FindElem(_T("DistanceType"));
				if (bRet)
				{
					tmpSearchDataParam.m_nDistanceType[i] = _ttoi(cXMLConfigurator.GetElemData());
				}
				...
				bRet = cXMLConfigurator.FindElem(_T("InspectVisibleFitPoint1"));
				if (bRet)
				{
					tmpSearchGuiParam.m_bInspectVisibleFitPoint[i] = _ttoi(cXMLConfigurator.GetElemData());
				}

				if (i < INSPCET_TOOL_LINE_NUMS/2)
				{
					if (bRet)
					{
						bRet = cXMLConfigurator.FindElem(_T("InspectVisibleFitPoint2"));
						tmpSearchGuiParam.m_bInspectVisibleFitPoint[i+1] = _ttoi(cXMLConfigurator.GetElemData());
					}
				}
	...
	bRet = cXMLConfigurator.FindElem(_T("InspectTrainResult"));
	if (bRet)
	{
		cXMLConfigurator.EnterElem();
		for (int i=0;i<INSPCET_TOOL_LINE_NUMS+1;i++)
		{
			...
			if (bRet)
			{
				...
				tmpTrainResult.m_dStartX1[i] = vPos.GetX();
				tmpTrainResult.m_dStartY1[i] = vPos.GetY();
				...
				tmpTrainResult.m_dEndX1[i] = vPos.GetX();
				tmpTrainResult.m_dEndY1[i] = vPos.GetY();
	...
	if (tmpTrainDataParam.m_bMaskEnable1  && tmpTrainDataParam.m_bEnableCheck)
	{
...
// 保存模板文件							
bool CCornerCheckSearchTool::SaveModelToFile(CString strDir)
{
	...
	// inspect enable
	cXMLConfigurator.AddElem(_T("InspectTrainParam1"));
	cXMLConfigurator.EnterElem(); //进入下一级目录

	strInfo.Format(_T("%d"), m_TrainDataParam.m_bEnableInspect[0]);
	cXMLConfigurator.AddElem(_T("EnableInspect1"), strInfo);
	// contrastthre1
	strInfo.Format(_T("%f"), m_TrainDataParam.m_dContrastThre1[0]);
	cXMLConfigurator.AddElem(_T("ContrastThre1"), strInfo);
	// fileterhalfwidth1
	strInfo.Format(_T("%d"), m_TrainDataParam.m_lFilterHalfWidth1[0]);
	cXMLConfigurator.AddElem(_T("FilterHalfWidth1"), strInfo);
	// outlinenum1
	strInfo.Format(_T("%d"), m_TrainDataParam.m_nOutLineNum1[0]);
	cXMLConfigurator.AddElem(_T("OutLineNum1"), strInfo);
	// edgeproperty1
	strInfo.Format(_T("%d"), m_TrainDataParam.m_edgeProperty1[0]);
	cXMLConfigurator.AddElem(_T("EdgeProperty1"), strInfo);
	// SortByScore1
	strInfo.Format(_T("%d"), m_TrainDataParam.m_bSortByScore1[0]);
	cXMLConfigurator.AddElem(_T("SortByScore1"), strInfo);

	// DualEdge1
	strInfo.Format(_T("%d"), m_TrainDataParam.m_bEnableDualEdge1[0]);
	cXMLConfigurator.AddElem(_T("DualEdge1"), strInfo);

	// contrastthre2
	strInfo.Format(_T("%f"), m_TrainDataParam.m_dContrastThre1[1]);
	cXMLConfigurator.AddElem(_T("ContrastThre2"), strInfo);
	// fileterhalfwidth2
	strInfo.Format(_T("%d"), m_TrainDataParam.m_lFilterHalfWidth1[1]);
	cXMLConfigurator.AddElem(_T("FilterHalfWidth2"), strInfo);
	// outlinenum2
	strInfo.Format(_T("%d"), m_TrainDataParam.m_nOutLineNum1[1]);
	cXMLConfigurator.AddElem(_T("OutLineNum2"), strInfo);
	// edgeproperty2
	strInfo.Format(_T("%d"), m_TrainDataParam.m_edgeProperty1[1]);
	cXMLConfigurator.AddElem(_T("EdgeProperty2"), strInfo);
	// SortByScore2
	strInfo.Format(_T("%d"), m_TrainDataParam.m_bSortByScore1[1]);
	cXMLConfigurator.AddElem(_T("SortByScore2"), strInfo);
	// DualEdge2
	strInfo.Format(_T("%d"), m_TrainDataParam.m_bEnableDualEdge1[1]);
	cXMLConfigurator.AddElem(_T("DualEdge2"), strInfo);

	cXMLConfigurator.LeaveElem();


	cXMLConfigurator.AddElem(_T("InspectTrainParam2"));
	cXMLConfigurator.EnterElem(); //进入下一级目录

	strInfo.Format(_T("%d"), m_TrainDataParam.m_bEnableInspect[1]);
	cXMLConfigurator.AddElem(_T("EnableInspect1"), strInfo);
	// contrastthre1
	strInfo.Format(_T("%f"), m_TrainDataParam.m_dContrastThre1[2]);
	cXMLConfigurator.AddElem(_T("ContrastThre1"), strInfo);
	// fileterhalfwidth1
	strInfo.Format(_T("%d"), m_TrainDataParam.m_lFilterHalfWidth1[2]);
	cXMLConfigurator.AddElem(_T("FilterHalfWidth1"), strInfo);
	// outlinenum1
	strInfo.Format(_T("%d"), m_TrainDataParam.m_nOutLineNum1[2]);
	cXMLConfigurator.AddElem(_T("OutLineNum1"), strInfo);
	// edgeproperty1
	strInfo.Format(_T("%d"), m_TrainDataParam.m_edgeProperty1[2]);
	cXMLConfigurator.AddElem(_T("EdgeProperty1"), strInfo);
	// SortByScore1
	strInfo.Format(_T("%d"), m_TrainDataParam.m_bSortByScore1[2]);
	cXMLConfigurator.AddElem(_T("SortByScore1"), strInfo);
	// DualEdge1
	strInfo.Format(_T("%d"), m_TrainDataParam.m_bEnableDualEdge1[2]);
	cXMLConfigurator.AddElem(_T("DualEdge1"), strInfo);

	// contrastthre2
	strInfo.Format(_T("%f"), m_TrainDataParam.m_dContrastThre1[3]);
	cXMLConfigurator.AddElem(_T("ContrastThre2"), strInfo);
	// fileterhalfwidth2
	strInfo.Format(_T("%d"), m_TrainDataParam.m_lFilterHalfWidth1[3]);
	cXMLConfigurator.AddElem(_T("FilterHalfWidth2"), strInfo);
	// outlinenum2
	strInfo.Format(_T("%d"), m_TrainDataParam.m_nOutLineNum1[3]);
	cXMLConfigurator.AddElem(_T("OutLineNum2"), strInfo);
	// edgeproperty2
	strInfo.Format(_T("%d"), m_TrainDataParam.m_edgeProperty1[3]);
	cXMLConfigurator.AddElem(_T("EdgeProperty2"), strInfo);
	// SortByScore2
	strInfo.Format(_T("%d"), m_TrainDataParam.m_bSortByScore1[3]);
	cXMLConfigurator.AddElem(_T("SortByScore2"), strInfo);
	// DualEdge2
	strInfo.Format(_T("%d"), m_TrainDataParam.m_bEnableDualEdge1[3]);
	cXMLConfigurator.AddElem(_T("DualEdge2"), strInfo);
	cXMLConfigurator.LeaveElem();

	//////////////////////////////////////////////////////////////////////////
	cXMLConfigurator.AddElem(_T("InspectTrainParam3"));
	cXMLConfigurator.EnterElem(); //进入下一级目录

	strInfo.Format(_T("%d"), m_TrainDataParam.m_bEnableInspect[2]);
	cXMLConfigurator.AddElem(_T("EnableInspect1"), strInfo);

	strInfo.Format(_T("%d"), m_TrainDataParam.m_bEnableInspect[3]);
	cXMLConfigurator.AddElem(_T("EnableInspect2"), strInfo);
	// contrastthre1
	strInfo.Format(_T("%f"), m_TrainDataParam.m_dContrastThre1[4]);
	cXMLConfigurator.AddElem(_T("ContrastThre1"), strInfo);
	// fileterhalfwidth1
	strInfo.Format(_T("%d"), m_TrainDataParam.m_lFilterHalfWidth1[4]);
	cXMLConfigurator.AddElem(_T("FilterHalfWidth1"), strInfo);
	// outlinenum1
	strInfo.Format(_T("%d"), m_TrainDataParam.m_nOutLineNum1[4]);
	cXMLConfigurator.AddElem(_T("OutLineNum1"), strInfo);
	// edgeproperty1
	strInfo.Format(_T("%d"), m_TrainDataParam.m_edgeProperty1[4]);
	cXMLConfigurator.AddElem(_T("EdgeProperty1"), strInfo);
	// SortByScore1
	strInfo.Format(_T("%d"), m_TrainDataParam.m_bSortByScore1[4]);
	cXMLConfigurator.AddElem(_T("SortByScore1"), strInfo);
	// DualEdge1
	strInfo.Format(_T("%d"), m_TrainDataParam.m_bEnableDualEdge1[4]);
	cXMLConfigurator.AddElem(_T("DualEdge1"), strInfo);
	cXMLConfigurator.LeaveElem();

	cXMLConfigurator.AddElem(_T("InspectTrainCircle"));
	cXMLConfigurator.EnterElem(); //进入下一级目录
	// SortByScore2
	strInfo.Format(_T("%d"), m_TrainDataParam.m_bSortByScore2);
	cXMLConfigurator.AddElem(_T("SortByScore2"), strInfo);
	// CircleOutLineNum
	...
	// CircleedgeProperty
	strInfo.Format(_T("%d"), (int)m_TrainDataParam.m_CircleedgeProperty);
	cXMLConfigurator.AddElem(_T("CircleedgeProperty"), strInfo);
	// EnableCircleDualEdge
	strInfo.Format(_T("%d"), m_TrainDataParam.m_bEnableCircleDualEdge);
	cXMLConfigurator.AddElem(_T("EnableCircleDualEdge"), strInfo);
	cXMLConfigurator.LeaveElem();
	...
	// inspect tool train gui LineMask
	cXMLConfigurator.AddElem(_T("InspectTrainGuiParam"));
	cXMLConfigurator.EnterElem(); //进入下一级目录
	for (int i=0;i<INSPCET_TOOL_LINE_NUMS+1;i++)
	{
		strInfo.Format(_T("%d"), m_TrainGuiParam.m_bEnableLineMask[i]);
		CString tmpStr = _T("");
		tmpStr.Format(_T("LineMaskEnable%d"),i);
		cXMLConfigurator.AddElem(tmpStr, strInfo);
	}
	...
	for (int i=0;i<INSPCET_TOOL_LINE_NUMS+1;i++)
	{
		...
		cXMLConfigurator.AddElem(strTmp);
		cXMLConfigurator.EnterElem();
		strInfo.Format(_T("%d"), m_TrainGuiParam.m_vdMaskData_FindLine1[i].size());
		cXMLConfigurator.AddElem(_T("MaskDataNum"), strInfo);

		for (int j=0; j<m_TrainGuiParam.m_vdMaskData_FindLine1[i].size(); j++)
		{
			strTmp.Format(_T("MaskData%d"), j);
			strInfo.Format(_T("%f"), m_TrainGuiParam.m_vdMaskData_FindLine1[i].at(j));
			cXMLConfigurator.AddElem(strTmp, strInfo);
		}
		strInfo.Format(_T("%d"), m_TrainGuiParam.m_vbCaliperMask_FindLine1[i].size());
		cXMLConfigurator.AddElem(_T("CaliperMaskDataNum"), strInfo);
		for (int j=0; j<m_TrainGuiParam.m_vbCaliperMask_FindLine1[i].size(); j++)
		{
			strTmp.Format(_T("CaliperMaskData%d"), j);
			strInfo.Format(_T("%d"), (int)m_TrainGuiParam.m_vbCaliperMask_FindLine1[i].at(j));
			cXMLConfigurator.AddElem(strTmp, strInfo);
		}
		cXMLConfigurator.LeaveElem();
	}
	...
	//线距检测工具找线与找圆
	cXMLConfigurator.AddElem(_T("InspectTrainGui_FindLine"));
	cXMLConfigurator.EnterElem(); //进入下一级目录
	...
	//CircleCaliperSearchDirection
	strInfo.Format(_T("%d"), (int)m_TrainGuiParam.m_CircleCaliperSearchDirection);
	cXMLConfigurator.AddElem(_T("CircleCaliperSearchDirection"), strInfo);
	//CircleCaliperVisible
	strInfo.Format(_T("%d"), m_TrainGuiParam.m_bCircleCaliperVisible);
	cXMLConfigurator.AddElem(_T("CircleCaliperVisible"), strInfo);
	//EnableCircleMask
	strInfo.Format(_T("%d"), m_TrainGuiParam.m_bEnableCircleMask);
	cXMLConfigurator.AddElem(_T("EnableCircleMask"), strInfo);

	//CenterX
	strInfo.Format(_T("%f"), m_TrainResult.m_dCircleCenterX);
	cXMLConfigurator.AddElem(_T("CircleResultCenterX"), strInfo);
	//CenterY
	strInfo.Format(_T("%f"), m_TrainResult.m_dCircleCenterY);
	cXMLConfigurator.AddElem(_T("CircleResultCenterY"), strInfo);
	//Radius
	strInfo.Format(_T("%f"), m_TrainResult.m_dCircleRadius);
	cXMLConfigurator.AddElem(_T("CircleResultRadius"), strInfo);	
	...
	for (int i=0;i<(INSPCET_TOOL_LINE_NUMS/2)+1;i++)
	{
		...
		cXMLConfigurator.EnterElem();
		// Caliper Num1
		strInfo.Format(_T("%d"), m_TrainGuiParam.m_nCaliperNum1[i]);
		cXMLConfigurator.AddElem(_T("CaliperNum1"), strInfo);
		// startX1
		strInfo.Format(_T("%f"), m_TrainGuiParam.m_dStartX1[i]);
		cXMLConfigurator.AddElem(_T("StartX1"), strInfo);
		// startY1
		strInfo.Format(_T("%f"), m_TrainGuiParam.m_dStartY1[i]);
		cXMLConfigurator.AddElem(_T("StartY1"), strInfo);
		// EndX1
		strInfo.Format(_T("%f"), m_TrainGuiParam.m_dEndX1[i]);
		cXMLConfigurator.AddElem(_T("EndX1"), strInfo);
		// EndY1
		strInfo.Format(_T("%f"), m_TrainGuiParam.m_dEndY1[i]);
		cXMLConfigurator.AddElem(_T("EndY1"), strInfo);
		// Project Len1
		strInfo.Format(_T("%f"), m_TrainGuiParam.m_dCaliperProjectLen1[i]);
		cXMLConfigurator.AddElem(_T("ProjectLen1"), strInfo);
		// Search Len1
		strInfo.Format(_T("%f"), m_TrainGuiParam.m_dCaliperSearchLen1[i]);
		cXMLConfigurator.AddElem(_T("SearchLen1"), strInfo);
		// Search Rotation1
		strInfo.Format(_T("%f"), m_TrainGuiParam.m_CaliperSearchRotation1[i].ToDouble());
		cXMLConfigurator.AddElem(_T("SearchRotation1"), strInfo);
		
		//Caliper Visible1
		strInfo.Format(_T("%d"), m_TrainGuiParam.m_bInspectCaliperVisible[i]);
		cXMLConfigurator.AddElem(_T("InspectCaliperVisible1"), strInfo);

		// Caliper Num2 
		strInfo.Format(_T("%d"), m_TrainGuiParam.m_nCaliperNum1[i+2]);
		cXMLConfigurator.AddElem(_T("CaliperNum2"), strInfo);
		// startX2
		strInfo.Format(_T("%f"), m_TrainGuiParam.m_dStartX1[i+2]);
		cXMLConfigurator.AddElem(_T("StartX2"), strInfo);
		// startY2
		strInfo.Format(_T("%f"), m_TrainGuiParam.m_dStartY1[i+2]);
		cXMLConfigurator.AddElem(_T("StartY2"), strInfo);
		// EndX
		strInfo.Format(_T("%f"), m_TrainGuiParam.m_dEndX1[i+2]);
		cXMLConfigurator.AddElem(_T("EndX2"), strInfo);
		// EndY2
		strInfo.Format(_T("%f"), m_TrainGuiParam.m_dEndY1[i+2]);
		cXMLConfigurator.AddElem(_T("EndY2"), strInfo);
		// Project Len2
		strInfo.Format(_T("%f"), m_TrainGuiParam.m_dCaliperProjectLen1[i+2]);
		cXMLConfigurator.AddElem(_T("ProjectLen2"), strInfo);
		// Search Len2
		strInfo.Format(_T("%f"), m_TrainGuiParam.m_dCaliperSearchLen1[i+2]);
		cXMLConfigurator.AddElem(_T("SearchLen2"), strInfo);
		// Search Rotation2
		strInfo.Format(_T("%f"), m_TrainGuiParam.m_CaliperSearchRotation1[i+2].ToDouble());
		cXMLConfigurator.AddElem(_T("SearchRotation2"), strInfo);
		//Caliper Visible2
		strInfo.Format(_T("%d"), m_TrainGuiParam.m_bInspectCaliperVisible[i+2]);
		cXMLConfigurator.AddElem(_T("InspectCaliperVisible2"), strInfo);
	...
	strInfo.Format(_T("%d"), m_SearchDataParam.m_bCheckAfterSearch);
	cXMLConfigurator.AddElem(_T("CheckAfterSearch"), strInfo);
	...
	//检测工具
	cXMLConfigurator.AddElem(_T("InspectSearchDataParam"));
	cXMLConfigurator.EnterElem();

	strInfo.Format("%d",m_SearchGuiParam.m_bEnableShowFitCirclePoint);
	cXMLConfigurator.AddElem(_T("EnableShowFitCirclePoint"), strInfo);

	strInfo.Format("%d",m_SearchGuiParam.m_bInspectVisibleFitPoint[4]);
	cXMLConfigurator.AddElem(_T("InspectVisibleFitPoint"), strInfo);
		...
		for (int i=0;i<INSPCET_TOOL_LINE_NUMS/2+1;i++)
		{
			...
			cXMLConfigurator.AddElem(tmpStr);
			cXMLConfigurator.EnterElem();
			strInfo.Format("%f",m_SearchDataParam.m_dPixelRatio[i]);
			cXMLConfigurator.AddElem(_T("PixelRatio"), strInfo);

			strInfo.Format("%d",m_SearchDataParam.m_nDistanceMode[i]);
			cXMLConfigurator.AddElem(_T("DistanceMode"), strInfo);

			strInfo.Format("%d",m_SearchDataParam.m_nDistanceType[i]);
			cXMLConfigurator.AddElem(_T("DistanceType"), strInfo);
			...
			if (i < INSPCET_TOOL_LINE_NUMS/2)
			{
				strInfo.Format("%d",m_SearchGuiParam.m_bInspectVisibleFitPoint[2*i]);
				cXMLConfigurator.AddElem(_T("InspectVisibleFitPoint1"), strInfo);

				strInfo.Format("%d",m_SearchGuiParam.m_bInspectVisibleFitPoint[i*2+1]);
				cXMLConfigurator.AddElem(_T("InspectVisibleFitPoint2"), strInfo);
			}
			cXMLConfigurator.LeaveElem();
		}
	...
	cXMLConfigurator.AddElem(_T("InspectTrainResult"));
	cXMLConfigurator.EnterElem();
	...
	for (int i=0;i<INSPCET_TOOL_LINE_NUMS+1;i++)
	{
		...
		cXMLConfigurator.AddElem(tmpStr);
		cXMLConfigurator.EnterElem();
		strInfo.Format(_T("%f"), m_TrainResult.m_dStartX1[i]);
		cXMLConfigurator.AddElem(_T("StartX1"), strInfo);
		strInfo.Format(_T("%f"), m_TrainResult.m_dStartY1[i]);
		cXMLConfigurator.AddElem(_T("StartY1"), strInfo);
		strInfo.Format(_T("%f"), m_TrainResult.m_dEndX1[i]);
		cXMLConfigurator.AddElem(_T("EndX1"), strInfo);
		strInfo.Format(_T("%f"), m_TrainResult.m_dEndY1[i]);
		cXMLConfigurator.AddElem(_T("EndY1"), strInfo);
		cXMLConfigurator.LeaveElem();
	}
	...
	if (m_TrainDataParam.m_bMaskEnable1 && m_ImageMask1.IsValid()  && m_tmpTrainDataParam.m_bEnableCheck)
	{
...
bool CCornerCheckSearchTool::SetGuiByCam(double nWidth,double nHeight)
{
    ...
    // 线距检测工具初始化属性
    m_GuiInspectFindLine[0].GetStartXYEndXY(sx,sy,ex,ey);
    m_GuiInspectFindLine[0].SetStartXYEndXY((sx/m_CamWidth)*nWidth, (sy/m_CamHeight)*nHeight,(ex/m_CamWidth)*nWidth,(ey/m_CamHeight)*nHeight);

    m_GuiInspectFindLine[1].GetStartXYEndXY(sx,sy,ex,ey);
    m_GuiInspectFindLine[1].SetStartXYEndXY((sx/m_CamWidth)*nWidth, (sy/m_CamHeight)*nHeight,(ex/m_CamWidth)*nWidth,(ey/m_CamHeight)*nHeight);

    m_GuiInspectFindLine[2].GetStartXYEndXY(sx,sy,ex,ey);
    m_GuiInspectFindLine[2].SetStartXYEndXY((sx/m_CamWidth)*nWidth, (sy/m_CamHeight)*nHeight,(ex/m_CamWidth)*nWidth,(ey/m_CamHeight)*nHeight);

    m_GuiInspectFindLine[3].GetStartXYEndXY(sx,sy,ex,ey);
    m_GuiInspectFindLine[3].SetStartXYEndXY((sx/m_CamWidth)*nWidth, (sy/m_CamHeight)*nHeight,(ex/m_CamWidth)*nWidth,(ey/m_CamHeight)*nHeight);
    
	m_GuiInspectFindLine[4].GetStartXYEndXY(sx,sy,ex,ey);
	m_GuiInspectFindLine[4].SetStartXYEndXY((sx/m_CamWidth)*nWidth, (sy/m_CamHeight)*nHeight,(ex/m_CamWidth)*nWidth,(ey/m_CamHeight)*nHeight);

    plength = m_GuiInspectFindLine[0].GetCaliperProjectionLength();
    slength = m_GuiInspectFindLine[0].GetCaliperSearchLength();
    m_GuiInspectFindLine[0].SetCaliperProjectionLength(plength/m_CamWidth*nWidth);
    m_GuiInspectFindLine[0].SetCaliperSearchLength(slength/m_CamWidth*nWidth);

    plength = m_GuiInspectFindLine[1].GetCaliperProjectionLength();
    slength = m_GuiInspectFindLine[1].GetCaliperSearchLength();
    m_GuiInspectFindLine[1].SetCaliperProjectionLength(plength/m_CamWidth*nWidth);
    m_GuiInspectFindLine[1].SetCaliperSearchLength(slength/m_CamWidth*nWidth);

    plength = m_GuiInspectFindLine[2].GetCaliperProjectionLength();
    slength = m_GuiInspectFindLine[2].GetCaliperSearchLength();
    m_GuiInspectFindLine[2].SetCaliperProjectionLength(plength/m_CamWidth*nWidth);
    m_GuiInspectFindLine[2].SetCaliperSearchLength(slength/m_CamWidth*nWidth);

    plength = m_GuiInspectFindLine[3].GetCaliperProjectionLength();
    slength = m_GuiInspectFindLine[3].GetCaliperSearchLength();
    m_GuiInspectFindLine[3].SetCaliperProjectionLength(plength/m_CamWidth*nWidth);
    m_GuiInspectFindLine[3].SetCaliperSearchLength(slength/m_CamWidth*nWidth);


	plength = m_GuiInspectFindLine[4].GetCaliperProjectionLength();
	slength = m_GuiInspectFindLine[4].GetCaliperSearchLength();
	m_GuiInspectFindLine[4].SetCaliperProjectionLength(plength/m_CamWidth*nWidth);
	m_GuiInspectFindLine[4].SetCaliperSearchLength(slength/m_CamWidth*nWidth);
...
// 更新训练控件数据信息	
bool CCornerCheckSearchTool::UpdateTrainGuiData(bool bSaveAndValidate)
{
	if (bSaveAndValidate)	// 从Gui控件获取数据
	{
		...
		//m_tmpTrainGuiParam.m_TrainRect1			= m_GuiTrainRect1.GetRect();
		//m_tmpTrainGuiParam.m_ModelPoint1			= m_GuiModelPoint1.GetCenter();
		//m_tmpTrainGuiParam.m_SearchRect1			= m_GuiSearchRect2.GetRect();
		//m_tmpTrainGuiParam.m_TrainAffineRect1	= m_GuiTrainAffineRect1.GetAffineRect();
		...
		for (int i=0;i<INSPCET_TOOL_LINE_NUMS+1;i++)
		{
			...
			m_tmpTrainGuiParam.m_bInspectCaliperVisible[i]=m_GuiInspectFindLine[i].IsCaliperVisible();

			m_GuiInspectFindLine[i].GetMaskData(m_tmpTrainGuiParam.m_vdMaskData_FindLine1[i]);
			m_GuiInspectFindLine[i].GetCaliperMasked(m_tmpTrainGuiParam.m_vbCaliperMask_FindLine1[i]);
			if (i == 4)
			{
	...
	//	if (m_tmpTrainDataParam.m_bEnableCheck)
		...
		for (int i=0;i<INSPCET_TOOL_LINE_NUMS+1;i++)
		{
			...
			m_GuiInspectFindLine[i].SetLineColor(m_tmpTrainGuiParam.m_LineColor);
			m_GuiInspectFindLine[i].SetLineWidth(m_tmpTrainGuiParam.m_nLineWidth);
			m_GuiInspectFindLine[i].SetCaliperNum(m_tmpTrainGuiParam.m_nCaliperNum1[i]);
			m_GuiInspectFindLine[i].SetStartXYEndXY(m_tmpTrainGuiParam.m_dStartX1[i], m_tmpTrainGuiParam.m_dStartY1[i], m_tmpTrainGuiParam.m_dEndX1[i],
				m_tmpTrainGuiParam.m_dEndY1[i]);
			m_GuiInspectFindLine[i].SetCaliperProjectionLength(m_tmpTrainGuiParam.m_dCaliperProjectLen1[i]);
			m_GuiInspectFindLine[i].SetCaliperSearchLength(m_tmpTrainGuiParam.m_dCaliperSearchLen1[i]);
			m_GuiInspectFindLine[i].SetCaliperSearchDirection(m_tmpTrainGuiParam.m_CaliperSearchRotation1[i]);
			
			m_GuiInspectFindLine[i].SetCaliperVisible(m_tmpTrainGuiParam.m_bInspectCaliperVisible[i]);
			m_GuiInspectFindLine[i].SetAffineRectFindLineVisible(!m_tmpTrainGuiParam.m_bInspectCaliperVisible[i]);

			m_GuiInspectFindLine[i].SetMaskData(m_tmpTrainGuiParam.m_vdMaskData_FindLine1[i]);
			if (i == 4)
			{
...
bool CCornerCheckSearchTool::UpdateSearchGuiData(bool bSaveAndValidate)
{
	if (bSaveAndValidate)	// 从Gui控件获取数据
	{
...
// 清除控件及其显示					
bool CCornerCheckSearchTool::ClearGraphicsArray(std::vector<scGuiGraphic*> &pGraphicsArray)
{
...
// 清除定位工具在Display上的控件
bool CCornerCheckSearchTool::ClearAllGui()
{
	ClearTrainGui();
...
// 清除搜索结果，需先清除结果GUI
bool CCornerCheckSearchTool::ClearResult()
{
	ClearResultGui();
	...
	ClearGraphicsArray(m_pGuiInspectFitPoint1Array);
	ClearGraphicsArray(m_pGuiInspectFitPoint2Array);
	ClearGraphicsArray(m_pGuiInspectFitPoint3Array);
	ClearGraphicsArray(m_pGuiInspectFitPoint4Array);
	ClearGraphicsArray(m_pGuiInspectFitPoint5Array);
	ClearGraphicsArray(m_pGuiInspectFitPoint6Array);
...
bool CCornerCheckSearchTool::ClearTrainGui()
{
	if (m_pGuiDisplay && ::IsWindow(m_hDispWnd))
	{
		int j=0;
...
bool CCornerCheckSearchTool::ClearResultGui()
{
	if (m_pGuiDisplay && ::IsWindow(m_hDispWnd))
	{
		int j=0;
		...
		for (j=0;j<m_pGuiInspectFitPoint1Array.size();j++)
		{
			m_pGuiStaticContainer->RemoveItem(m_pGuiInspectFitPoint1Array[j]->GetTipText());
		}

		for (j=0;j<m_pGuiInspectFitPoint2Array.size();j++)
		{
			m_pGuiStaticContainer->RemoveItem(m_pGuiInspectFitPoint2Array[j]->GetTipText());
		}

		for (j=0;j<m_pGuiInspectFitPoint3Array.size();j++)
		{
			m_pGuiStaticContainer->RemoveItem(m_pGuiInspectFitPoint3Array[j]->GetTipText());
		}

		for (j=0;j<m_pGuiInspectFitPoint4Array.size();j++)
		{
			m_pGuiStaticContainer->RemoveItem(m_pGuiInspectFitPoint4Array[j]->GetTipText());
		}

		for (j=0;j<m_pGuiInspectFitPoint5Array.size();j++)
		{
			m_pGuiStaticContainer->RemoveItem(m_pGuiInspectFitPoint5Array[j]->GetTipText());
		}

		for (j=0;j<m_pGuiInspectFitPoint6Array.size();j++)
		{
			m_pGuiStaticContainer->RemoveItem(m_pGuiInspectFitPoint6Array[j]->GetTipText());
		}
...
// 获取训练模板原始图像
bool CCornerCheckSearchTool::GetPatternImage(cpImage &ImagePattern)
{
	/*bool bRet = false;*/
...
bool CCornerCheckSearchTool::GetPatternImage2(cpImage &ImagePattern)
{
	//bool bRet = false;
	if (m_tmpImageTrain.IsValid() == false)
	{
...
// 恢复训练参数
bool CCornerCheckSearchTool::RestoreTrainParam()
{ 
...
// 恢复搜索参数		
bool CCornerCheckSearchTool::RestoreSearchParam()
{
...
void CCornerCheckSearchTool::SetLanguage(int nLanguage)
{
...
//scFindCircle
void CCornerCheckSearchTool::ShowFitPoint(std::vector<scGuiGraphic*> & m_pGuiInspectFitPointArray,scGuiCoordCross m_GuiInspectFitPoint[MAXPTSNUM],scFindCircle* m_FindCircleTool,const CString strFitPoint)
{
...
//scFindLine
void CCornerCheckSearchTool::ShowFitPoint(std::vector<scGuiGraphic*> & m_pGuiInspectFitPointArray,scGuiCoordCross m_GuiInspectFitPoint[MAXPTSNUM],scFindLine* m_FindLineTool,const CString strFitPoint)
{
	std::vector<bool> bUsed;
	std::vector<sc2Vector> points;
...
CString CCornerCheckSearchTool::GetResultErrInfo()
{
	CString strErrInfo;
	...
	case 1:
		{
			...
			case 0:
				...
				break;
			case -1:							
				strErrInfo += _T("检查区域定位失败");
				break;
			case -2:							
				strErrInfo += _T("检查几何定位失败");
				break;
			case -3:							
...
bool CCornerCheckSearchTool::ExecutePatternSerach(cpImage &ImageSearch,scPatternSearchRot* m_PatternSearchTool,scRect* m_SearchRect,int m_nSearchNum,double m_dAcceptThre,double m_dConfusionThre,double dTransX,double dTransY,double dRotation)
{
	bool bRet = false;
...
bool CCornerCheckSearchTool::ExecuteQuickSearch(cpImage &ImageSearch,scQuickSearchTool* m_QuickSearchTool,scRect* m_SearchRect,int m_nSearchNum,double m_dAcceptThre,double m_dAngleLow,double m_dAngleHigh,double m_dSizeLow1,double m_dSizeHigh1,double dTransX,double dTransY,double dRotation)
{
	bool bRet = false;
...
bool CCornerCheckSearchTool::ExecuteFindLine(cpImage &ImageSearch,scFindLine* m_FindLineTool,StructFindLineTrainData m_sFindLineTrainData,double dTransX,double dTransY,double dRotation,scLineSeg &ResultLine)
{
	bool bRet = false;
...
bool CCornerCheckSearchTool::GetFindLineCenterPoint(scFindLine* m_FindLineTool,sc2Vector &pointCenter2)
{
	bool bRet = false;
	if(NULL == m_FindLineTool)
	{
...
void CCornerCheckSearchTool::ShowResultRect(std::vector<scGuiGraphic*>& m_pGuiSearchRectArray,scGuiAffineRect m_GuiSearchAffineRect[MAXRESNUM],const int nOffset,scAffineRect* m_AffineRect,const CString strRect,COLORREF color,int LineWidth)
{
...
void CCornerCheckSearchTool::ShowResultCross(std::vector<scGuiGraphic*>& m_pGuiSearchCrossArray,scGuiCross m_GuiSearchCross[MAXRESNUM],const int nOffset,const sc2Vector &m_CrossVector,const CString strCross,COLORREF color,int LineWidth)
{
	CString strTmpKey = _T("");
...
void CCornerCheckSearchTool::ShowResultCross(std::vector<scGuiGraphic*>& m_pGuiSearchCrossArray,scGuiCross m_GuiSearchCross[MAXRESNUM],const int nOffset,const sc2Vector &m_CrossVector,const CString strCross,const CString strCrossLabel,COLORREF color,int LineWidth)
{
	CString strTmpKey = _T("");
	strTmpKey = m_strKeyText + strCross;
	CString tmpIndex = _T("");
...
void CCornerCheckSearchTool::ShowResultLine(std::vector<scGuiGraphic*>& m_pGuiSearchLineArray,scGuiLine m_GuiSearchLine[MAXRESNUM],const int nOffset,scLine *m_scLine,const CString strLine,COLORREF color,int LineWidth)
{
...
void CCornerCheckSearchTool::SetSearchResult(const CResultL2LInspectDistance &tmpL2LReslut, CSearchResult& searchResult)
{
	// 线线检测1
	if(m_TrainDataParam.m_bEnableInspect[0])
	{
		// 找线NG
		if (tmpL2LReslut.m_nResult[0] == 0)
		{

		}
		else if (tmpL2LReslut.m_nResult[0] == 1)	// 找线OK, 结果OK
		{
			CResultLine line;
			// 直线1
			line.m_vLineX[0] = tmpL2LReslut.m_dStartPosX[0];
			line.m_vLineY[0] = tmpL2LReslut.m_dStartPosY[0];
			line.m_vLineX[1] = tmpL2LReslut.m_dEndPosX[0];
			line.m_vLineY[1] = tmpL2LReslut.m_dEndPosY[0];
			searchResult.m_vLineSegSearchResult.push_back(line);
			// 直线2
			line.m_vLineX[0] = tmpL2LReslut.m_dStartPosX[1];
			line.m_vLineY[0] = tmpL2LReslut.m_dStartPosY[1];
			line.m_vLineX[1] = tmpL2LReslut.m_dEndPosX[1];
			line.m_vLineY[1] = tmpL2LReslut.m_dEndPosY[1];
			searchResult.m_vLineSegSearchResult.push_back(line);
			// 测量线段结果
			
			line.m_vLineX[0] = tmpL2LReslut.m_dMeasurePosX[0];
			line.m_vLineY[0] = tmpL2LReslut.m_dMeasurePosY[0];
			line.m_vLineX[1] = tmpL2LReslut.m_dProjectPosX[0];
			line.m_vLineY[1] = tmpL2LReslut.m_dProjectPosY[0];
			
			searchResult.m_vLineSegSearchResult.push_back(line);
		}
		else if (tmpL2LReslut.m_nResult[0] == -1)	// 找线OK, 结果NG
		{
			CResultLine line;
			// 直线1
			line.m_vLineX[0] = tmpL2LReslut.m_dStartPosX[0];
	...
	// 线线检测2
	if(m_TrainDataParam.m_bEnableInspect[1])
	{
		// 找线NG
		if (tmpL2LReslut.m_nResult[1] == 0)
		{

		}
		else if (tmpL2LReslut.m_nResult[1] == 1)	// 找线OK, 结果OK
		{
			CResultLine line;
			// 直线1
			line.m_vLineX[0] = tmpL2LReslut.m_dStartPosX[2];
			line.m_vLineY[0] = tmpL2LReslut.m_dStartPosY[2];
			line.m_vLineX[1] = tmpL2LReslut.m_dEndPosX[2];
			line.m_vLineY[1] = tmpL2LReslut.m_dEndPosY[2];
			searchResult.m_vLineSegSearchResult.push_back(line);
			// 直线2
			line.m_vLineX[0] = tmpL2LReslut.m_dStartPosX[3];
			line.m_vLineY[0] = tmpL2LReslut.m_dStartPosY[3];
			line.m_vLineX[1] = tmpL2LReslut.m_dEndPosX[3];
			line.m_vLineY[1] = tmpL2LReslut.m_dEndPosY[3];
			searchResult.m_vLineSegSearchResult.push_back(line);
			// 测量线段结果
			line.m_vLineX[0] = tmpL2LReslut.m_dMeasurePosX[1];
			line.m_vLineY[0] = tmpL2LReslut.m_dMeasurePosY[1];
			line.m_vLineX[1] = tmpL2LReslut.m_dProjectPosX[1];
			line.m_vLineY[1] = tmpL2LReslut.m_dProjectPosY[1];
			searchResult.m_vLineSegSearchResult.push_back(line);
		}
		else if (tmpL2LReslut.m_nResult[1] == -1)	// 找线OK, 结果NG
		{
			CResultLine line;
			// 直线1
			line.m_vLineX[0] = tmpL2LReslut.m_dStartPosX[2];
	...
	// 线圆检测
	if(m_TrainDataParam.m_bEnableInspect[2])
	{
		// 找线NG
		if (tmpL2LReslut.m_nResult[2] == 0)
		{

		}
		else if (tmpL2LReslut.m_nResult[2] == 1)	// 找线OK, 结果OK
		{
			CResultLine line;
			CResultCircleSearchTool circle;
			// 直线1
			line.m_vLineX[0] = tmpL2LReslut.m_dStartPosX[4];
			line.m_vLineY[0] = tmpL2LReslut.m_dStartPosY[4];
			line.m_vLineX[1] = tmpL2LReslut.m_dEndPosX[4];
			line.m_vLineY[1] = tmpL2LReslut.m_dEndPosY[4];
			searchResult.m_vLineSearchResult.push_back(line);
			// 圆
			circle.m_scCircleCircle = sc2Vector(tmpL2LReslut.m_dCircleCenterX, tmpL2LReslut.m_dCircleCenterY);
			circle.m_dRadius = tmpL2LReslut.m_dCircleRadius;
			searchResult.m_vCircleSearchResult.push_back(circle);
			// 测量线段结果
			line.m_vLineX[0] = tmpL2LReslut.m_dMeasurePosX[2];
			line.m_vLineY[0] = tmpL2LReslut.m_dMeasurePosY[2];
			line.m_vLineX[1] = tmpL2LReslut.m_dProjectPosX[2];
			line.m_vLineY[1] = tmpL2LReslut.m_dProjectPosY[2];
			searchResult.m_vLineSegSearchResult.push_back(line);
		}
		else if (tmpL2LReslut.m_nResult[2] == -1)	// 找线OK, 结果NG
		{
			CResultLine line;
			CResultCircleSearchTool circle;
			// 直线1
	...
	// 圆检测
	if(m_TrainDataParam.m_bEnableInspect[3])
	{
		// 找线NG
		if (tmpL2LReslut.m_nResult[2] == 0)
		{
