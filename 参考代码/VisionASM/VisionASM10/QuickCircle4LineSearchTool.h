...
#define QUICK_CIRCLE_4LINE_NUM 4

// PatternCircleLineSearch定位工具训练Data参数类
typedef struct tagQuickCircle4LineSearchTrainDataParam : public CBaseTrainDataParam
{
	// 几何
	double	m_dGrau;									// 颗粒度
	...
	// 找线工具
	double			m_dContrastThre[QUICK_CIRCLE_4LINE_NUM] ;			// 对比度阈值
	long			m_lFilterHalfWidth[QUICK_CIRCLE_4LINE_NUM];		// 滤波器半宽
	int				m_nOutLineNum[QUICK_CIRCLE_4LINE_NUM];			// 局外点
	EdgePolarity	m_edgeProperty[QUICK_CIRCLE_4LINE_NUM];			// 边缘属性
	bool			m_bSortByScore[QUICK_CIRCLE_4LINE_NUM];			// 按分数排序
	int				m_nLineAngleType[QUICK_CIRCLE_4LINE_NUM];		// 直线角度类型：0,任意角度；1,水平线；2,垂直线；
	bool            m_bEnableLine[QUICK_CIRCLE_4LINE_NUM];			// 直线启用
	bool			m_bEnableLineDualEdge[QUICK_CIRCLE_4LINE_NUM];	// 启用/禁用找线工具双边缘
	tagQuickCircle4LineSearchTrainDataParam()
	{
		m_dGrau			= 2.0;
		m_lNoiseThre	= 10;
		m_lTotalMag		= 200;
...
}CQuickCircle4LineSearchTrainDataParam;

// PatternCircleLineSearch定位工具训练Gui参数类
typedef struct tagQuickCircle4LineSearchTrainGuiParam : public CBaseTrainGuiParam
{
	...
	// 找线工具
	int				m_nCaliperNum[QUICK_CIRCLE_4LINE_NUM];			// 卡尺个数
	double			m_dStartX[QUICK_CIRCLE_4LINE_NUM];				// 起始点X
	double			m_dStartY[QUICK_CIRCLE_4LINE_NUM];				// 起始点Y
	double			m_dEndX[QUICK_CIRCLE_4LINE_NUM];					// 终止点X
	double			m_dEndY[QUICK_CIRCLE_4LINE_NUM];					// 终止点Y
	double			m_dCaliperProjectLen[QUICK_CIRCLE_4LINE_NUM];		// 投影长度
	double			m_dCaliperSearchLen[QUICK_CIRCLE_4LINE_NUM];		// 搜索长度
	scRadian		m_CaliperSearchRotation[QUICK_CIRCLE_4LINE_NUM];	// 搜索角度
	bool			m_bLineCaliperVisible[QUICK_CIRCLE_4LINE_NUM];	//找线卡尺是否可见	
	// 找线工具掩摸
	svStd vector<double> m_vdLineCaliperMaskData[QUICK_CIRCLE_4LINE_NUM];			// 掩膜数据
	svStd vector<bool>   m_vbLineCaliperMask[QUICK_CIRCLE_4LINE_NUM];				// 卡尺是否被掩膜
	bool			     m_bEnableLineMask[QUICK_CIRCLE_4LINE_NUM];			// 是否启用掩膜
	...
	tagQuickCircle4LineSearchTrainGuiParam()
	{
		//圆参数
...
}CQuickCircle4LineSearchTrainGuiParam;

// PatternCircleLineSearch定位工具搜索Data参数类
typedef struct tagQuickCircle4LineSearchSearchDataParam : public CBaseSearchDataParam
{
	...
	tagQuickCircle4LineSearchSearchDataParam()
	{
		m_nSearchNum	= 1;
		m_dAcceptThre	= 0.8;
		m_dAngleLow		= -5.0;
...
}CQuickCircle4LineSearchSearchDataParam;

// PatternCircleLineSearch定位工具搜索Gui参数类
typedef struct tagQuickCircle4LineSearchGuiParam : public CBaseSearchGuiParam
{
	...
	bool			m_bEnableShowFitLinePoint[QUICK_CIRCLE_4LINE_NUM];			// 显示直线拟合点

	tagQuickCircle4LineSearchGuiParam()
	{
...
}CQuickCircle4LineSearchSearchGuiParam;

// PatternSearch训练结果
typedef struct tagQuickCircle4LineSearchTrainResult
{
	double m_dModelX; // 参考点X					
	double m_dModelY; // 参考点Y
	double m_dSharpness;	//模板清晰度值

	double m_dStartX[QUICK_CIRCLE_4LINE_NUM]; // 线段起点X
	double m_dStartY[QUICK_CIRCLE_4LINE_NUM]; // 线段起点Y
	double m_dEndX[QUICK_CIRCLE_4LINE_NUM];	  // 线段终点X
	double m_dEndY[QUICK_CIRCLE_4LINE_NUM];	  // 线段终点Y
	...
	tagQuickCircle4LineSearchTrainResult()
	{
		m_dModelX = 0;
		m_dModelY = 0;
		m_dSharpness = 1.0;
...
}CQuickCircle4LineSearchTrainResult;

class CQuickCircle4LineSearchTool :public CBaseSearchTool
{
public:
	CQuickCircle4LineSearchTool(void);
	CQuickCircle4LineSearchTool(CQuickCircle4LineSearchTool &cPatternCircleLineSearchTool);
	CQuickCircle4LineSearchTool& operator=(CQuickCircle4LineSearchTool &cPatternCircleLineSearchTool);
	~CQuickCircle4LineSearchTool(void);
...
private:
	...
	scFindLine							m_FindLineTool[QUICK_CIRCLE_4LINE_NUM];		// 找线工具
	...
	CQuickCircle4LineSearchTrainDataParam			m_tmpTrainDataParam;			// 当前训练Data参数
	CQuickCircle4LineSearchTrainGuiParam			m_tmpTrainGuiParam;				// 当前训练Gui参数	
	CQuickCircle4LineSearchSearchDataParam		m_tmpSearchDataParam;			// 当前搜索Data参数	
	CQuickCircle4LineSearchSearchGuiParam			m_tmpSearchGuiParam;			// 当前搜索Gui参数
	CQuickCircle4LineSearchTrainResult			m_tmpTrainResult;				// 当前训练结果
	cpImage							m_tmpImageTrain;				// 当前训练图像
	cpImage							m_tmpImageMask;					// 当前掩模图像
	CQuickCircle4LineSearchTrainDataParam			m_TrainDataParam;				// 训练Data参数
	CQuickCircle4LineSearchTrainGuiParam			m_TrainGuiParam;				// 训练Gui参数	
	CQuickCircle4LineSearchSearchDataParam		m_SearchDataParam;				// 搜索Data参数	
	CQuickCircle4LineSearchSearchGuiParam			m_SearchGuiParam;				// 搜索Gui参数	
	CQuickCircle4LineSearchTrainResult			m_TrainResult;					// 训练结果
	...
	scGuiFindLine                       m_GuiFindLine[QUICK_CIRCLE_4LINE_NUM];		// 两个找线工具
	scGuiLine                           m_GuiTrainResultLine[QUICK_CIRCLE_4LINE_NUM];	// 训练结果线段
	...
	scGuiLine							m_GuiSearchLine[MAXRESNUM*QUICK_CIRCLE_4LINE_NUM];
