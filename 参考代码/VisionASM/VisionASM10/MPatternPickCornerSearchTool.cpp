...
//
//
//CMPatternPickCornerSearchTool::CMPatternPickCornerSearchTool(void)
//{
//}
//
//
...
CMPatternPickCornerSearchTool::CMPatternPickCornerSearchTool(void)
{
	...
	m_GuiACFHand1SearchRect.SetCenterWidthHeight(250, 300, 50, 50);	
	m_GuiACFHand1SearchRect.SetLabelVisible(TRUE);
	m_GuiACFHand2SearchRect.SetCenterWidthHeight(300, 350, 50, 50);	
	m_GuiACFHand2SearchRect.SetLabelVisible(TRUE);	

	m_GuiSearch1HideRect.SetCenterWidthHeight(350, 240, 352, 180);
	m_GuiSearch1HideRect.SetLabelVisible(TRUE);
	m_GuiSearch2HideRect.SetCenterWidthHeight(350, 240, 352, 180);
	...
	case 0:
		...
		m_GuiACFHand1SearchRect.SetLabel(_T("ACF1"));
		m_GuiACFHand2SearchRect.SetLabel(_T("ACF2"));
		break;
	case 1:
		...
		m_GuiACFHand1SearchRect.SetLabel(_T("ACF检测1"));
		m_GuiACFHand2SearchRect.SetLabel(_T("ACF检测2"));
		break;
	default:
		m_GuiTrainRect.SetLabel(_T("训练区域"));
		m_GuiSearchRect.SetLabel(_T("搜索区域"));
	...
	//ACF检测参数
	m_tmpTrainDataParam.m_dCurMinMean = 0;
	m_tmpTrainDataParam.m_dCurMaxMean = 255;
	m_tmpTrainDataParam.m_dCurMinStdDev = 0;
	m_tmpTrainDataParam.m_dCurMaxStdDev = 255;
	//m_nCurLogicIndex = 0;

	m_tmpTrainDataParam.m_bMeanInvalid = FALSE;
	m_tmpTrainDataParam.m_bStdDevInvalid = FALSE;
	m_tmpTrainDataParam.m_bMeanInvert = FALSE;
	m_tmpTrainDataParam.m_bStdDevInvert = FALSE;
	//m_tmpTrainDataParam.m_nInpectTool = FALSE;
	m_tmpTrainDataParam.m_nCurLogicIndex = 0;
	...
	m_tmpTrainGuiParam.m_ACFSearch1HideRect    = m_GuiACFHand1SearchRect.GetRect();
	m_tmpTrainGuiParam.m_ACFSearch2HideRect    = m_GuiACFHand2SearchRect.GetRect();
...
CMPatternPickCornerSearchTool::CMPatternPickCornerSearchTool(CMPatternPickCornerSearchTool &cPatternCornerSearchTool)
{
	if (cPatternCornerSearchTool.GetSearchToolType() != eMPatternPickCornerSearchTool)
	{
		return;
	}
	...
	m_GuiSearch2HideRect.SetCenterWidthHeight(360, 240, 352, 180);
	//m_GuiSearchRect.SetLabel(_T("搜索区域"));
	m_GuiSearch2HideRect.SetLabelVisible(TRUE);
...
CMPatternPickCornerSearchTool& CMPatternPickCornerSearchTool::operator=(CMPatternPickCornerSearchTool &cPatternCornerSearchTool)
{
...
CSearchToolType CMPatternPickCornerSearchTool::GetSearchToolType()
{
	return eMPatternPickCornerSearchTool;
}

// 设置显示控件指针		
bool CMPatternPickCornerSearchTool::SetDisplay(scGuiDisplay *pGuiDisplay)
{
	ClearAllGui();
...
// 获取显示控件指针										
scGuiDisplay* CMPatternPickCornerSearchTool::GetDisplay()
{
	return m_pGuiDisplay;
}

// 设置Gui图形的关键字														
bool CMPatternPickCornerSearchTool::SetGuiKeyText(CString strGuiKeyText)									
{
...
// 获取Gui图形的关键字
bool CMPatternPickCornerSearchTool::GetGuiKeyText(CString &strGuiKeyText)
{
...
// 更新训练界面显示					
bool CMPatternPickCornerSearchTool::ShowTrainGui(bool bShow)
{
...
// 是否显示训练结果界面
bool CMPatternPickCornerSearchTool::ShowTrainResultGui(bool bShow)
{
	return false;
}

// 更新搜索界面显示									
bool CMPatternPickCornerSearchTool::ShowSearchResultGui(bool bShow)							
{
...
// 更新训练界面显示					
bool CMPatternPickCornerSearchTool::UpdateTrainGuiDisplay()
{	
	...
	m_GuiACFHand1SearchRect.SetVisible(m_bTrainGuiShow && m_tmpTrainGuiParam.m_nInpectTool);
	m_GuiACFHand2SearchRect.SetVisible(m_bTrainGuiShow && m_tmpTrainGuiParam.m_nInpectTool);
	...
	if (m_pGuiDisplay && ::IsWindow(m_hDispWnd))
	{
		...
		m_pGuiInteractiveContainer->AddItem(&m_GuiACFHand1SearchRect, m_strKeyText+_T("_ACFHand1SearchRect"));
		m_pGuiInteractiveContainer->AddItem(&m_GuiACFHand2SearchRect, m_strKeyText+_T("_ACFHand2SearchRect"));
...
bool CMPatternPickCornerSearchTool::UpdateTrainResultGuiDisplay()
{
	// 训练结果
...
// 更新搜索界面显示									
bool CMPatternPickCornerSearchTool::UpdateSearchResultGuiDisplay()							
{
	int i=0;
	...
	{
		 m_pGuiSearchResultRectArray[i]->SetVisible(m_bSearchResultGuiShow && m_tmpSearchGuiParam.m_bRectVisible);
	}
...
// 更新界面显示
bool CMPatternPickCornerSearchTool::UpdateGuiDisplay()
{
	UpdateTrainGuiDisplay();
...
// 设置训练Data参数	
bool CMPatternPickCornerSearchTool::SetTrainDataParam(CBaseTrainDataParam *pTrainDataParam)
{
	if (!pTrainDataParam)
	{
		return false;
	...
	if (pTrainDataParam->GetSearchToolType() != eMPatternPickCornerSearchTool)
	{
		return false;
	}

	m_tmpTrainDataParam = *((CMPatternPickCornerSearchTrainDataParam*)pTrainDataParam);

	return true;
}

// 获取训练Data参数									
bool CMPatternPickCornerSearchTool::GetTrainDataParam(CBaseTrainDataParam *pTrainDataParam)
{
	if (!pTrainDataParam)
	{
		return false;
	...
	*((CMPatternPickCornerSearchTrainDataParam*)pTrainDataParam) = m_tmpTrainDataParam;

	return true;
}

// 获取搜索Gui参数						
bool CMPatternPickCornerSearchTool::SetTrainGuiParam(CBaseTrainGuiParam *pTrainGuiParam)
{
	if (!pTrainGuiParam)
	{
		return false;
	...
	if (pTrainGuiParam->GetSearchToolType() != eMPatternPickCornerSearchTool)
	{
		return false;
	}

	m_tmpTrainGuiParam = *((CMPatternPickCornerSearchTrainGuiParam*)pTrainGuiParam);
...
// 获取搜索Gui参数							
bool CMPatternPickCornerSearchTool::GetTrainGuiParam(CBaseTrainGuiParam *pTrainGuiParam)
{
	if (!pTrainGuiParam)
	{
		return false;
	...
	*((CMPatternPickCornerSearchTrainGuiParam*)pTrainGuiParam) = m_tmpTrainGuiParam;

	return true;
}

// 设置训练原始图像				
bool CMPatternPickCornerSearchTool::SetTrainImage(const cpImage &ImageTrain)
{
...
// 获取训练原始图像									
bool CMPatternPickCornerSearchTool::GetTrainImage(cpImage &ImageTrain)
{
...
// 设置训练原始图像									
bool CMPatternPickCornerSearchTool::SetMaskImage(const cpImage &ImageMask)
{
...
// 获取训练原始图像								
bool CMPatternPickCornerSearchTool::GetMaskImage(cpImage &ImageMask)
{
...
// 执行训练											
bool CMPatternPickCornerSearchTool::Train()
{
	m_bTrainOK = false;
...
// 获取训练结果图像														
bool CMPatternPickCornerSearchTool::GetTrainResultImage(cpImage &ImageTrainResult)
{
...
// 获取训练结果
bool CMPatternPickCornerSearchTool::GetTrainResult(CBaseTrainResult *pTrainResult)
{
	if (!pTrainResult)
	{
		return false;
	...
	if (pTrainResult->GetSearchToolType() != eMPatternPickCornerSearchTool)
	{
		return false;
	}

	*((CMPatternPickCornerSearchTrainResult*)pTrainResult) = m_tmpTrainResult;

	return true;
}	

// 是否训练成功
bool CMPatternPickCornerSearchTool::IsTrained()
{
	return m_bTrainOK;
}

// 设置搜索Data参数	
bool CMPatternPickCornerSearchTool::SetSearchDataParam(CBaseSearchDataParam *pSearchDataParam)
{	
	if (!pSearchDataParam)
	{
		return false;
	...
	if (pSearchDataParam->GetSearchToolType() != eMPatternPickCornerSearchTool)
	{
		return false;
	}

	m_tmpSearchDataParam = *((CMPatternPickCornerSearchSearchDataParam*)pSearchDataParam);

	return true;
}

// 获取搜索Data参数				
bool CMPatternPickCornerSearchTool::GetSearchDataParam(CBaseSearchDataParam *pSearchDataParam)
{
	if (!pSearchDataParam)
	{
		return false;
	...
	*((CMPatternPickCornerSearchSearchDataParam*)pSearchDataParam) = m_tmpSearchDataParam;

	return true;
}

// 设置搜索Gui参数					
bool CMPatternPickCornerSearchTool::SetSearchGuiParam(CBaseSearchGuiParam *pSearchGuiParam)
{
	if (!pSearchGuiParam)
	{
		return false;
	...
	if (pSearchGuiParam->GetSearchToolType() != eMPatternPickCornerSearchTool)
	{
		return false;
	}

	m_tmpSearchGuiParam = *((CMPatternPickCornerSearchSearchGuiParam*)pSearchGuiParam);
...
// 获取搜索Gui参数			
bool CMPatternPickCornerSearchTool::GetSearchGuiParam(CBaseSearchGuiParam *pSearchGuiParam)
{
	if (!pSearchGuiParam)
	{
		return false;
	...
	*((CMPatternPickCornerSearchSearchGuiParam*)pSearchGuiParam) = m_tmpSearchGuiParam;

	return true;
}

// 执行搜索 					
bool CMPatternPickCornerSearchTool::Search(cpImage &ImageSearch)
{
	if (!m_bTrainOK)
	{
...
//		m_FindLineTool[i].SetCaliperProperty(m_TrainDataParam.m_dContrastThre[i], m_TrainDataParam.m_edgeProperty[i], m_TrainDataParam.m_lFilterHalfWidth[i]);
	...
	if (bRet)
	{
		...
		std::vector<bool> ACFindex;
		ACFindex.resize(2,false);
		if (m_tmpTrainGuiParam.m_nInpectTool)
		{
			
			ACFSearch(ImageSearch,ACFindex);
		}
			...
			if (bFindLine[0] && bFindLine[1] && bFindLine[2] && bFindLine[3])
			{
				...
				if (PointInRect(center,m_tmpTrainGuiParam.m_Hand1SearchRect)&&!hand1flag)
				{
					...
					if (m_tmpTrainGuiParam.m_bSearchHideRectVisible1)
					{
						...
						else if (m_tmpTrainGuiParam.m_nInpectTool && ACFindex.at(0))
						{
							sc2Vector UpLeftPos2 = m_tmpTrainGuiParam.m_Search2HideRect.GetUL();   //左上角
							sc2Vector DownRightPos2 = m_tmpTrainGuiParam.m_Search2HideRect.GetLR();//右下角

							if( (UpLeftPos2.GetX() < pos.GetX()) && (pos.GetX() < DownRightPos2.GetX()) &&
								(UpLeftPos2.GetY() < pos.GetY()) && (pos.GetY() < DownRightPos2.GetY()) )
							{
								ACFindex.at(0) = FALSE;
								hand1flag=true;
								searchResultHand1 = searchResult;
				...
				if (PointInRect(center,m_tmpTrainGuiParam.m_Hand2SearchRect)&&!hand2flag)
				{
					if (m_tmpTrainGuiParam.m_bSearchHideRectVisible2)
					{
						...
						else if (m_tmpTrainGuiParam.m_nInpectTool && ACFindex.at(1))
						{
							sc2Vector UpLeftPos1 = m_tmpTrainGuiParam.m_Search1HideRect.GetUL();   //左上角
							sc2Vector DownRightPos1 = m_tmpTrainGuiParam.m_Search1HideRect.GetLR();//右下角

							if( (UpLeftPos1.GetX() < pos.GetX()) && (pos.GetX() < DownRightPos1.GetX()) &&
								(UpLeftPos1.GetY() < pos.GetY()) && (pos.GetY() < DownRightPos1.GetY()) )
							{
								hand2flag=true;
								ACFindex.at(1) = FALSE;
								searchResultHand2 = searchResult;
		...
		if (m_SearchResultArray.size() <= 0 || ACFindex.at(0)||ACFindex.at(1) )
		{
			return false;
		}
...
// 获取搜索结果个数										
int CMPatternPickCornerSearchTool::GetResultNum()
{
	return m_SearchResultArray.size();
}

// 获取搜索结果														
bool CMPatternPickCornerSearchTool::GetResult(int nIndex, CSearchResult &searchResult)
{
...
// 加载模板训练图像
bool CMPatternPickCornerSearchTool::LoadModelTrainPatternImage(CString strDir ,cpImage &ImagePattern)
{
	// strDir为文件夹路径
	// 仅配置ModelImage.bmp
	if (strDir == _T(""))
	...
	if (cSearchToolType != eMPatternPickCornerSearchTool)
	{
		return false;
	}
...
// 加载模板文件
bool CMPatternPickCornerSearchTool::LoadModelFromFile(CString strDir)
{
	// strDir为文件夹路径
	// 配置文件包括以下部分:
	//    (1) 配置文件Model.xml
	...
	sc2Vector vPos;
	//	BYTE byR, byG, byB;
	CMPatternPickCornerSearchTrainDataParam tmpTrainDataParam;
	CMPatternPickCornerSearchTrainGuiParam tmpTrainGuiParam;
	CMPatternPickCornerSearchSearchDataParam tmpSearchDataParam;
	CMPatternPickCornerSearchSearchGuiParam tmpSearchGuiParam;	
	CMPatternPickCornerSearchTrainResult    tmpTrainResult;
	...
	//ACF检测参数
	bRet = cXMLConfigurator.FindElem(_T("CurMinMean"));
	if (!bRet)
	{
		return false;
	}
	tmpTrainDataParam.m_dCurMinMean = _tcstod(cXMLConfigurator.GetElemData(), NULL);
	bRet = cXMLConfigurator.FindElem(_T("CurMaxMean"));
	if (!bRet)
	{
		return false;
	}
	tmpTrainDataParam.m_dCurMaxMean = _tcstod(cXMLConfigurator.GetElemData(), NULL);
	bRet = cXMLConfigurator.FindElem(_T("CurMinStdDev"));
	if (!bRet)
	{
		return false;
	}
	tmpTrainDataParam.m_dCurMinStdDev = _tcstod(cXMLConfigurator.GetElemData(), NULL);
	bRet = cXMLConfigurator.FindElem(_T("CurMaxStdDev"));
	if (!bRet)
	{
		return false;
	}
	tmpTrainDataParam.m_dCurMaxStdDev = _tcstod(cXMLConfigurator.GetElemData(), NULL);
	//m_nCurLogicIndex = 0;
	...
	tmpTrainDataParam.m_bMeanInvalid = _ttoi(cXMLConfigurator.GetElemData());
	bRet = cXMLConfigurator.FindElem(_T("MeanInvert"));
	if (!bRet)
	{
		return false;
	}
	tmpTrainDataParam.m_bMeanInvert = _ttoi(cXMLConfigurator.GetElemData());
	bRet = cXMLConfigurator.FindElem(_T("StdDevInvalid"));
	if (!bRet)
	{
		return false;
	}
	tmpTrainDataParam.m_bStdDevInvalid = _ttoi(cXMLConfigurator.GetElemData());
	bRet = cXMLConfigurator.FindElem(_T("StdDevInvert"));
	if (!bRet)
	{
		return false;
	}
	tmpTrainDataParam.m_bStdDevInvert = _ttoi(cXMLConfigurator.GetElemData());

	// outlinenum1
	bRet = cXMLConfigurator.FindElem(_T("CurLogicIndex"));
	if (!bRet)
	{
		return false;
	}
	tmpTrainDataParam.m_nCurLogicIndex = _ttol(cXMLConfigurator.GetElemData());
	...
	bRet = cXMLConfigurator.FindElem(_T("InpectTool"));
	if (!bRet)
	{
		return false;
	}
	tmpTrainGuiParam.m_nInpectTool = _ttoi(cXMLConfigurator.GetElemData());
	...
	//scRect			m_ACFSearch1HideRect;					// ACF检测1
	// search Hide rect
	bRet = cXMLConfigurator.FindElem(_T("ACFSearch1HideRect"));
	if(bRet)
	{
		cXMLConfigurator.EnterElem();
		strInfo.Format(_T("%f"), tmpTrainGuiParam.m_ACFSearch1HideRect.GetOrigin().GetX());
		...
		vPos.SetY(_tcstod(cXMLConfigurator.GetElemData(), NULL));
		tmpTrainGuiParam.m_ACFSearch1HideRect.SetOrigion(vPos);
		bRet = cXMLConfigurator.FindElem(_T("SizeX"));
		if (!bRet)
		{
			return false;
		...
		vPos.SetY(_tcstod(cXMLConfigurator.GetElemData(), NULL));
		tmpTrainGuiParam.m_ACFSearch1HideRect.SetSize(vPos);
		cXMLConfigurator.LeaveElem();
	}

	//scRect			m_ACFSearch2HideRect;					// ACF检测2
	// search Hide rect
	bRet = cXMLConfigurator.FindElem(_T("ACFSearch2HideRect"));
	if(bRet)
	{
		cXMLConfigurator.EnterElem();
		strInfo.Format(_T("%f"), tmpTrainGuiParam.m_ACFSearch2HideRect.GetOrigin().GetX());
		...
		vPos.SetY(_tcstod(cXMLConfigurator.GetElemData(), NULL));
		tmpTrainGuiParam.m_ACFSearch2HideRect.SetOrigion(vPos);
		bRet = cXMLConfigurator.FindElem(_T("SizeX"));
		if (!bRet)
		{
			return false;
		...
		vPos.SetY(_tcstod(cXMLConfigurator.GetElemData(), NULL));
		tmpTrainGuiParam.m_ACFSearch2HideRect.SetSize(vPos);
		cXMLConfigurator.LeaveElem();
	}
...
// 保存模板文件							
bool CMPatternPickCornerSearchTool::SaveModelToFile(CString strDir)
{
	...
	strInfo.Format(_T("%f"), m_TrainDataParam.m_dCurMinMean);
	cXMLConfigurator.AddElem(_T("CurMinMean"), strInfo);
	strInfo.Format(_T("%f"), m_TrainDataParam.m_dCurMaxMean);
	cXMLConfigurator.AddElem(_T("CurMaxMean"), strInfo);
	strInfo.Format(_T("%f"), m_TrainDataParam.m_dCurMinStdDev);
	cXMLConfigurator.AddElem(_T("CurMinStdDev"), strInfo);
	strInfo.Format(_T("%f"), m_TrainDataParam.m_dCurMaxStdDev);
	cXMLConfigurator.AddElem(_T("CurMaxStdDev"), strInfo);

	strInfo.Format(_T("%d"), m_TrainDataParam.m_bMeanInvalid);
	cXMLConfigurator.AddElem(_T("MeanInvalid"), strInfo);
	strInfo.Format(_T("%d"), m_TrainDataParam.m_bMeanInvert);
	cXMLConfigurator.AddElem(_T("MeanInvert"), strInfo);
	strInfo.Format(_T("%d"), m_TrainDataParam.m_bStdDevInvalid);
	cXMLConfigurator.AddElem(_T("StdDevInvalid"), strInfo);
	strInfo.Format(_T("%d"), m_TrainDataParam.m_bStdDevInvert);
	cXMLConfigurator.AddElem(_T("StdDevInvert"), strInfo);

	strInfo.Format(_T("%d"), m_TrainDataParam.m_nCurLogicIndex);
	cXMLConfigurator.AddElem(_T("CurLogicIndex"), strInfo);
	...
	strInfo.Format(_T("%d"), m_TrainGuiParam.m_nInpectTool);
	cXMLConfigurator.AddElem(_T("InpectTool"), strInfo);
	// search rect
	...
	// ACFSearch1HideRect
	cXMLConfigurator.AddElem(_T("ACFSearch1HideRect"));
	cXMLConfigurator.EnterElem();
	strInfo.Format(_T("%f"), m_TrainGuiParam.m_ACFSearch1HideRect.GetOrigin().GetX());
	cXMLConfigurator.AddElem(_T("OriginX"), strInfo);
	strInfo.Format(_T("%f"), m_TrainGuiParam.m_ACFSearch1HideRect.GetOrigin().GetY());
	cXMLConfigurator.AddElem(_T("OriginY"), strInfo);
	strInfo.Format(_T("%f"), m_TrainGuiParam.m_ACFSearch1HideRect.GetSize().GetX());
	cXMLConfigurator.AddElem(_T("SizeX"), strInfo);
	strInfo.Format(_T("%f"), m_TrainGuiParam.m_ACFSearch1HideRect.GetSize().GetY());
	cXMLConfigurator.AddElem(_T("SizeY"), strInfo);
	cXMLConfigurator.LeaveElem();

	// ACFSearch2HideRect
	cXMLConfigurator.AddElem(_T("ACFSearch2HideRect"));
	cXMLConfigurator.EnterElem();
	strInfo.Format(_T("%f"), m_TrainGuiParam.m_ACFSearch2HideRect.GetOrigin().GetX());
	cXMLConfigurator.AddElem(_T("OriginX"), strInfo);
	strInfo.Format(_T("%f"), m_TrainGuiParam.m_ACFSearch2HideRect.GetOrigin().GetY());
	cXMLConfigurator.AddElem(_T("OriginY"), strInfo);
	strInfo.Format(_T("%f"), m_TrainGuiParam.m_ACFSearch2HideRect.GetSize().GetX());
	cXMLConfigurator.AddElem(_T("SizeX"), strInfo);
	strInfo.Format(_T("%f"), m_TrainGuiParam.m_ACFSearch2HideRect.GetSize().GetY());
	cXMLConfigurator.AddElem(_T("SizeY"), strInfo);
	cXMLConfigurator.LeaveElem();
...
// 更新训练控件数据信息	
bool CMPatternPickCornerSearchTool::UpdateTrainGuiData(bool bSaveAndValidate)
{
	...
	{	
		...
		m_GuiACFHand1SearchRect.SetLineColor(m_tmpTrainGuiParam.m_LineColor);
		m_GuiACFHand1SearchRect.SetLineWidth(m_tmpTrainGuiParam.m_nLineWidth);
		m_GuiACFHand1SearchRect.SetRect(m_tmpTrainGuiParam.m_ACFSearch1HideRect);

		m_GuiACFHand2SearchRect.SetLineColor(m_tmpTrainGuiParam.m_LineColor);
		m_GuiACFHand2SearchRect.SetLineWidth(m_tmpTrainGuiParam.m_nLineWidth);
		m_GuiACFHand2SearchRect.SetRect(m_tmpTrainGuiParam.m_ACFSearch2HideRect);
...
bool CMPatternPickCornerSearchTool::UpdateSearchGuiData(bool bSaveAndValidate)
{
	if (bSaveAndValidate)	// 从Gui控件获取数据
	{
...
// 清除控件及其显示					
bool CMPatternPickCornerSearchTool::ClearGraphicsArray(std::vector<scGuiGraphic*> &pGraphicsArray)
{
...
// 清除定位工具在Display上的控件
bool CMPatternPickCornerSearchTool::ClearAllGui()
{
	ClearTrainGui();
...
// 清除搜索结果，需先清除结果GUI
bool CMPatternPickCornerSearchTool::ClearResult()
{
	ClearResultGui();
...
bool CMPatternPickCornerSearchTool::SetGuiByCam(double nWidth,double nHeight)
{
	...
	//搜索区域
	m_GuiACFHand1SearchRect.GetCenterWidthHeight(centerx,centery,xlen,ylen);
	m_GuiACFHand1SearchRect.SetCenterWidthHeight((centerx/m_CamWidth)*nWidth, (centery/m_CamHeight)*nHeight, (xlen/m_CamWidth)*nWidth, (ylen/m_CamHeight)*nHeight);

	m_GuiACFHand2SearchRect.GetCenterWidthHeight(centerx,centery,xlen,ylen);
	m_GuiACFHand2SearchRect.SetCenterWidthHeight((centerx/m_CamWidth)*nWidth, (centery/m_CamHeight)*nHeight, (xlen/m_CamWidth)*nWidth, (ylen/m_CamHeight)*nHeight);
...
bool CMPatternPickCornerSearchTool::ClearTrainGui()
{
	if (m_pGuiDisplay && ::IsWindow(m_hDispWnd))
	{
		int j=0;
		...
		m_pGuiInteractiveContainer->RemoveItem(m_strKeyText+_T("_ACFHand1SearchRect"));		
		m_pGuiInteractiveContainer->RemoveItem(m_strKeyText+_T("_ACFHand2SearchRect"));
...
bool CMPatternPickCornerSearchTool::ClearResultGui()
{
	if (m_pGuiDisplay && ::IsWindow(m_hDispWnd))
	{
		int j=0;
...
// 获取训练模板原始图像
bool CMPatternPickCornerSearchTool::GetPatternImage(cpImage &ImagePattern)
{
...
// 恢复训练参数
bool CMPatternPickCornerSearchTool::RestoreTrainParam()
{ 
...
// 恢复搜索参数		
bool CMPatternPickCornerSearchTool::RestoreSearchParam()
{
...
void CMPatternPickCornerSearchTool::SetLanguage(int nLanguage)
{
	...
	case 0:
		...
		m_GuiSearch1HideRect.SetLabel(_T("Search1 Hide Area"));
		m_GuiSearch2HideRect.SetLabel(_T("Search2 Hide Area"));
		m_GuiACFHand1SearchRect.SetLabel(_T("acf"));
		m_GuiACFHand2SearchRect.SetLabel(_T("acf"));
	...
	case 1:
		...
		m_GuiSearch1HideRect.SetLabel(_T("手爪1 屏蔽区域"));
		m_GuiSearch2HideRect.SetLabel(_T("手爪2 屏蔽区域"));
		m_GuiACFHand1SearchRect.SetLabel(_T("ACF1检测区域"));
		m_GuiACFHand2SearchRect.SetLabel(_T("ACF2检测区域"));
		break;
	default:
		m_GuiTrainRect.SetLabel(_T("训练区域"));
		m_GuiSearchRect.SetLabel(_T("搜索区域"));
...
CString	CMPatternPickCornerSearchTool::GetResultErrInfo()
{
	CString strErrInfo;
...
BOOL CMPatternPickCornerSearchTool::PointInRect(sc2Vector point,scRect rect)
{
...
bool CMPatternPickCornerSearchTool::ACFSearch(cpImage &ImageSearch,std::vector<bool> &index)
{
	...
	for (int i=0;i<m_tmpTrainDataParam.m_nInsToolNum;i++)
	{
		scRect rect;
		if (i == 0)
		{
			rect= m_tmpTrainGuiParam.m_ACFSearch1HideRect;
		} 
		else
		{
			rect= m_tmpTrainGuiParam.m_ACFSearch2HideRect;
		}
		
		if(bIsProcessImage)
		{
			rect = rect.Intersect(scRect(0,0,tmpImage.Width(),tmpImage.Height()));
		}
		else
		{
		...
		else
		{
			...
			if (m_tmpTrainDataParam.m_bMeanInvalid)
			{
				if ((m_dCurMean.at(i)<m_tmpTrainDataParam.m_dCurMinMean) || (m_dCurMean.at(i)-0.001>m_tmpTrainDataParam.m_dCurMaxMean))
				{
					m_bmean=false;
				}
			}

			if (m_tmpTrainDataParam.m_bStdDevInvalid)
			{
				if ((m_dCurStdDev.at(i)<m_tmpTrainDataParam.m_dCurMinStdDev)||(m_dCurStdDev.at(i)-0.001>m_tmpTrainDataParam.m_dCurMaxStdDev))
				{
					m_bstddev=false;
				}
			}
			...
			if (m_tmpTrainDataParam.m_bMeanInvert && m_tmpTrainDataParam.m_bMeanInvalid)
			{
				m_bmean = !m_bmean;
			}

			if (m_tmpTrainDataParam.m_bStdDevInvert && m_tmpTrainDataParam.m_bStdDevInvalid)
			{
				m_bstddev = !m_bstddev;
			}
	...
	//scRect rect = *(m_tmpTrainGuiParam.m_SearchRect.at(0));
	...
	//	resultRect.m_bFindRect = bInspectResult.at(i);
	//	resultRect.m_searchRect.SetCornerPoLengthsRotAndSkew(rect.GetUL(), rect.GetWidth(), rect.GetHeight());
	...
	for (int i=0;i<bInspectResult.size();i++)
	{
		...
		if (i == 0)
		{
			pTempAffineRect->SetRect(m_tmpTrainGuiParam.m_ACFSearch1HideRect);
			//m_dCurMean;
			//m_dCurStdDev;
			//strLable.Format(_T("ACF检测_%d_亮度%f_均匀度%f"),i+1,m_dCurMean.at(i),m_dCurStdDev.at(i));
		} 
		else
		{
			pTempAffineRect->SetRect(m_tmpTrainGuiParam.m_ACFSearch2HideRect);
			//strLable.Format(_T("ACF检测_%d"), i+1);
		}
		strLable.Format(_T("ACF检测_%d_亮度%f_均匀度%f"),i+1,m_dCurMean.at(i),m_dCurStdDev.at(i));
