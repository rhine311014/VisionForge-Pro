...
class CDlgSearch;
class CDlgPatternMulCircleSearchTool : public CDialog
{
	DECLARE_DYNAMIC(CDlgPatternMulCircleSearchTool)

public:
	CDlgPatternMulCircleSearchTool(CWnd* pParent = NULL);   // 标准构造函数
	virtual ~CDlgPatternMulCircleSearchTool();

	// 对话框数据
	enum { IDD = IDD_DLG_PATTERNMULCIRCLESEARCH_TOOL };
...
public:
	...
	BOOL	 m_bTempEnableDualEdge[DLG_PATTERN_CIRCLE_NUM];
public:
