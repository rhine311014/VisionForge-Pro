...
// AreaInspect定位工具训练Data参数类
typedef struct tagAreaInspectTrainDataParam : public CBaseTrainDataParam
{
	int  	m_nDefectType;
	int		m_nBinaryThre;
...
// 		m_lStatisticACFAreaMin = LONG_MAX;
// 		m_lStatisticACFAreaMax = 0;
// 		m_lStatisticHoleAreaMax = 10;
// 		m_lStastisticNumber = 0;

		m_strPositionToolName = _T("");
...
}CAreaInspectTrainDataParam;

// AreaInspect定位工具训练Gui参数类
typedef struct tagAreaInspectTrainGuiParam : public CBaseTrainGuiParam
{
	scRect			m_SearchRect;						// 检测区域
	COLORREF        m_LineColor;						// 图形颜色
	int				m_nLineWidth;						// 图形线宽
	bool			m_bSearchRectVisible;				// 检测区域是否可见

    scAffineRect	m_TrainAffineRect;					// 训练区域
    scRect			m_BaseSearchRect;						// 搜索区域
    sc2Vector		m_ModelPoint;						// 参考点

    bool			m_bTrainAffineRectVisible;			// 训练区域是否可见
    bool			m_bModelPointVisible;				// 模板参考点是否可见
    bool			m_bBaseSearchRectVisible;			// 搜索区域是否可见

	tagAreaInspectTrainGuiParam()
	{
        ...
        m_bBaseSearchRectVisible		= true;
	}
	virtual CSearchToolType GetSearchToolType()
	{
		return eAreaInspectTool;
	}
}CAreaInspectTrainGuiParam;

// AreaInspect定位工具搜索Data参数类
typedef struct tagAreaInspectSearchDataParam : public CBaseSearchDataParam
{
	int		m_nSearchNum;								// 搜索个数
	double	m_dAcceptThre;								// 接收阈值
	double  m_dAngleLow;								// 低角度
	double	m_dAngleHigh;								// 高角度
	tagAreaInspectSearchDataParam()
	{
		m_nSearchNum	= 1;
		m_dAcceptThre	= 0.8;
		m_dAngleLow		= -5.0;
...
}CAreaInspectSearchDataParam;

// AreaInspect定位工具搜索Gui参数类
typedef struct tagAreaInspectSearchGuiParam : public CBaseSearchGuiParam
{
	COLORREF        m_LineColor;						// 图形颜色
	int				m_nLineWidth;						// 图形线宽
	bool			m_bAffineRectVisible;				// 搜索结果矩形可见
	bool			m_bModelPointVisible;				// 参考点是够可见
	tagAreaInspectSearchGuiParam()
	{
...
}CAreaInspectSearchGuiParam;

// AreaInspect训练结果
typedef struct tagAreaInspectTrainResult
{
	double m_dModelX; // 参考点X					
	double m_dModelY; // 参考点Y

	tagAreaInspectTrainResult()
	{
		m_dModelX = 0;
		m_dModelY = 0;
	}
...
}CAreaInspectTrainResult;

//
//	AreaInspect定位工具
//
class /*SV_SEARCHTOOL_API*/ CAreaInspectTool : public CBaseSearchTool  
{
public:
	CAreaInspectTool();
	CAreaInspectTool(CAreaInspectTool &cAreaInspectTool);
	CAreaInspectTool& operator=(CAreaInspectTool &cAreaInspectTool);
	virtual ~CAreaInspectTool();
public:
	bool	m_bInspectFinish;
	bool	m_bInspectOK;
    std::vector<bool> m_vbInspectOK;
...
private:
	...
	scQuickSearchTool                   m_AreaInspectTool;				// AreaInspect搜索工具
	...
	CAreaInspectTrainDataParam			m_tmpTrainDataParam;			// 当前训练Data参数
	CAreaInspectTrainGuiParam			m_tmpTrainGuiParam;				// 当前训练Gui参数	
	CAreaInspectSearchDataParam			m_tmpSearchDataParam;			// 当前搜索Data参数	
	CAreaInspectSearchGuiParam			m_tmpSearchGuiParam;			// 当前搜索Gui参数
	CAreaInspectTrainResult				m_tmpTrainResult;				// 当前训练结果
	cpImage							m_tmpImageTrain;				// 当前训练图像
	cpImage							m_tmpImageMask;					// 当前掩模图像
	CAreaInspectTrainDataParam			m_TrainDataParam;				// 训练Data参数
	CAreaInspectTrainGuiParam			m_TrainGuiParam;				// 训练Gui参数	
	CAreaInspectSearchDataParam			m_SearchDataParam;				// 搜索Data参数	
	CAreaInspectSearchGuiParam			m_SearchGuiParam;				// 搜索Gui参数
	CAreaInspectTrainResult				m_TrainResult;					// 训练结果
    ...
    scGuiRect							m_GuiBaseSearchRect;				// 搜索矩形
	scGuiCross                          m_GuiModelPoint;				// 参考点
	
	
	// 搜索结果GUI
	std::vector<scGuiGraphic*>          m_pGuiSearchResultRectArray;	// 搜索结果矩形
    std::vector<scGuiGraphic*>          m_pGuiSearchAffineRectArray;     //粗定位结果
    std::vector<scGuiGraphic*>          m_pGuiSearchCrossArray;
	scGuiRect							m_GuiSearchResultRect[MAXRESNUM];
