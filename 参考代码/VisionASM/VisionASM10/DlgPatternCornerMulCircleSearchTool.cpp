	...
	, m_nImageChannel(0)
{
	...
	for (int i = 0; i < DLG_PATTERN_LINE_NUM; i++)
	{
		...
		m_bTempEnableCircleDualEdge[i] = FALSE;
	}
...
void CDlgPatternCornerMulCircleSearchTool::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Check(pDX, IDC_CHECK_LINE_ENABLE_MASK, m_bEnableLineMask);
...
BEGIN_MESSAGE_MAP(CDlgPatternCornerMulCircleSearchTool, CDialog)
	...
	ON_BN_CLICKED(IDC_CHECK_CIRCLE_MASK, &CDlgPatternCornerMulCircleSearchTool::OnBnClickedCheckCircleMask)
...
void CDlgPatternCornerMulCircleSearchTool::UpdateTrainData(bool bValidate)
{
	...
	{
		UpdateData(TRUE);
		((CPatternCornerMulCircleSearchTrainDataParam*)m_pDlgParent->m_pTrainDataParam)->m_nImageChannel = m_nImageChannel;
		...
		m_bTempEnableLineMask[m_nLineIndex] = m_bEnableLineMask;
		m_bTempShowLineCaliper[m_nLineIndex] = m_bShowLineCaliper;
		m_bTempEnableLineDualEdge[m_nLineIndex] = m_bEnableLineDualEdge;
		...
		for (int i = 0; i < DLG_PATTERN_LINE_NUM; i++)
		{
			...
			((CPatternCornerMulCircleSearchTrainGuiParam*)m_pDlgParent->m_pTrainGuiParam)->m_bEnableLineMask[i]	= m_bTempEnableLineMask[i];
			...
			((CPatternCornerMulCircleSearchTrainDataParam*)m_pDlgParent->m_pTrainDataParam)->m_bEnableLineDualEdge[i] = m_bTempEnableLineDualEdge[i];
			((CPatternCornerMulCircleSearchTrainDataParam*)m_pDlgParent->m_pTrainDataParam)->m_lLineFilterHalfWidth[i] = m_lTempLineFilterHalfWidth[i];
		...
		m_bTempEnableCircleDualEdge[m_nCircleIndex]		= m_bEnableCircleDualEdge;
		...
		for (int i = 0; i < DLG_PATTERN_CIRCLE_NUM; i++)
		{
			...
			((CPatternCornerMulCircleSearchTrainDataParam*)m_pDlgParent->m_pTrainDataParam)->m_bEnableCircleDualEdge[i] = m_bTempEnableCircleDualEdge[i];
			((CPatternCornerMulCircleSearchTrainGuiParam*)m_pDlgParent->m_pTrainGuiParam)->m_bEnableCircleMask[i]		= m_bTempEnableCircleMask[i];
			((CPatternCornerMulCircleSearchTrainDataParam*)m_pDlgParent->m_pTrainDataParam)->m_lCircleFilterHalfWidth[i] = m_lTempCircleFilterHalfWidth[i];
		}
	...
	{		
		m_nImageChannel			= ((CPatternCornerMulCircleSearchTrainDataParam*)m_pDlgParent->m_pTrainDataParam)->m_nImageChannel;
		...
		for (int i = 0; i < DLG_PATTERN_LINE_NUM; i++)
		{
			...
			m_bTempEnableLineMask[i] =((CPatternCornerMulCircleSearchTrainGuiParam*)m_pDlgParent->m_pTrainGuiParam)->m_bEnableLineMask[i];
			...
			m_bTempEnableLineDualEdge[i] = ((CPatternCornerMulCircleSearchTrainDataParam*)m_pDlgParent->m_pTrainDataParam)->m_bEnableLineDualEdge[i];
			m_lTempLineFilterHalfWidth[i] = ((CPatternCornerMulCircleSearchTrainDataParam*)m_pDlgParent->m_pTrainDataParam)->m_lLineFilterHalfWidth[i];
		}
		...
		m_bEnableLineMask = m_bTempEnableLineMask[m_nLineIndex];
		m_bShowLineCaliper = m_bTempShowLineCaliper[m_nLineIndex];
		m_bEnableLineDualEdge = m_bTempEnableLineDualEdge[m_nLineIndex];
		m_checkLineEnableMask.SetCheck(m_bEnableLineMask);
		...
		for (int i = 0;i < DLG_PATTERN_CIRCLE_NUM; i++)
		{
			...
			m_bTempEnableCircleDualEdge[i]	= ((CPatternCornerMulCircleSearchTrainDataParam*)m_pDlgParent->m_pTrainDataParam)->m_bEnableCircleDualEdge[i];
			m_bTempEnableCircleMask[i]		= ((CPatternCornerMulCircleSearchTrainGuiParam*)m_pDlgParent->m_pTrainGuiParam)->m_bEnableCircleMask[i];
			m_lTempCircleFilterHalfWidth[i]	= ((CPatternCornerMulCircleSearchTrainDataParam*)m_pDlgParent->m_pTrainDataParam)->m_lCircleFilterHalfWidth[i];
		}
		...
		m_bEnableCircleDualEdge	= m_bTempEnableCircleDualEdge[m_nCircleIndex];
...
void CDlgPatternCornerMulCircleSearchTool::OnBnClickedRadioCircleIndex1()
{
	...
	m_bTempEnableCircleDualEdge[m_nPreCircleIndex]	= m_bEnableCircleDualEdge;
...
void CDlgPatternCornerMulCircleSearchTool::OnBnClickedRadioLineIndex1()
{
	...
	m_bTempEnableLineDualEdge[m_nPreLineIndex]	= m_bEnableLineDualEdge;
	m_bTempEnableLineMask[m_nPreLineIndex]		= m_bEnableLineMask;
	...
	m_bEnableShowLinePoint	    = m_bTempEnableShowLinePoint[m_nLineIndex];
	m_bShowLineCaliper			= m_bTempShowLineCaliper[m_nLineIndex];
	m_bEnableLineDualEdge		= m_bTempEnableCircleDualEdge[m_nLineIndex];
	m_bEnableLineMask			= m_bTempEnableLineMask[m_nLineIndex];
	m_lLineFilterHalfWidth		= m_lTempLineFilterHalfWidth[m_nLineIndex];
	m_nPreLineIndex = m_nLineIndex;
...
void CDlgPatternCornerMulCircleSearchTool::OnBnClickedCheckCircleMask()
{
	// TODO: 在此添加控件通知处理程序代码
	UpdateTrainData(true);
}
...
BOOL CDlgPatternCornerMulCircleSearchTool::PreTranslateMessage(MSG* pMsg)
{
...
void CDlgPatternCornerMulCircleSearchTool::OnEnSetfocusEditLineFilterHalfWidth() 
{
...
void CDlgPatternCornerMulCircleSearchTool::OnEnSetfocusEditCircleFilterHalfWidth() 
{
	// TODO: If this is a RICHEDIT control, the control will not
	// send this notification unless you override the CDialog::OnInitDialog()
	// function and call CRichEditCtrl().SetEventMask()
