...
IMPLEMENT_DYNAMIC(CDlgManualSearchPos1, CDialog)

CDlgManualSearchPos1::CDlgManualSearchPos1(CWnd* pParent /*=NULL*/)
	: CDialog(CDlgManualSearchPos1::IDD, pParent)
{
...
CDlgManualSearchPos1::~CDlgManualSearchPos1()
{
}

void CDlgManualSearchPos1::DoDataExchange(CDataExchange* pDX)
{
	CDialog::DoDataExchange(pDX);
}


BEGIN_MESSAGE_MAP(CDlgManualSearchPos1, CDialog)
	ON_WM_DESTROY()
	ON_MESSAGE(WM_SV_DISPLAY_CLICK,OnGuiDisplayClick)
	ON_BN_CLICKED(IDOK, &CDlgManualSearchPos1::OnBnClickedOk)
END_MESSAGE_MAP()


// CDlgManualSearchPos1 消息处理程序


BOOL CDlgManualSearchPos1::OnInitDialog()
{
	...
	if (m_nMarkType == 0)
	{
		...
		strTitle.Format(GetSysInfoString(m_psaSysInfoStrings,IDS_STR_SELECT_MARK_OBJECT_POSITION),m_nCurPlatformIndex+1,m_vnPosIndex.at(0)+1);
	//	strTitle.Format("手动选择Mark点---工位%d对象位置%d",m_nCurPlatformIndex+1,m_vnPosIndex.at(0)+1);
	}
	else if (m_nMarkType == 1)
	{
		strTitle.Format(GetSysInfoString(m_psaSysInfoStrings,IDS_STR_SELECT_MARK_TARGET_POSITION),m_nCurPlatformIndex+1,m_vnPosIndex.at(0)+1);
	//	strTitle.Format("手动选择Mark点---工位%d目标位置%d",m_nCurPlatformIndex+1,m_vnPosIndex.at(0)+1);
	}
	else
	{
		strTitle.Format(GetSysInfoString(m_psaSysInfoStrings,IDS_STR_SELECT_MARK_POSITION),m_nCurPlatformIndex+1,m_vnPosIndex.at(0)+1);
	//	strTitle.Format("手动选择Mark点---工位%d位置%d",m_nCurPlatformIndex+1,m_vnPosIndex.at(0)+1);
	}
...
void CDlgManualSearchPos1::OnDestroy()
{
	CDialog::OnDestroy();
...
LRESULT CDlgManualSearchPos1::WindowProc(UINT message, WPARAM wParam, LPARAM lParam)
{
...
void CDlgManualSearchPos1::InitDlgItem()
{
	int nPosNum = 1;
...
void CDlgManualSearchPos1::SetVisionAlignerRef(vcBaseVisionAlign *pVisionAligner, std::vector<int> vnPosIndex)
{
	if ( (pVisionAligner == NULL) || (vnPosIndex.size() != 1) )
	{
		return;
	}
...
BOOL CDlgManualSearchPos1::PreTranslateMessage(MSG* pMsg)
{
	// TODO: 在此添加专用代码和/或调用基类
...
LRESULT CDlgManualSearchPos1::OnGuiDisplayClick(WPARAM wParam, LPARAM lParam)
{
...
void CDlgManualSearchPos1::OnBnClickedOk()
{
	// TODO: 在此添加控件通知处理程序代码
...
void CDlgManualSearchPos1::UpdateDialogLanguage()
{
	//	return;
	...
	while(xml.FindElem(_T("")))
	{
		...
		if(_T("IDD_DLG_MANUAL_SEARCH_POS1") == strDlgID)
		{
			int nID;
