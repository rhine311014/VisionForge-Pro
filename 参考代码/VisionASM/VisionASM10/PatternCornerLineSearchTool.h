// PatternCornerLineSearchTool.h: interface for the CPatternCornerLineSearchTool class.
//
//////////////////////////////////////////////////////////////////////
...
#define PCLT_LINE_NUM 3


// PatternCornerLineSearch定位工具训练Data参数类
typedef struct tagPatternCornerLineSearchTrainDataParam : public CBaseTrainDataParam
{
	...
	double			m_dContrastThre[PCLT_LINE_NUM] ;			// 对比度阈值
	long			m_lFilterHalfWidth[PCLT_LINE_NUM];		// 滤波器半宽
	int				m_nOutLineNum[PCLT_LINE_NUM];			// 局外点
	EdgePolarity	m_edgeProperty[PCLT_LINE_NUM];			// 边缘属性
	bool			m_bSortByScore[PCLT_LINE_NUM];			// 按分数排序
	BOOL			m_bEnableDualEdge[PCLT_LINE_NUM];		// 启用/禁用找线工具双边缘

	int             m_nLineAngleType;           // 找线角度类型

	tagPatternCornerLineSearchTrainDataParam()
	{
...
}CPatternCornerLineSearchTrainDataParam;

// PatternCornerLineSearch定位工具训练Gui参数类
typedef struct tagPatternCornerLineSearchTrainGuiParam : public CBaseTrainGuiParam
{
	...
	// 找线工具1
	int				m_nCaliperNum[PCLT_LINE_NUM];			// 卡尺个数
	double			m_dStartX[PCLT_LINE_NUM];				// 起始点X
	double			m_dStartY[PCLT_LINE_NUM];				// 起始点Y
	double			m_dEndX[PCLT_LINE_NUM];					// 终止点X
	double			m_dEndY[PCLT_LINE_NUM];					// 终止点Y
	double			m_dCaliperProjectLen[PCLT_LINE_NUM];		// 投影长度
	double			m_dCaliperSearchLen[PCLT_LINE_NUM];		// 搜索长度
	scRadian		m_CaliperSearchRotation[PCLT_LINE_NUM];	// 搜索角度
	bool			m_bLineCaliperVisible[PCLT_LINE_NUM];		//找线卡尺是否可见	
	...
	// 找线工具掩摸
	svStd vector<double> m_vdCaliperMaskData[PCLT_LINE_NUM];			// 掩膜数据
	svStd vector<bool>   m_vbCaliperMask[PCLT_LINE_NUM];				// 卡尺是否被掩膜
	bool			     m_bEnableLineMask[PCLT_LINE_NUM];			// 是否启用掩膜
	int				m_nLineAngleType[PATTERN_LINE_NUM];			// 直线角度类型：0,任意角度；1,水平线；2,垂直线；
	...
	tagPatternCornerLineSearchTrainGuiParam()
	{
...
}CPatternCornerLineSearchTrainGuiParam;

// PatternnCornerSearch定位工具搜索Data参数类
typedef struct tagPatternCornerLineSearchSearchDataParam : public CBaseSearchDataParam
{
	...
	tagPatternCornerLineSearchSearchDataParam()
	{
		m_nSearchNum	= 1;
		m_dAcceptThre	= 0.6;
...
}CPatternCornerLineSearchSearchDataParam;

// PatternCornerLineSearch定位工具搜索Gui参数类
typedef struct tagPatternCornerLineSearchGuiParam : public CBaseSearchGuiParam
{
	COLORREF        m_LineColor;						// 图形颜色
	int				m_nLineWidth;						// 图形线宽
	...
	bool			m_bEnableShowLinePoint[PCLT_LINE_NUM];				// 拟合数据点是否可见

	COLORREF        m_cLineColorOKFitPoint;			// 拟合数据点OK颜色
	COLORREF        m_cLineColorNGFitPoint;			// 拟合数据点NG颜色


	tagPatternCornerLineSearchGuiParam()
	{
...
}CPatternCornerLineSearchSearchGuiParam;

// PatternSearch训练结果
typedef struct tagPatternCornerLineSearchTrainResult
{
	double m_dModelX; // 参考点X					
	double m_dModelY; // 参考点Y

	double m_dStartX[PCLT_LINE_NUM]; // 线段起点X
	double m_dStartY[PCLT_LINE_NUM]; // 线段起点Y
	double m_dEndX[PCLT_LINE_NUM];	  // 线段终点X
	double m_dEndY[PCLT_LINE_NUM];	  // 线段终点Y

	tagPatternCornerLineSearchTrainResult()
	{
		m_dModelX = 0;
		m_dModelY = 0;
...
}CPatternCornerLineSearchTrainResult;

class CPatternCornerLineSearchTool :public CBaseSearchTool
{
public:
	CPatternCornerLineSearchTool(void);
	
	CPatternCornerLineSearchTool(CPatternCornerLineSearchTool& cPatternCornerLineSearchTool);
	CPatternCornerLineSearchTool& operator=(CPatternCornerLineSearchTool &cPatternCornerLineSearchTool);
	~CPatternCornerLineSearchTool(void);
...
private:
	...
	scFindLine							m_FindLineTool[PCLT_LINE_NUM];		// 找线工具
	...
	CPatternCornerLineSearchTrainDataParam		m_tmpTrainDataParam;			// 当前训练Data参数
	CPatternCornerLineSearchTrainGuiParam			m_tmpTrainGuiParam;				// 当前训练Gui参数	
	CPatternCornerLineSearchSearchDataParam		m_tmpSearchDataParam;			// 当前搜索Data参数	
	CPatternCornerLineSearchSearchGuiParam		m_tmpSearchGuiParam;			// 当前搜索Gui参数
	CPatternCornerLineSearchTrainResult			m_tmpTrainResult;				// 当前训练结果
	cpImage							m_tmpImageTrain;				// 当前训练图像
	cpImage							m_tmpImageMask;					// 当前掩模图像
	CPatternCornerLineSearchTrainDataParam		m_TrainDataParam;				// 训练Data参数
	CPatternCornerLineSearchTrainGuiParam			m_TrainGuiParam;				// 训练Gui参数	
	CPatternCornerLineSearchSearchDataParam		m_SearchDataParam;				// 搜索Data参数	
	CPatternCornerLineSearchSearchGuiParam		m_SearchGuiParam;				// 搜索Gui参数	
	CPatternCornerLineSearchTrainResult			m_TrainResult;					// 训练结果
	...
	scGuiFindLine                       m_GuiFindLine[PCLT_LINE_NUM];		// 两个找线工具
	scGuiLine                           m_GuiTrainResultLine[PCLT_LINE_NUM];	// 训练结果线段
