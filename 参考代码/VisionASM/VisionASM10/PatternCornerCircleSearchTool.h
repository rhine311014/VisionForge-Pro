// PatternCornerCircleSearchTool.h: interface for the CPatternCornerSearchTool class.
//
//////////////////////////////////////////////////////////////////////
...
// PatternCornerCircleSearch定位工具训练Data参数类
typedef struct tagPatternCornerCircleSearchTrainDataParam : public CBaseTrainDataParam
{
	...
	EdgePolarity	m_LineedgeProperty[LINE_NUM];			// 边缘属性
	bool			m_bLineSortByScore[LINE_NUM];			// 按分数排序
	int				m_nLineAngleType[LINE_NUM];				// 直线角度类型：0,任意角度；1,水平线；2,垂直线；
	...
	tagPatternCornerCircleSearchTrainDataParam()
	{
		...
		{
			m_dLineContrastThre[i]		= 1.0;
			m_lLineFilterHalfWidth[i]	= 1;
			m_nLineOutLineNum[i]		= 0;
			m_LineedgeProperty[i]		= eDontCare;
...
}CPatternCornerCircleSearchTrainDataParam;

// PatternCornerCircleSearch定位工具训练Gui参数类
typedef struct tagPatternCornerCircleSearchTrainGuiParam : public CBaseTrainGuiParam
{
	...
	tagPatternCornerCircleSearchTrainGuiParam()
	{
		m_nLineCaliperNum[0]				= 2;
		...
		m_nLineCaliperNum[1]				= 2;
		m_dLineStartX[1]					= 100.0;
		m_dLineStartY[1]					= 100.0;
		m_dLineEndX[1]						= 100.0;
		m_dLineEndY[1]						= 100.0;
		m_dLineCaliperProjectLen[1]			= 20.0;
		m_dLineCaliperSearchLen[1]			= 100.0;
		m_LineCaliperSearchRotation[1]		= scRadian(skPI/2.0);
...
}CPatternCornerCircleSearchTrainGuiParam;

// PatternCornerCircleSearch定位工具搜索Data参数类
typedef struct tagPatternCornerCircleSearchSearchDataParam : public CBaseSearchDataParam
{
	int		m_nSearchNum;								// 搜索个数
	double	m_dAcceptThre;								// 接收阈值
	double  m_dConfusionThre;							// 混淆阈值。混淆阈值不小于接收阈值

	tagPatternCornerCircleSearchSearchDataParam()
	{
		m_nSearchNum	= 1;
		m_dAcceptThre	= 0.6;
...
}CPatternCornerCircleSearchSearchDataParam;

// PatternCornerCircleSearch定位工具搜索Gui参数类
typedef struct tagPatternCornerCircleSearchGuiParam : public CBaseSearchGuiParam
{
	COLORREF        m_LineColor;						// 图形颜色
	int				m_nLineWidth;						// 图形线宽
	...
	bool			m_bCircleVisible;					// 找线工具是否可见
	bool			m_bCircleCenterVisible;				// 找线工具是否可见
	...
	tagPatternCornerCircleSearchGuiParam()
	{
...
}CPatternCornerCircleSearchSearchGuiParam;

// PatternCornerCircleSearch训练结果
typedef struct tagPatternCornerCircleSearchTrainResult
{
	double m_dModelX; // 参考点X					
	double m_dModelY; // 参考点Y

	double m_dLineStartX[LINE_NUM]; // 线段起点X
	double m_dLineStartY[LINE_NUM]; // 线段起点Y
	double m_dLineEndX[LINE_NUM];	  // 线段终点X
	double m_dLineEndY[LINE_NUM];	  // 线段终点Y

	double m_dCircleCenterX;
	double m_dCircleCenterY;
	double m_dCircleRadius;

	tagPatternCornerCircleSearchTrainResult()
	{
		m_dModelX = 0;
		m_dModelY = 0;
...
}CPatternCornerCircleSearchTrainResult;

class CPatternCornerCircleSearchTool :public CBaseSearchTool
{
public:
	CPatternCornerCircleSearchTool(void);
	CPatternCornerCircleSearchTool(CPatternCornerCircleSearchTool &cPatternCornerCircleSearchTool);
	CPatternCornerCircleSearchTool& operator=(CPatternCornerCircleSearchTool &cPatternCornerCircleSearchTool);
	~CPatternCornerCircleSearchTool(void);
...
private:
	...
	CPatternCornerCircleSearchTrainDataParam		m_tmpTrainDataParam;			// 当前训练Data参数
	CPatternCornerCircleSearchTrainGuiParam			m_tmpTrainGuiParam;				// 当前训练Gui参数	
	CPatternCornerCircleSearchSearchDataParam		m_tmpSearchDataParam;			// 当前搜索Data参数	
	CPatternCornerCircleSearchSearchGuiParam		m_tmpSearchGuiParam;			// 当前搜索Gui参数
	CPatternCornerCircleSearchTrainResult			m_tmpTrainResult;				// 当前训练结果
	cpImage							m_tmpImageTrain;				// 当前训练图像
	cpImage							m_tmpImageMask;					// 当前掩模图像
	CPatternCornerCircleSearchTrainDataParam		m_TrainDataParam;				// 训练Data参数
	CPatternCornerCircleSearchTrainGuiParam			m_TrainGuiParam;				// 训练Gui参数	
	CPatternCornerCircleSearchSearchDataParam		m_SearchDataParam;				// 搜索Data参数	
	CPatternCornerCircleSearchSearchGuiParam		m_SearchGuiParam;				// 搜索Gui参数	
	CPatternCornerCircleSearchTrainResult			m_TrainResult;					// 训练结果
	...
	std::vector<scGuiGraphic*>          m_pGuiSearchResultCircleFitPointArray;
