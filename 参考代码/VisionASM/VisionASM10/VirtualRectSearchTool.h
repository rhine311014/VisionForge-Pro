// RectSearchTool.h: interface for the CVirtualRectSearchTool class.
//
//////////////////////////////////////////////////////////////////////
...
// RectSearch定位工具训练Data参数类
typedef struct tagVirtualRectSearchTrainDataParam : public CBaseTrainDataParam
{
	double			m_dGrau;							// 颗粒度
	...
	bool			m_bEnableDualEdge[RECT_LINE_NUM];		// 是否启用双极性找线

	tagVirtualRectSearchTrainDataParam()
	{
		m_dGrau			= 2.0;
		m_lNoiseThre	= 10;
		m_lTotalMag		= 200;
...
}CVirtualRectSearchTrainDataParam;

// RectSearch定位工具训练Gui参数类
typedef struct tagVirtualRectSearchTrainGuiParam : public CBaseTrainGuiParam
{
	...
	tagVirtualRectSearchTrainGuiParam()
	{
		m_nCaliperNum[0]				= 2;
		m_dStartX[0]					= 100.0;
		m_dStartY[0]					= 100.0;
...
}CVirtualRectSearchTrainGuiParam;

// RectSearch定位工具搜索Data参数类
typedef struct tagVirtualRectSearchSearchDataParam : public CBaseSearchDataParam
{
	...
	int		m_nRiviseMode;								//修正模式


	tagVirtualRectSearchSearchDataParam()
	{
		m_nSearchNum	= 1;
		m_dAcceptThre	= 0.8;
		m_dAngleLow		= -5.0;
...
}CVirtualRectSearchSearchDataParam;

// RectSearch定位工具搜索Gui参数类
typedef struct tagVirtualRectSearchGuiParam : public CBaseSearchGuiParam
{
	COLORREF        m_LineColor;						// 图形颜色
	int				m_nLineWidth;						// 图形线宽
	...
	int				m_nResultOutMode;					// 选择矩形输出点的位置

	tagVirtualRectSearchGuiParam()
	{
...
}CVirtualRectSearchSearchGuiParam;

// RectSearch训练结果
typedef struct tagVirtualRectSearchTrainResult
{
	...
	tagVirtualRectSearchTrainResult()
	{
		m_dModelX = 0;
		m_dModelY = 0;
		for (int i=0; i<RECT_LINE_NUM; i++)
...
}CVirtualRectSearchTrainResult;


//
//	角定位工具
//
class /*SV_SEARCHTOOL_API*/ CVirtualRectSearchTool : public CBaseSearchTool    
{
public:
	CVirtualRectSearchTool();
	CVirtualRectSearchTool(CVirtualRectSearchTool &CVirtualRectSearchTool);
	CVirtualRectSearchTool& operator=(CVirtualRectSearchTool &CVirtualRectSearchTool);
	virtual ~CVirtualRectSearchTool();
...
private:
	...
	bool GetFindLineCenterPoint(scFindLine* m_FindLineTool,sc2Vector &pointCenter2);			//获取找线中心点
private:	
	...
	CVirtualRectSearchTrainDataParam			m_tmpTrainDataParam;			// 当前训练Data参数
	CVirtualRectSearchTrainGuiParam			m_tmpTrainGuiParam;				// 当前训练Gui参数	
	CVirtualRectSearchSearchDataParam		m_tmpSearchDataParam;			// 当前搜索Data参数	
	CVirtualRectSearchSearchGuiParam			m_tmpSearchGuiParam;			// 当前搜索Gui参数
	CVirtualRectSearchTrainResult			m_tmpTrainResult;				// 当前训练结果
	cpImage							m_tmpImageTrain;				// 当前训练图像
	cpImage							m_tmpImageMask;					// 当前掩模图像
	CVirtualRectSearchTrainDataParam			m_TrainDataParam;				// 训练Data参数
	CVirtualRectSearchTrainGuiParam			m_TrainGuiParam;				// 训练Gui参数	
	CVirtualRectSearchSearchDataParam		m_SearchDataParam;				// 搜索Data参数	
	CVirtualRectSearchSearchGuiParam			m_SearchGuiParam;				// 搜索Gui参数
	CVirtualRectSearchTrainResult			m_TrainResult;					// 训练结果
