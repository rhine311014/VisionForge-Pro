// PatternSearchGenRectTool.h: interface for the CPatternSearchGenRectTool class.
//
//////////////////////////////////////////////////////////////////////
...
#include "svGuiFindGenRect.h"
#include "svGuiGenRect.h"
#include "svGenRect.h"
#include "svFindGenRect.h"
#pragma pack(push,8)

// PatternSearchGenRect定位工具训练Data参数类
typedef struct tagPatternSearchGenRectTrainDataParam : public CBaseTrainDataParam
{
	...
	tagPatternSearchGenRectTrainDataParam()
	{
...
}CPatternSearchGenRectTrainDataParam;

// PatternSearchGenRect定位工具训练Gui参数类
typedef struct tagPatternSearchGenRectTrainGuiParam : public CBaseTrainGuiParam
{
	scRect			m_TrainRect;						// 训练区域
	scRect			m_SearchRect;						// 搜索区域
	sc2Vector		m_ModelPoint;						// 参考点

	scGenRect		m_GenRect;							// 圆角矩形
	...
	bool			m_bGenRectVisible;					// 找圆工具是否可见
	tagPatternSearchGenRectTrainGuiParam()
	{
		m_GenRect = scGenRect(sc2Vector(200,200),100,100,scRadian(0));
		...
		m_bGenRectVisible			= true;
	}
	virtual CSearchToolType GetSearchToolType()
	{
		return ePatternSearchGenRectTool;
	}
}CPatternSearchGenRectTrainGuiParam;

// PatternSearchGenRect定位工具搜索Data参数类
typedef struct tagPatternSearchGenRectSearchDataParam : public CBaseSearchDataParam
{
	int		m_nSearchNum;								// 搜索个数
	double	m_dAcceptThre;								// 接收阈值
	double  m_dConfusionThre;							// 混淆阈值。混淆阈值不小于接收阈值
	int		m_nMethod;									// 定位方法
	tagPatternSearchGenRectSearchDataParam()
	{
		m_nSearchNum	= 1;
		m_dAcceptThre	= 0.6;
...
}CPatternSearchGenRectSearchDataParam;

// PatternSearchGenRect定位工具搜索Gui参数类
typedef struct tagPatternSearchGenRectGuiParam : public CBaseSearchGuiParam
{
	COLORREF        m_LineColor;						// 图形颜色
	int				m_nLineWidth;						// 图形线宽
	bool			m_bAffineRectVisible;				// 搜索结果矩形可见
	bool			m_bModelPointVisible;				// 参考点是否可见
	bool			m_bGenRectVisible;					// 圆形是否可见
	bool			m_bGenRectCenterVisible;			// 圆心是否可见
	tagPatternSearchGenRectGuiParam()
	{
...
}CPatternSearchGenRectSearchGuiParam;

// PatternSearch训练结果
typedef struct tagPatternSearchGenRectTrainResult
{
	double m_dModelX;
	double m_dModelY;
	double m_dCenterX;
	double m_dCenterY;
	tagPatternSearchGenRectTrainResult()
	{
		m_dModelX = 0;
		m_dModelY = 0;
		m_dCenterX = 0;
...
}CPatternSearchGenRectTrainResult;

//
//	圆定位工具
//
class /*SV_SEARCHTOOL_API*/ CPatternSearchGenRectTool : public CBaseSearchTool    
{
public:
	CPatternSearchGenRectTool();
	CPatternSearchGenRectTool(CPatternSearchGenRectTool &cPatternSearchGenRectTool);
	CPatternSearchGenRectTool& operator=(CPatternSearchGenRectTool &cPatternSearchGenRectTool);
	virtual ~CPatternSearchGenRectTool();
...
private:	
	...
	scFindGenRect						m_FindGenRectTool;				// 找线工具
	...
	CPatternSearchGenRectTrainDataParam			m_tmpTrainDataParam;			// 当前训练Data参数
	CPatternSearchGenRectTrainGuiParam			m_tmpTrainGuiParam;				// 当前训练Gui参数	
	CPatternSearchGenRectSearchDataParam		m_tmpSearchDataParam;			// 当前搜索Data参数	
	CPatternSearchGenRectSearchGuiParam			m_tmpSearchGuiParam;			// 当前搜索Gui参数
	CPatternSearchGenRectTrainResult			m_tmpTrainResult;				// 当前训练结果
	cpImage							m_tmpImageTrain;				// 当前训练图像
	cpImage							m_tmpImageMask;					// 当前掩模图像
 	CPatternSearchGenRectTrainDataParam			m_TrainDataParam;				// 训练Data参数
 	CPatternSearchGenRectTrainGuiParam			m_TrainGuiParam;				// 训练Gui参数	
 	CPatternSearchGenRectSearchDataParam		m_SearchDataParam;				// 搜索Data参数	
 	CPatternSearchGenRectSearchGuiParam			m_SearchGuiParam;				// 搜索Gui参数
	CPatternSearchGenRectTrainResult			m_TrainResult;					// 训练结果
	...
	scGuiFindGenRect                    m_GuiFindGenRect;				// 找圆工具
	scGuiGenRect                        m_GuiTrainResultGenRect;         // 训练结果圆
	...
	std::vector<scGuiGraphic*>          m_pGuiSearchGenRectArray;		// 搜索结果直线
	std::vector<scGuiGraphic*>          m_pGuiSearchGenRectCenterArray;	// 搜索结果直线

	scGuiAffineRect						m_GuiSearchAffineRect[MAXRESNUM];
	scGuiCross							m_GuiSearchCross[MAXRESNUM];
	scGuiGenRect						m_GuiSearchGenRect[MAXRESNUM];
	scGuiCross							m_GuiSearchGenRectCenter[MAXRESNUM];
