...
// AreaInspectEx定位工具训练Data参数类
typedef struct tagAreaInspectExTrainDataParam : public CBaseTrainDataParam
{
	...
	tagAreaInspectExTrainDataParam()
	{
...
}CAreaInspectExTrainDataParam;

// AreaInspectEx定位工具训练Gui参数类
typedef struct tagAreaInspectExTrainGuiParam : public CBaseTrainGuiParam
{
	...
	tagAreaInspectExTrainGuiParam& operator=(tagAreaInspectExTrainGuiParam& cAreaInspectToolExTrainGuiParam)
	{
		for (int i=0;i<m_SearchRect.size();i++)
		{
			if (m_SearchRect.at(i)!=NULL)
		...
		for(int i=0;i<cAreaInspectToolExTrainGuiParam.m_SearchRect.size();i++)
		{
			scRect* tempRect = new scRect;
			if(cAreaInspectToolExTrainGuiParam.m_SearchRect[i])
			{
				*tempRect = *cAreaInspectToolExTrainGuiParam.m_SearchRect[i];
			}else
			{
		...
		m_LineColor = cAreaInspectToolExTrainGuiParam.m_LineColor;						// 图形颜色
		m_nLineWidth = cAreaInspectToolExTrainGuiParam.m_nLineWidth;					// 图形线宽
		m_bSearchRectVisible = cAreaInspectToolExTrainGuiParam.m_bSearchRectVisible;	// 检测区域是否可见
		return *this;
	}
...
}CAreaInspectExTrainGuiParam;

// AreaInspectEx定位工具搜索Data参数类
typedef struct tagAreaInspectExSearchDataParam : public CBaseSearchDataParam
{
	int		m_nSearchNum;								// 搜索个数
	double	m_dAcceptThre;								// 接收阈值
	double  m_dAngleLow;								// 低角度
	double	m_dAngleHigh;								// 高角度
	tagAreaInspectExSearchDataParam()
	{
		m_nSearchNum	= 1;
		m_dAcceptThre	= 0.8;
		m_dAngleLow		= -5.0;
...
}CAreaInspectExSearchDataParam;

// AreaInspectEx定位工具搜索Gui参数类
typedef struct tagAreaInspectExSearchGuiParam : public CBaseSearchGuiParam
{
	std::vector<COLORREF>        m_LineColor;						// 图形颜色
	std::vector<int>				m_nLineWidth;						// 图形线宽
	std::vector<BOOL>			m_bAffineRectVisible;				// 搜索结果矩形可见
	std::vector<BOOL>			m_bModelPointVisible;				// 参考点是够可见
	tagAreaInspectExSearchGuiParam()
	{
		for (int i=0;i<MAXTOOLNUM;i++)
		{
			m_LineColor.push_back(RGB(0, 0, 255));
...
}CAreaInspectExSearchGuiParam;

// AreaInspectEx训练结果
typedef struct tagAreaInspectExTrainResult
{
	double m_dModelX; // 参考点X					
	double m_dModelY; // 参考点Y

	tagAreaInspectExTrainResult()
	{
		m_dModelX = 0;
		m_dModelY = 0;
	}
...
}CAreaInspectExTrainResult;

//
//	AreaInspectEx定位工具
//
class /*SV_SEARCHTOOL_API*/ CAreaInspectToolEx : public CBaseSearchTool  
{
public:
	CAreaInspectToolEx();
	CAreaInspectToolEx(CAreaInspectToolEx &cAreaInspectToolEx);
	CAreaInspectToolEx& operator=(CAreaInspectToolEx &cAreaInspectToolEx);
	virtual ~CAreaInspectToolEx();
public:
...
private:
	...
	scQuickSearchTool                   m_AreaInspectTool;				// AreaInspectEx搜索工具
	...
	CAreaInspectExTrainDataParam			m_tmpTrainDataParam;			// 当前训练Data参数
	CAreaInspectExTrainGuiParam			m_tmpTrainGuiParam;				// 当前训练Gui参数	
	CAreaInspectExSearchDataParam			m_tmpSearchDataParam;			// 当前搜索Data参数	
	CAreaInspectExSearchGuiParam			m_tmpSearchGuiParam;			// 当前搜索Gui参数
	CAreaInspectExTrainResult				m_tmpTrainResult;				// 当前训练结果
	cpImage							m_tmpImageTrain;				// 当前训练图像
	cpImage							m_tmpImageMask;					// 当前掩模图像
	CAreaInspectExTrainDataParam			m_TrainDataParam;				// 训练Data参数
	CAreaInspectExTrainGuiParam			m_TrainGuiParam;				// 训练Gui参数	
	CAreaInspectExSearchDataParam			m_SearchDataParam;				// 搜索Data参数	
	CAreaInspectExSearchGuiParam			m_SearchGuiParam;				// 搜索Gui参数
	CAreaInspectExTrainResult				m_TrainResult;					// 训练结果
