// DlgProCircleSearchToolSetting.cpp : implementation file
//
...
CDlgProCircleSearchTool::CDlgProCircleSearchTool(CWnd* pParent /*=NULL*/)
	: CDialog(CDlgProCircleSearchTool::IDD, pParent)
	, m_nImageChannel(0)
{
	//{{AFX_DATA_INIT(CDlgProCircleSearchTool)
	m_nSearchNum = 1;
	m_dAcceptThre = 0.6;
	m_lNoiseThre = 10;
	m_lTotalMag = 500;
...
void CDlgProCircleSearchTool::DoDataExchange(CDataExchange* pDX)
{
	CDialog::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(CDlgProCircleSearchTool)
...
BEGIN_MESSAGE_MAP(CDlgProCircleSearchTool, CDialog)
	//{{AFX_MSG_MAP(CDlgProCircleSearchTool)
...
/////////////////////////////////////////////////////////////////////////////
// CDlgProCircleSearchTool message handlers

void CDlgProCircleSearchTool::UpdateControlDisplay()
{	
	UpdateData(TRUE);
...
void CDlgProCircleSearchTool::UpdateTrainData(bool bValidate)
{
	...
	{
		UpdateData(TRUE);
		((CProCircleSearchTrainDataParam*)m_pDlgParent->m_pTrainDataParam)->m_nImageChannel = m_nImageChannel; 
		((CProCircleSearchTrainDataParam*)m_pDlgParent->m_pTrainDataParam)->m_dGrau = m_dGrau; 
		((CProCircleSearchTrainDataParam*)m_pDlgParent->m_pTrainDataParam)->m_lNoiseThre = m_lNoiseThre;
		((CProCircleSearchTrainDataParam*)m_pDlgParent->m_pTrainDataParam)->m_lTotalMag = m_lTotalMag;
		((CProCircleSearchTrainDataParam*)m_pDlgParent->m_pTrainDataParam)->m_bMaskEnable = m_bEnableMask/*m_imageMask.IsValid() ? m_bEnableMask : false*/;
		((CProCircleSearchTrainDataParam*)m_pDlgParent->m_pTrainDataParam)->m_edgeProperty = (EdgePolarity)m_nCirclePolarityMode;
	...
	{
		m_nImageChannel	= ((CProCircleSearchTrainDataParam*)m_pDlgParent->m_pTrainDataParam)->m_nImageChannel;
		m_dGrau			= ((CProCircleSearchTrainDataParam*)m_pDlgParent->m_pTrainDataParam)->m_dGrau;
		m_lNoiseThre	= ((CProCircleSearchTrainDataParam*)m_pDlgParent->m_pTrainDataParam)->m_lNoiseThre;
		m_lTotalMag		= ((CProCircleSearchTrainDataParam*)m_pDlgParent->m_pTrainDataParam)->m_lTotalMag;
		m_nCirclePolarityMode = ((CProCircleSearchTrainDataParam*)m_pDlgParent->m_pTrainDataParam)->m_edgeProperty;

		m_bEnableMask	= ((CProCircleSearchTrainDataParam*)m_pDlgParent->m_pTrainDataParam)->m_bMaskEnable;
		if (m_bEnableMask)
		{
...
void CDlgProCircleSearchTool::UpdateSearchData(bool bValidate)
{
	if (!m_pDlgParent->m_pSearchTool)
	{
		return;
	...
	if (bValidate) // 从界面获取搜索参数
	{
		...
		((CProCircleSearchSearchDataParam*)m_pDlgParent->m_pSearchDataParam)->m_nSearchNum = m_nSearchNum;
		((CProCircleSearchSearchDataParam*)m_pDlgParent->m_pSearchDataParam)->m_dAcceptThre = m_dAcceptThre;
		((CProCircleSearchSearchDataParam*)m_pDlgParent->m_pSearchDataParam)->m_dAngleLow = m_dAngleLow;
		((CProCircleSearchSearchDataParam*)m_pDlgParent->m_pSearchDataParam)->m_dAngleHigh = m_dAngleHigh;
	...
	{
		m_nSearchNum	= ((CProCircleSearchSearchDataParam*)m_pDlgParent->m_pSearchDataParam)->m_nSearchNum;
		m_dAcceptThre	= ((CProCircleSearchSearchDataParam*)m_pDlgParent->m_pSearchDataParam)->m_dAcceptThre;
		m_dAngleLow		= ((CProCircleSearchSearchDataParam*)m_pDlgParent->m_pSearchDataParam)->m_dAngleLow;
		m_dAngleHigh	= ((CProCircleSearchSearchDataParam*)m_pDlgParent->m_pSearchDataParam)->m_dAngleHigh;
...
void CDlgProCircleSearchTool::UpdateDialogLanguage()
{
//	return;
	...
	while(xml.FindElem(_T("")))
	{
		...
		if(_T("IDD_DLG_PROCIRCLESEARCH_TOOL") == strDlgID)
		{
			int nID;
...
void CDlgProCircleSearchTool::CheckDlgDataValidation()
{
	UpdateData(TRUE);
...
void CDlgProCircleSearchTool::OnBtnGetTrainImage() 
{
...
void CDlgProCircleSearchTool::OnBtnTrain() 
{
...
void CDlgProCircleSearchTool::OnBtnSetMaskImage() 
{
	// TODO: Add your control notification handler code here
	if (!m_pDlgParent->m_pSearchTool)
	{
...
void CDlgProCircleSearchTool::OnChkEnableMask() 
{
	// TODO: Add your control notification handler code here
	UpdateData(TRUE);
...
BOOL CDlgProCircleSearchTool::OnInitDialog() 
{
...
void CDlgProCircleSearchTool::OnBtnSetCenter() 
{
	if (m_pDlgParent->m_pSearchTool->IsTrained())
	{
		cpImage imageTrain, imagePattern;
		m_pDlgParent->m_pSearchTool->GetTrainImage(imageTrain);
		CProCircleSearchTrainGuiParam *pTrainGuiParam = new CProCircleSearchTrainGuiParam;
...
void CDlgProCircleSearchTool::OnSetfocusEdtGrau() 
{
	TRACE(_T("CDlgProCircleSearchTool EdtGrau SetFocus\n"));
...
void CDlgProCircleSearchTool::OnKillfocusEdtGrau() 
{
	TRACE(_T("CDlgProCircleSearchTool EdtGrau KillFocus\n"));
}

void CDlgProCircleSearchTool::OnSetfocusEdtNoisethre() 
{
	if (m_pDlgParent->m_bSysKeyboardEnabled)
	{
		::PostMessage(m_pDlgParent->GetSafeHwnd(), WM_SETFOCUS, NULL, NULL);
...
void CDlgProCircleSearchTool::OnSetfocusEdtTotalMag() 
{
	if (m_pDlgParent->m_bSysKeyboardEnabled)
	{
		::PostMessage(m_pDlgParent->GetSafeHwnd(), WM_SETFOCUS, NULL, NULL);
...
void CDlgProCircleSearchTool::OnSetfocusEdtSearchNum() 
{
	// TODO: Add your control notification handler code here
	if (m_pDlgParent->m_bSysKeyboardEnabled)
	{
...
void CDlgProCircleSearchTool::OnSetfocusEdtAcceptThre() 
{
	if (m_pDlgParent->m_bSysKeyboardEnabled)
	{
		::PostMessage(m_pDlgParent->GetSafeHwnd(), WM_SETFOCUS, NULL, NULL);
...
void CDlgProCircleSearchTool::OnSetfocusEdtAngleLow() 
{
	if (m_pDlgParent->m_bSysKeyboardEnabled)
	{
		::PostMessage(m_pDlgParent->GetSafeHwnd(), WM_SETFOCUS, NULL, NULL);
...
void CDlgProCircleSearchTool::OnSetfocusEdtAngleHigh() 
{
	if (m_pDlgParent->m_bSysKeyboardEnabled)
	{
		::PostMessage(m_pDlgParent->GetSafeHwnd(), WM_SETFOCUS, NULL, NULL);
...
void CDlgProCircleSearchTool::OnBtnFullImageSearch() 
{
	if (m_pDlgParent->m_imageTrain.IsValid())
	{
		CProCircleSearchTrainGuiParam trainGuiParam;
...
BOOL CDlgProCircleSearchTool::PreTranslateMessage(MSG* pMsg) 
{
...
LRESULT CDlgProCircleSearchTool::WindowProc(UINT message, WPARAM wParam, LPARAM lParam) 
{
	switch (message)
	{
	case WM_SETFOCUS:
		{
			TRACE(_T("CDlgProCircleSearchTool WM_SETFOCUS\n"));
		}
		break;
	case WM_KILLFOCUS:
		{
			TRACE(_T("CDlgProCircleSearchTool WM_KILLFOCUS\n"));
		}
		break;
	default:
		break;
