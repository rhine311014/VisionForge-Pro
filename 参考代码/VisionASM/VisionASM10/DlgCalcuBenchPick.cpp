...
IMPLEMENT_DYNAMIC(CDlgCalcuBenchPick, CDialog)


CDlgCalcuBenchPick::CDlgCalcuBenchPick(CWnd* pParent /*=NULL*/)
	: CDialog(CDlgCalcuBenchPick::IDD, pParent)
	, m_nRadio(0)
{
	m_nType = 0;
	...
	m_bCommGetBenchPickPlatformAxisPosSuccess = FALSE;
	m_nCurRegTriggerIndex = 0;
}

CDlgCalcuBenchPick::~CDlgCalcuBenchPick()
{
	int i=0;
...
void CDlgCalcuBenchPick::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Radio(pDX, IDC_RADIO_0, m_nRadio);
	DDX_Control(pDX, IDC_COMBO_EXPRODUCT_INDEX, m_comboExProductIndex);
}


BEGIN_MESSAGE_MAP(CDlgCalcuBenchPick, CDialog)
	ON_WM_DESTROY()
	ON_BN_CLICKED(IDC_BTN_SERIAL_GRAB, &CDlgCalcuBenchPick::OnBnClickedBtnSerialGrab)
	ON_BN_CLICKED(IDCANCEL, &CDlgCalcuBenchPick::OnBnClickedCancel)
	ON_BN_CLICKED(IDOK, &CDlgCalcuBenchPick::OnBnClickedOk)
	ON_BN_CLICKED(IDC_BTN_SEARCH_OBJECT_POS0, &CDlgCalcuBenchPick::OnBnClickedBtnSearchObjectPos0)
	ON_BN_CLICKED(IDC_BTN_SEARCH_VIRTUAL_OBJECT_POS0, &CDlgCalcuBenchPick::OnBnClickedBtnSearchVirtualObjectPos0)
	ON_BN_CLICKED(IDC_BTN_SEARCH_OBJECT_POS1, &CDlgCalcuBenchPick::OnBnClickedBtnSearchObjectPos1)
	ON_BN_CLICKED(IDC_BTN_SEARCH_VIRTUAL_OBJECT_POS1, &CDlgCalcuBenchPick::OnBnClickedBtnSearchVirtualObjectPos1)
	ON_BN_CLICKED(IDC_BTN_CALCU_VIRTUAL_OBJECT, &CDlgCalcuBenchPick::OnBnClickedBtnCalcuVirtualObject)
	ON_WM_TIMER()
	ON_MESSAGE(WM_SV_DISPLAY_MOUSEMOVE,OnGuiDisplayMouseMove)
	ON_BN_CLICKED(IDC_RADIO_0, &CDlgCalcuBenchPick::OnBnClickedRadio)
	ON_BN_CLICKED(IDC_RADIO_90, &CDlgCalcuBenchPick::OnBnClickedRadio)
	ON_BN_CLICKED(IDC_RADIO_180, &CDlgCalcuBenchPick::OnBnClickedRadio)
	ON_BN_CLICKED(IDC_RADIO_270, &CDlgCalcuBenchPick::OnBnClickedRadio)
END_MESSAGE_MAP()


// CDlgCalcuBenchPick 消息处理程序


void CDlgCalcuBenchPick::SetVisionAlignerRef( vcBaseVisionAlign *pVisionAligner)
{
...
BOOL CDlgCalcuBenchPick::OnInitDialog()
{
...
void CDlgCalcuBenchPick::InitDlgItem()
{
	// 	// 	CRect cr;
...
//	m_labelNote.SetText(_T("注:请确保各拍照位置的平台θ轴位置一致！！！"));
	...
	for (i=0; i<m_pVisionASM->GetPosNum() && i < 2; i++)
	{
		...
		pGUIDisplay->RegisterWndMsg(this, SV_DISPLAY_MOUSEMOVE|SV_DISPLAY_DBLCLICK);
		switch (lgidLanguage)
		{
...
// 	}
	...
	if (ePlatformXY == sysPlatformInfo.m_eMidPlatformType)
	{
		GetDlgItem(IDC_RADIO_0)->EnableWindow(FALSE);
		GetDlgItem(IDC_RADIO_90)->EnableWindow(FALSE);
		GetDlgItem(IDC_RADIO_180)->EnableWindow(FALSE);
		GetDlgItem(IDC_RADIO_270)->EnableWindow(FALSE);

		if (m_pVisionASM->GetObjectPosNum()==1)
		{
			GetDlgItem(IDC_BTN_SEARCH_VIRTUAL_OBJECT_POS1)->ShowWindow(SW_HIDE);
		}
	}
	...
	if(sysPlatformInfo.m_bEnableMulRegTrigger)
	{
		// 记录当前触发端索引，退出窗口后恢复
		m_nCurRegTriggerIndex = m_pVisionASM->m_XYDCommProtocol.m_nCurRegTriggerIndex;
		// 强制默认 获取轴为第一段地址轴；避免获取轴时的异常;也需要将拾取触发作为第一段； 检测之类作为其他段
		m_pVisionASM->m_XYDCommProtocol.m_nCurRegTriggerIndex = 0;
	}
	...
	//	strInfo.Format(_T("虚拟对象位置%d"),i+1);
	//	pGuiText = new scGuiText;
	//	pGuiText->SetXYRotationText(sc2Vector(10,10),scRadian(0),strInfo);
	//	pGuiText->SetLineColor(RGB(255,0,0));
	//	pGuiText->SetVisible(TRUE);
...
void CDlgCalcuBenchPick::UpdateStatusBar()
{
...
void CDlgCalcuBenchPick::OnDestroy()
{
	CDialog::OnDestroy();
	...
	if (m_pVisionASM != NULL)
	{
		...
		{
			//退出窗口后恢复 触发索引
			m_pVisionASM->m_XYDCommProtocol.m_nCurRegTriggerIndex = m_nCurRegTriggerIndex;
		}
	}
...
LRESULT CDlgCalcuBenchPick::WindowProc(UINT message, WPARAM wParam, LPARAM lParam)
{
...
// 			if (m_vpGuiDisplay[nPosIndex]->GetImage() != NULL)
// 			{
// 				m_vpGuiVirtualObjectDisplay[nPosIndex]->SetImage(*(scImage8*)m_vpGuiDisplay[nPosIndex]->GetImage(),TRUE);
// 			}
// 			else
// 			{
...
void CDlgCalcuBenchPick::UpdateBtnLiveGrab()
{
	// 更新“连续采集/停止采集”按钮 
...
void CDlgCalcuBenchPick::UpdateShutterDisplay()
{
	// 	// 更新曝光时间显示
...
BOOL CDlgCalcuBenchPick::SetGUI()
{
	int nCamNum = m_pVisionASM->GetCameraNum();
	nCamNum = (nCamNum == 4) ? 2 : nCamNum;
...
// 更新训练Gui控件显示
bool CDlgCalcuBenchPick::UpdateGuiDisplay()
{
	// 更新
	int nCamNum = m_pVisionASM->GetCameraNum();
	nCamNum = (nCamNum == 4) ? 2 : nCamNum;
...
void CDlgCalcuBenchPick::OnBnClickedBtnSerialGrab()
{
	// TODO: 在此添加控件通知处理程序代码
	int i=0;
	BOOL bSucceed = TRUE;
...
void CDlgCalcuBenchPick::OnBnClickedBtnSearchObjectPos0()
{
	// TODO: 在此添加控件通知处理程序代码
	m_nEvent = 1;
	CString strLog;
	strLog.Format(_T("点击[搜索基准对象Mark]\n"));
	g_opLogFile.SaveLogText(strLog,2);
}


void CDlgCalcuBenchPick::OnBnClickedBtnSearchVirtualObjectPos0()
{
	// TODO: 在此添加控件通知处理程序代码
	m_nEvent = 2;
	CString strLog;
	strLog.Format(_T("点击[获取拾取平台位置]\n"));
	g_opLogFile.SaveLogText(strLog,2);
}


void CDlgCalcuBenchPick::OnBnClickedBtnSearchObjectPos1()
{
	// TODO: 在此添加控件通知处理程序代码
	m_nEvent = 3;
	CString strLog;
	strLog.Format(_T("点击[搜索基准对象位置1]\n"));
	g_opLogFile.SaveLogText(strLog,2);
}


void CDlgCalcuBenchPick::OnBnClickedBtnSearchVirtualObjectPos1()
{
	// TODO: 在此添加控件通知处理程序代码
	m_nEvent = 4;
	CString strLog;
	strLog.Format(_T("点击[搜索基准对象位置2]\n"));
	g_opLogFile.SaveLogText(strLog,2);
}


void CDlgCalcuBenchPick::OnBnClickedBtnCalcuVirtualObject()
{
	// TODO: 在此添加控件通知处理程序代码
	if (m_pVisionASM == NULL)
	{
	...
	CString strLog;
	strLog.Format(_T("点击[计算基准拾取位置]\n"));
	g_opLogFile.SaveLogText(strLog,2);

	if (!m_bCommGetBenchPickPlatformAxisPosSuccess)
	{
		m_bCalcuVirtualObjectSuccess = FALSE;
		m_bStatusBarFlag = FALSE;
	...
	if (TRUE == m_pVisionASM->ExecuteBenchMarkPick(m_nRadio,nExProductIndex))
	{
...
void CDlgCalcuBenchPick::OnBnClickedOk()
{
	// TODO: 在此添加控件通知处理程序代码
	CString strLog;
	strLog.Format(_T("[保存退出]计算基准拾取位置\n"));
	g_opLogFile.SaveLogText(strLog,1);

	if (m_bCalcuVirtualObjectSuccess)
	{
		if (m_pVisionASM != NULL 
			&& TRUE == m_pVisionASM->SaveBenchMarkPick())
		{
			m_bStatusBarFlag = TRUE;
...
void CDlgCalcuBenchPick::OnBnClickedCancel()
{
	// TODO: 在此添加控件通知处理程序代码
	CString strLog;
	strLog.Format(_T("[不保存退出]计算基准拾取位置\n"));
	g_opLogFile.SaveLogText(strLog,1);
...
void CDlgCalcuBenchPick::OnTimer(UINT_PTR nIDEvent)
{
...
UINT CDlgCalcuBenchPick::ThreadReceiveData( LPVOID pParam )
{
	CDlgCalcuBenchPick *pOwner = (CDlgCalcuBenchPick*)pParam;
	CString strReceived;
	while (!pOwner->m_bExitThread)
	{
		if (pOwner->m_nEvent == 1)
		{
			...
			int nPosIndex = 0;
			SysPlatformInfo sysPlatformInfo;
			pOwner->m_pVisionASM->GetSysPlatformInfo(sysPlatformInfo);
			CPlatformOptionInfo optionInfo;
			pOwner->m_pVisionASM->GetPlatformOptionInfo(optionInfo);
			CAlignerParam* pAlignerParam = pOwner->m_pVisionASM->GetProductAlignerParam();
		...
		{
			pOwner->m_nEvent = 0;
			pOwner->CommGetBenchPickPlatformAxisPos();
		}	
		Sleep(10);
	}
...
void CDlgCalcuBenchPick::SearchObjectPos(int nPosIndex)
{
	if (m_pVisionASM == NULL)
	{
		return;
	...
	CAlignerParam* pAlignerParam = m_pVisionASM->GetProductAlignerParam();
	if (pAlignerParam == NULL)
	{
		return;
	}
	/*if (pAlignerParam->m_bDiffPosWithSameSearchResultEnabled)*/
	if (optionInfo.m_bEnableMultiMarkMode)
	{
		m_vpGuiAlnObjectImageMarkPos.at(nPosIndex+1)->SetVisible(FALSE);
	}

	m_pVisionASM->SetDefaultObjectSearchMarkPos(TRUE);
	//改变以下变量以 保存平台轴信息
	//BOOL bSaveAlignPos = m_pVisionASM->m_pSystempOptionConfig->m_bSaveAlignPos;

	BOOL bSaveAlignPos = optionInfo.m_bSaveAlignPos;
    BOOL bAutoAlign =m_pVisionASM->m_bAutoAlign;

	optionInfo.m_bSaveAlignPos = TRUE;
	m_pVisionASM->SetPlatformOptionInfo(optionInfo);
	m_pVisionASM->m_bAutoAlign = FALSE;
	...
	//恢复原值
	optionInfo.m_bSaveAlignPos = bSaveAlignPos;
	m_pVisionASM->SetPlatformOptionInfo(optionInfo);
	/*m_pVisionASM->m_pSystempOptionConfig->m_bSaveAlignPos = bSaveAlignPos;*/
	m_pVisionASM->m_bAutoAlign=bAutoAlign;
}

void CDlgCalcuBenchPick::SearchVirtualObjectPos(int nPosIndex)
{
	if (m_pVisionASM == NULL)
	{
		return;
...
void CDlgCalcuBenchPick::CommGetBenchPickPlatformAxisPos()
{
	m_bCommGetBenchPickPlatformAxisPosSuccess = FALSE;
	if (m_pVisionASM == NULL)
	{
	...
	// 通信获取平台轴位置
	m_bCommGetBenchPickPlatformAxisPosSuccess = m_pVisionASM->GetCalibAlignerBenchPickPlatformAxisPos();

}

void CDlgCalcuBenchPick::UpdateDialogLanguage()
{
	//	return;
	LANGID lgidLanguage = MAKELANGID(LANG_CHINESE, SUBLANG_CHINESE_SIMPLIFIED);
	if (m_psaSysInfoStrings != NULL)
	...
	while(xml.FindElem(_T("")))
	{
		...
		if(_T("IDD_DLG_CALCU_BENCH_PICK") == strDlgID)
		{
			int nID;
...
LRESULT CDlgCalcuBenchPick::OnGuiDisplayMouseMove(WPARAM wParam, LPARAM lParam)
{
	CString strStatus = " ";
	...
	for (int i = 0; i < m_vpGuiDisplay.size(); i++)
	{
		...
		{
			if (m_vpGuiDisplay.at(i)->GetImageCP() != NULL)
			{
				if (m_vpGuiDisplay.at(i)->GetImageCP()->IsValid())
				{
					long nWidth  = m_vpGuiDisplay.at(i)->GetImageCP()->Width();
					long nHeight = m_vpGuiDisplay.at(i)->GetImageCP()->Height();
					...
					m_vpGuiDisplay.at(i)->TransformPos(x,y,dImageX,dImageY);

					// 获取对应像素灰度值
					BYTE R,G,B;
					m_vpGuiDisplay.at(i)->GetImagePixelValue(LONG(dImageX),LONG(dImageY),R,G,B);
...
void CDlgCalcuBenchPick::OnBnClickedRadio()
{
	// TODO: 在此添加控件通知处理程序代码
	int nRadioOld = m_nRadio;
	UpdateData(TRUE);
	CString strLog;
	if (nRadioOld != m_nRadio)
	{
		CString strVal[] = {_T("0度位置"),_T("90度位置"),_T("180度位置"),_T("270度位置")};
		strLog.Format(_T("修改[拾取位置角度]由%s到%s\n"), strVal[nRadioOld],strVal[m_nRadio]);
		g_opLogFile.SaveLogText(strLog,2);
	}
}
