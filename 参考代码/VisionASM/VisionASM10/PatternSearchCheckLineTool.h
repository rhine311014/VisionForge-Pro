...
#define PATTERN_LINE_NUM 1

// PatternSearch定位工具训练Data参数类
typedef struct tagPatternSearchCheckLineTrainDataParam : public CBaseTrainDataParam
{
	...
	double			m_dContrastThre[PATTERN_LINE_NUM] ;			// 对比度阈值
	long			m_lFilterHalfWidth[PATTERN_LINE_NUM];		// 滤波器半宽
	int				m_nOutLineNum[PATTERN_LINE_NUM];			// 局外点
	EdgePolarity	m_edgeProperty[PATTERN_LINE_NUM];			// 边缘属性
	bool			m_bSortByScore[PATTERN_LINE_NUM];			// 按分数排序
	int				m_nLineAngleType[PATTERN_LINE_NUM];			// 直线角度类型：0,任意角度；1,水平线；2,垂直线；
	BOOL			m_bEnableDualEdge[PATTERN_LINE_NUM];		// 启用/禁用找线工具双边缘

	tagPatternSearchCheckLineTrainDataParam()
	{
...
}CPatternSearchCheckLineTrainDataParam;


// PatternSearch定位工具训练Gui参数类
typedef struct tagPatternSearchCheckLineTrainGuiParam : public CBaseTrainGuiParam
{
	...
	// 找线工具1
	int				m_nCaliperNum[PATTERN_LINE_NUM];			// 卡尺个数
	double			m_dStartX[PATTERN_LINE_NUM];				// 起始点X
	double			m_dStartY[PATTERN_LINE_NUM];				// 起始点Y
	double			m_dEndX[PATTERN_LINE_NUM];					// 终止点X
	double			m_dEndY[PATTERN_LINE_NUM];					// 终止点Y
	double			m_dCaliperProjectLen[PATTERN_LINE_NUM];		// 投影长度
	double			m_dCaliperSearchLen[PATTERN_LINE_NUM];		// 搜索长度
	scRadian		m_CaliperSearchRotation[PATTERN_LINE_NUM];	// 搜索角度
	bool			m_bCaliperVisible[PATTERN_LINE_NUM];		//卡尺是否可见
	// 找线工具掩摸
	...
	tagPatternSearchCheckLineTrainGuiParam()
	{
...
}CPatternSearchCheckLineTrainGuiParam;

// PatternSearch定位工具搜索Data参数类
typedef struct tagPatternSearchCheckLineSearchDataParam : public CBaseSearchDataParam
{
	...
	tagPatternSearchCheckLineSearchDataParam()
	{
		m_nSearchNum	= 1;
		m_dAcceptThre	= 0.6;
...
}CPatternSearchCheckLineSearchDataParam;

// PatternSearch定位工具搜索Gui参数类
typedef struct tagPatternSearchCheckLineGuiParam : public CBaseSearchGuiParam
{
	COLORREF        m_LineColor;						// 图形颜色
	int				m_nLineWidth;						// 图形线宽
	...
	tagPatternSearchCheckLineGuiParam()
	{
...
}CPatternSearchCheckLineSearchGuiParam;


// QuickSearch训练结果
typedef struct tagPatternSearchCheckLineTrainResult
{
	double m_dModelX; // 参考点X					
	double m_dModelY; // 参考点Y

	double m_dStartX[PATTERN_LINE_NUM]; // 线段起点X
	double m_dStartY[PATTERN_LINE_NUM]; // 线段起点Y
	double m_dEndX[PATTERN_LINE_NUM];	  // 线段终点X
	double m_dEndY[PATTERN_LINE_NUM];	  // 线段终点Y


	tagPatternSearchCheckLineTrainResult()
	{
		m_dModelX = 0;
		m_dModelY = 0;
...
}CPatternSearchCheckLineTrainResult;


class CPatternSearchCheckLineTool : public CBaseSearchTool
{
public:

	CPatternSearchCheckLineTool();
	CPatternSearchCheckLineTool(CPatternSearchCheckLineTool &cPatternSearchTool);
	CPatternSearchCheckLineTool& operator=(CPatternSearchCheckLineTool &cPatternSearchTool);
	virtual ~CPatternSearchCheckLineTool();
...
private:
	...
	CPatternSearchCheckLineTrainDataParam		m_tmpTrainDataParam;			// 当前训练Data参数
	CPatternSearchCheckLineTrainGuiParam		m_tmpTrainGuiParam;				// 当前训练Gui参数	
	CPatternSearchCheckLineSearchDataParam		m_tmpSearchDataParam;			// 当前搜索Data参数	
	CPatternSearchCheckLineSearchGuiParam		m_tmpSearchGuiParam;			// 当前搜索Gui参数
	CPatternSearchCheckLineTrainResult			m_tmpTrainResult;				// 当前训练结果
	cpImage								m_tmpImageTrain;				// 当前训练图像
	cpImage								m_tmpImageMask;					// 当前掩模图像

	CPatternSearchCheckLineTrainDataParam		m_TrainDataParam;				// 训练Data参数
	CPatternSearchCheckLineTrainGuiParam		m_TrainGuiParam;				// 训练Gui参数	
	CPatternSearchCheckLineSearchDataParam		m_SearchDataParam;				// 搜索Data参数	
	CPatternSearchCheckLineSearchGuiParam		m_SearchGuiParam;				// 搜索Gui参数	
	CPatternSearchCheckLineTrainResult			m_TrainResult;					// 训练结果
