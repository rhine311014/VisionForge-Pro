...
#define BTNST_PRESS_WHEN_CHECKED
...
CButtonST::CButtonST()
{
	...
	m_nDisableIconType = (int)BTNST_DISABLETYPE_NORMAL;
...
void CButtonST::PreSubclassWindow() 
{
	...
	if ( (m_nTypeStyle == BS_CHECKBOX) || (m_nTypeStyle == BS_AUTOCHECKBOX) ) 
		m_bIsCheckBox = TRUE;

	// Check if this is a radio button
	if ( (m_nTypeStyle == BS_RADIOBUTTON) || (m_nTypeStyle == BS_AUTORADIOBUTTON) ) 
		m_bIsRadioButton = TRUE;
...
UINT CButtonST::OnGetDlgCode() 
{
	...
	::GetWindowText(m_hWnd, strInfo.GetBuffer(32), 32);
...
{
	ASSERT(m_bIsCheckBox || m_bIsRadioButton);
...
{	
	...
	else if (m_bIsRadioButton)
	{
		::SendMessage(m_hWnd, BM_SETCHECK, BST_CHECKED, 0);
		
		HWND hWndCtrl = ::GetWindow(m_hWnd, GW_HWNDNEXT);
...
void CButtonST::DrawItem(LPDRAWITEMSTRUCT lpDIS)
{
	...
	if ((m_bIsCheckBox) || (m_bIsRadioButton))
	{
...
#ifdef BTNST_PRESS_WHEN_CHECKED
	// If button is pressed then press image also
	if (bIsPressed)
#else	
	// If button is CheckBox or RadioButton, do not press image
	if (bIsPressed && !m_bIsCheckBox && !m_bIsRadioButton)
#endif
...
{
	BYTE		byIndex		= 0;
	UINT uiFlag = DSS_NORMAL;

	// Select the icon to use
	if (bIsDisabled)
	{
		if (m_csIcons[2].hIcon == BTNST_DISABLETYPE_NORMALDISABLE)
		{
			uiFlag = DSS_DISABLED;
			byIndex = 0;
		}
	...
	else if (((m_bIsCheckBox || m_bIsRadioButton) && bIsPressed) 
			|| (!(m_bIsCheckBox || m_bIsRadioButton) && (bIsPressed || m_bMouseOnButton)))
	{
		byIndex = 0;
	}
	else
...
void CButtonST::DrawTheText(CDC* pDC, LPCTSTR lpszText, RECT* rpItem, CRect* rpCaption, BOOL bIsPressed, BOOL bIsDisabled)
{
	...
	if (bIsPressed)
#else	
	// If button is CheckBox or RadioButton, do not "press" title
	if (bIsPressed && !m_bIsCheckBox && !m_bIsRadioButton)
#endif
...
//		If the function fails, the return value is NULL.
//
HICON CButtonST::CreateUnionIcon(HICON hIcon)
{
	...
	if (::GetObject(csII.hbmColor, sizeof(BITMAP), &bmp))
	{
		...
		if (csGrayII.hbmColor)
		{
			...
			DWORD		dwLoopY = 0, dwLoopX = 0;
			COLORREF	crBkgrd = ::GetBkColor(hMainDC);
			COLORREF	crFrgrd = RGB(100, 100, 100);

			COLORREF	crNewPixel = crFrgrd;
			COLORREF	crPixel;
			
			for (dwLoopY = 0; dwLoopY < dwHeight-1; dwLoopY+=2)
			{
				for (dwLoopX = 0; dwLoopX < dwWidth-1; dwLoopX+=2)
				{
					crPixel = ::GetPixel(hMemDC1, dwLoopX, dwLoopY);
					if (crPixel)
						::SetPixel(hMemDC2, dwLoopX, dwLoopY, crNewPixel);
					else
						::SetPixel(hMemDC2, dwLoopX, dwLoopY, crPixel);

					crPixel = ::GetPixel(hMemDC1, dwLoopX+1, dwLoopY);
					if (crPixel)
						::SetPixel(hMemDC2, dwLoopX+1, dwLoopY, crNewPixel);
					else
						::SetPixel(hMemDC2, dwLoopX+1, dwLoopY, crPixel);
					
					crPixel = ::GetPixel(hMemDC1, dwLoopX, dwLoopY+1);
					if (crPixel)
						::SetPixel(hMemDC2, dwLoopX, dwLoopY+1, crNewPixel);
					else
						::SetPixel(hMemDC2, dwLoopX, dwLoopY+1, crPixel);
					
					crPixel = ::GetPixel(hMemDC1, dwLoopX+1, dwLoopY+1);
					if (crPixel)
						::SetPixel(hMemDC2, dwLoopX+1, dwLoopY+1, crNewPixel);
					else
						::SetPixel(hMemDC2, dwLoopX+1, dwLoopY+1, crPixel);

					crNewPixel = (crNewPixel == crBkgrd) ? crFrgrd : crBkgrd;
				} // for
				crNewPixel = (crNewPixel == crBkgrd) ? crFrgrd : crBkgrd;
			} // for
...
//
DWORD CButtonST::SetIcon( int nIconIn, UINT uiCxDesiredIn, UINT uiCyDesiredIn, 
						  int nIconOut /*= NULL*/, UINT uiCxDesiredOut /*= 0*/, UINT uiCyDesiredOut /*= 0*/,
						  int nIconDisabled /*= NULL*/, UINT uiCxDesiredDisabled /*= 0*/, UINT uiCyDesiredDisabled /*= 0*/ )
{
	HICON		hIconIn			= NULL;
	HICON		hIconOut		= NULL;
	...
	hIconIn = (HICON)::LoadImage(hInstResource, MAKEINTRESOURCE(nIconIn), IMAGE_ICON, uiCxDesiredIn, uiCyDesiredIn, 0);
	...
	switch (nIconOut)
	{
		...
		case (int)BTNST_AUTO_DARKER:
			...
			break;
		default:
			hIconOut = (HICON)::LoadImage(hInstResource, MAKEINTRESOURCE(nIconOut), IMAGE_ICON, uiCxDesiredOut, uiCyDesiredOut, 0);
			break;
	} // switch
	...
	case NULL:
		break;
	case (int)BTNST_DISABLETYPE_NORMALDISABLE:
		hIconDisabled = BTNST_DISABLETYPE_NORMALDISABLE;
		break;
	case (int)BTNST_DISABLETYPE_UNION:
		hIconDisabled = BTNST_DISABLETYPE_UNION;
		break;
	case (int)BTNST_DISABLETYPE_GRAY:
		hIconDisabled = BTNST_DISABLETYPE_GRAY;
		break;
	case (int)BTNST_DISABLETYPE_NORMAL:
		hIconDisabled = (HICON)::LoadImage(hInstResource, MAKEINTRESOURCE(hIconIn), IMAGE_ICON, uiCxDesiredIn, uiCyDesiredIn, 0);
	default:
		hIconDisabled = (HICON)::LoadImage(hInstResource, MAKEINTRESOURCE(nIconDisabled), IMAGE_ICON, uiCxDesiredDisabled, uiCyDesiredDisabled, 0);
		break;
	} // switch

	return SetIcon(hIconIn, hIconOut, hIconDisabled);
} // End of SetIcon
...
//			Failed loading the specified resource.
//
DWORD CButtonST::SetIcon(int nIconIn, int nIconOut, int nIconDisabled)
{
	return SetIcon(nIconIn, 0, 0, nIconOut, 0, 0, nIconDisabled, 0, 0);
} // End of SetIcon
...
//			Failed loading the specified resource.
//
DWORD CButtonST::SetIcon(HICON hIconIn, HICON hIconOut, HICON hIconDisabled)
{
	BOOL		bRetValue;
	ICONINFO	ii;
	...
	if (hIconIn)
	{
		...
		if (hIconDisabled)
		{
			BOOL bValidDisableIcon = FALSE;
			switch ((int)hIconDisabled)
			{
			case (int)BTNST_DISABLETYPE_GRAY:
				hIconDisabled = CreateGrayscaleIcon(hIconIn);
				bValidDisableIcon = TRUE;
				break;
			case (int)BTNST_DISABLETYPE_UNION:
				hIconDisabled = CreateUnionIcon(hIconIn);
				bValidDisableIcon = TRUE;
				break;
			case (int)BTNST_DISABLETYPE_NORMAL:
				bValidDisableIcon = FALSE;
				break;
			case (int)BTNST_DISABLETYPE_NORMALDISABLE:
				bValidDisableIcon = FALSE;
				break;
			...
			m_csIcons[2].hIcon = hIconDisabled;
			
			if (bValidDisableIcon)
			{
				// Get icon dimension
				::ZeroMemory(&ii, sizeof(ICONINFO));
