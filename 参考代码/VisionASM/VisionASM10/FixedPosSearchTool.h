// FixedPosSearchTool.h: interface for the CFixedPosSearchTool class.
//
//////////////////////////////////////////////////////////////////////

#if !defined(AFX_FIXEDPOSSEARCHTOOL_H__C3DCB990_16C4_4EC9_A628_5695EDE4D89D__INCLUDED_)
#define AFX_FIXEDPOSSEARCHTOOL_H__C3DCB990_16C4_4EC9_A628_5695EDE4D89D__INCLUDED_
...
// 固定点类型
typedef enum tagFixedPosMode
{
	eImageCenter = 0, // 图像中心
		eAnyPoint	  // 任意点
}CFixedPosMode;

// FixedPosSearch定位工具训练Data参数类
typedef struct tagFixedPosSearchTrainDataParam : public CBaseTrainDataParam
{	
...
}CFixedPosSearchTrainDataParam;

// FixedPosSearch定位工具训练Gui参数类
typedef struct tagFixedPosSearchTrainGuiParam : public CBaseTrainGuiParam
{	
	sc2Vector		m_ModelPoint;						// 参考点
	CFixedPosMode	m_FixedPosMode;						// 固定点类型
	COLORREF        m_LineColor;						// 图形颜色
	int				m_nLineWidth;						// 图形线宽
	bool			m_bModelPointVisible;				// 训练区域是否可见

	bool			m_bEnableSecondModelPoint;			// 启用第二参考点
	sc2Vector		m_ModelPoint2;						// 参考点2
	CFixedPosMode   m_FixedPosMode2;					// 固定点类型
	tagFixedPosSearchTrainGuiParam()
	{
		m_FixedPosMode			= eAnyPoint;
		m_LineColor				= RGB(0, 0, 255);
		m_nLineWidth			= 1;
		m_bModelPointVisible	= true;

		m_bEnableSecondModelPoint = false;
		m_FixedPosMode2			= eAnyPoint;
	}
	virtual CSearchToolType GetSearchToolType()
	{
		return eFixedPosSearchTool;
...
}CFixedPosSearchTrainGuiParam;

// FixedPosSearch定位工具搜索Data参数类
typedef struct tagFixedPosSearchSearchDataParam : public CBaseSearchDataParam
{
	virtual CSearchToolType GetSearchToolType()
	{
		return eFixedPosSearchTool;
...
}CFixedPosSearchSearchDataParam;

// FixedPosSearch定位工具搜索Gui参数类
typedef struct tagFixedPosSearchGuiParam : public CBaseSearchGuiParam
{
	COLORREF        m_LineColor;						// 图形颜色
	int				m_nLineWidth;						// 图形线宽
	bool			m_bModelPointVisible;				// 参考点是否可见

	tagFixedPosSearchGuiParam()
	{
...
}CFixedPosSearchSearchGuiParam;

// QuickSearch训练结果
typedef struct tagFixedPosSearchTrainResult
{
	double m_dModelX;
	double m_dModelY;
...
}CFixedPosSearchTrainResult;


//
// 固定位置定位工具
//
class /*SV_SEARCHTOOL_API*/ CFixedPosSearchTool : public CBaseSearchTool      
{
public:
	CFixedPosSearchTool();
	CFixedPosSearchTool(CFixedPosSearchTool &cFixedSearchTool);
	CFixedPosSearchTool& operator=(CFixedPosSearchTool &cFixedSearchTool);
	virtual ~CFixedPosSearchTool();
...
private:
	...
	CFixedPosSearchTrainDataParam		m_tmpTrainDataParam;			// 当前训练Data参数
	CFixedPosSearchTrainGuiParam		m_tmpTrainGuiParam;				// 当前训练Gui参数	
	CFixedPosSearchSearchDataParam		m_tmpSearchDataParam;			// 当前搜索Data参数	
	CFixedPosSearchSearchGuiParam		m_tmpSearchGuiParam;			// 当前搜索Gui参数
	CFixedPosSearchTrainResult			m_tmpTrainResult;				// 当前训练结果
	cpImage							m_tmpImageTrain;				// 当前训练图像
	cpImage							m_tmpImageMask;					// 当前掩模图像
	CFixedPosSearchTrainDataParam		m_TrainDataParam;				// 训练Data参数
	CFixedPosSearchTrainGuiParam		m_TrainGuiParam;				// 训练Gui参数	
	CFixedPosSearchSearchDataParam		m_SearchDataParam;				// 搜索Data参数	
	CFixedPosSearchSearchGuiParam		m_SearchGuiParam;				// 搜索Gui参数
	CFixedPosSearchTrainResult			m_TrainResult;					// 训练结果
	...
	sc2Vector                           m_TrainModelPoint;				// 训练之后的参考点
...
#endif // !defined(AFX_FIXEDPOSSEARCHTOOL_H__C3DCB990_16C4_4EC9_A628_5695EDE4D89D__INCLUDED_)
