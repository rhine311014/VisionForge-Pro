...
#endif //BIF_NEWDIALOGSTYLE
class CDlgSearch;
class CDlgPatternDouleSearchTool : public CDialogEx
{
	DECLARE_DYNAMIC(CDlgPatternDouleSearchTool)

public:
	CDlgPatternDouleSearchTool(CWnd* pParent = NULL);   // 标准构造函数
	virtual ~CDlgPatternDouleSearchTool();

	//训练对话框有关参数传递
	CDlgSearch		*m_pDlgParent;
	scGuiDisplay	m_GuiPatternDisplay1;
	scGuiDisplay	m_GuiPatternDisplay2;
	cpImage		m_imageMask;			//工具1掩膜设置
	cpImage		m_imageMask2;			//工具2掩膜设置
	CStringArray*	m_psaSysInfoStrings;


// 对话框数据
	enum { IDD = IDD_DLG_PATTERNDOUBLESEARCH_TOOL };
