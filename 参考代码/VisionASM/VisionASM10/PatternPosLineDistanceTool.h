...
// PatternPosLineDistance定位工具训练Data参数类
typedef struct tagPatternPosLineDistanceTrainDataParam : public CBaseTrainDataParam
{
	...
	bool			m_bEnableSearchPos;					// 是否启用定位找点，如果不启用则使用固定参考点
	bool			m_bEnableFindLine1;					// 是否启用找线工具1，如果不启用则只进行找线0

	tagPatternPosLineDistanceTrainDataParam()
	{
...
}CPatternPosLineDistanceTrainDataParam;

// PatternPosLineDistance定位工具训练Gui参数类
typedef struct tagPatternPosLineDistanceTrainGuiParam : public CBaseTrainGuiParam
{
	...
	tagPatternPosLineDistanceTrainGuiParam()
	{
		m_nCaliperNum[0]				= 2;
		m_dStartX[0]					= 100.0;
		m_dStartY[0]					= 100.0;
...
}CPatternPosLineDistanceTrainGuiParam;

// PatternPosLineDistance定位工具搜索Data参数类
typedef struct tagPatternPosLineDistanceSearchDataParam : public CBaseSearchDataParam
{
	...
	bool	m_bEnableMax[LINE_NUM];						// 是否启用最大值判断
	double	m_dDistanceMax[LINE_NUM];					// 距离最大值（毫米）
	bool	m_bEnableMin[LINE_NUM];						// 是否启用最小值判断
	double	m_dDistanceMin[LINE_NUM];					// 距离最小值（毫米）

	tagPatternPosLineDistanceSearchDataParam()
	{
		m_nSearchNum	= 1;
		m_dAcceptThre	= 0.6;
		...
		for (int i=0; i<LINE_NUM; i++)
		{
			m_bEnableMax[i]		= false;
			m_dDistanceMax[i]	= 1.0;
			m_bEnableMin[i]		= false;
			m_dDistanceMin[i]	= 0.0;
		}
	}
	virtual CSearchToolType GetSearchToolType()
	{
...
}CPatternPosLineDistanceSearchDataParam;

// PatternPosLineDistance定位工具搜索Gui参数类
typedef struct tagPatternPosLineDistanceGuiParam : public CBaseSearchGuiParam
{
	COLORREF        m_LineColor;						// 图形颜色
	int				m_nLineWidth;						// 图形线宽
	...
	bool			m_bLineSegProjectVisible;			// 结果点到结果直线的投影线段是否可见
	...
	tagPatternPosLineDistanceGuiParam()
	{
...
}CPatternPosLineDistanceSearchGuiParam;

// PatternSearch训练结果
typedef struct tagPatternPosLineDistanceTrainResult
{
	...
	tagPatternPosLineDistanceTrainResult()
	{
		m_dModelX = 0;
		m_dModelY = 0;
...
}CPatternPosLineDistanceTrainResult;

class CPatternPosLineDistanceTool : public CBaseSearchTool
{
public:
	CPatternPosLineDistanceTool(void);
	CPatternPosLineDistanceTool(CPatternPosLineDistanceTool &cPatternPosLineDistanceTool);
	CPatternPosLineDistanceTool& operator=(CPatternPosLineDistanceTool &cPatternPosLineDistanceTool);
	~CPatternPosLineDistanceTool(void);
...
private:
	...
	CPatternPosLineDistanceTrainDataParam		m_tmpTrainDataParam;			// 当前训练Data参数
	CPatternPosLineDistanceTrainGuiParam		m_tmpTrainGuiParam;				// 当前训练Gui参数	
	CPatternPosLineDistanceSearchDataParam		m_tmpSearchDataParam;			// 当前搜索Data参数	
	CPatternPosLineDistanceSearchGuiParam		m_tmpSearchGuiParam;			// 当前搜索Gui参数
	CPatternPosLineDistanceTrainResult			m_tmpTrainResult;				// 当前训练结果
	cpImage							m_tmpImageTrain;				// 当前训练图像
	cpImage							m_tmpImageMask;					// 当前掩模图像
	CPatternPosLineDistanceTrainDataParam		m_TrainDataParam;				// 训练Data参数
	CPatternPosLineDistanceTrainGuiParam		m_TrainGuiParam;				// 训练Gui参数	
	CPatternPosLineDistanceSearchDataParam		m_SearchDataParam;				// 搜索Data参数	
	CPatternPosLineDistanceSearchGuiParam		m_SearchGuiParam;				// 搜索Gui参数	
	CPatternPosLineDistanceTrainResult			m_TrainResult;					// 训练结果
	...
	std::vector<scGuiGraphic*>          m_pGuiSearchLineSegProjectArray;// 结果点到结果直线的投影线段
	...
	scGuiLineSeg						m_GuiSearchLineSegProject[MAXRESNUM*2];
