...
#ifdef VS_GRABBASE_EXPORTS 
#define VS_GRABBASE_API _declspec(dllexport)
#else
#define VS_GRABBASE_API _declspec(dllimport)
#endif

VS_GRABBASE_API void SetHigherLevelXmlFilePathEnable(bool bHigherLevelXmlFilePath);
VS_GRABBASE_API bool GetHigherLevelXmlFilePathEnable();

#pragma pack(push,8)
class VS_GRABBASE_API vcGrabBase
{
public:
	vcGrabBase(void);
	virtual ~vcGrabBase(void);
...
// 	virtual bool CheckSoftwareTriggerPresence(); 													// 查询相机是否有软触发采集
	...
	/////////////////////////////////相机属性相关//////////////////////////////////
	// 帧率设置
	virtual bool SetCurFrameRate(float fFrameRate);													// 设置帧率，可能需要改变最大最小曝光时间
	virtual bool GetCurFrameRate(float *pfFrameRate);												// 获取当前帧率
	virtual bool GetFrameRateRange(float *fMinFrameRate, float *fMaxFrameRate);						// 获取相机帧率范围
	// 曝光时间设置
	...
	virtual void* GetPointCloudGenerateTool()const;										            // 获取点云生成工具指针
...
protected:
	//////////////////////////////////////公共函数//////////////////////////////////
	static bool InitSystem(bool bHigherLevelXmlFilePath);
	static bool FreeSystem();
...
protected:
	static vcGrabBase*	CreateCamera();
	static void DeleteCamera(vcGrabBase* pGrabBase);

protected:
	virtual void RecordCameraLog(CString str,int nCameraIndex);
...
protected:
	...
	bool m_bFrameRateMaxLoaded;		//是否从xml文件中成功加载最大帧率
	float m_fFrameRateMax;			//从xml文件中加载的最大帧率
};
