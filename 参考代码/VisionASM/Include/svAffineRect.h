...
class scAffineRect;
smDerivedShapePtrhDecl(scAffineRect);

class SVCORE_API scAffineRect : public scShape  
{
public:
	scAffineRect();														// 构造函数

	scAffineRect(const sc2XformLinear& affRectFromUS);					// 二维仿射变换构造仿射矩形

	scAffineRect(const sc2Vector& cornerPo,								// 用原点cornerPo，x角点conerPx和y角点cornerPy构造仿射矩形
		const sc2Vector& cornerPx,
		const sc2Vector& cornerPy);

	scAffineRect (const sc2Vector& center, double xLen, double yLen,	// 用中心center，x边长xLen，y边长yLen，旋转角度xRotation和切变角skew构造仿射矩形
		const scRadian& xRotation = scRadian(0),
		const scRadian& skew      = scRadian(0));

	scAffineRect(double centerX,double centerY,							// 用中心center，x边长xLen，y边长yLen，旋转角度xRotation和切变角skew构造仿射矩形
		double xLen,double yLen, 
		double xRotation, double skew);

	scAffineRect(double centerX,double centerY,							// 用中心center，x边长xLen，y边长yLen构造仿射矩形
		double xLen, double yLen);	 

	void SetAffineRectFromUnitSq(const sc2XformLinear& affRectFromUS);	// 设置仿射变换

	void SetCornersPoPxPy(const sc2Vector& cornerPo,					// 设置原点cornerPo，x角点cornerPx，y角点cornerPy
		const sc2Vector& cornerPx,
		const sc2Vector& cornerPy);

	void SetCornersPoPxPy(double cornerPoX,double cornerPoY,           
		double cornerPxX, double cornerPxY,
		double cornerPyX, double cornerPyY);
	...
	void SetCenter(double centerX,double centerY);						// 设置中心center
	void SetCornerPo(const sc2Vector& po);								// 设置原点po
	void SetCornerPo(double poX,double poY);							// 设置原点po
	...
	const sc2XformLinear& GetAffineRectFromUnitSq () const;				// 获取仿射变换
	...
	scAffineRect Map(const sc2XformLinear& c) const;					// 对当前图形进行sc2Xform变换c，得到新的仿射矩形

	bool Degenerate() const;											// 判断仿射矩形的参数是否有效

	static bool IsBadSkew(const scRadian& skew);						// 判断切变角是否有效

	bool operator==(const scAffineRect& rhs) const;						// 重载==操作符
	bool operator!=(const scAffineRect& rhs) const						// 重载!=操作符
	{ return !(*this == rhs);};				

	virtual ShapeType GetType() const{ return eAffineRect; };				// 返回类型的枚举值
