...
class SV_VISION_3D_API scSphereFitTool
{
public:
	scSphereFitTool(void);
	~scSphereFitTool(void);
	...
	//设置/获取平面拟合方法
	void SetFitSphereMethod(FitMethod3D method) {m_FitMethod = method; };
	FitMethod3D GetFitSphereMethod() const { return m_FitMethod; };
	...
	//根据深度图拟合平面，输出结果图像
	bool Execute(cpImage img,scShape* pROI,scSphere& SphereResult,double& dRms,cpImageMask& ResultImage);

	//根据点云数据拟合平面，输出是否参与拟合的点
	bool Execute(const svStd vector<sc3Vector>& vPoints,scSphere& SphereResult,double& dRms,svStd vector<bool>& vbUsed);
