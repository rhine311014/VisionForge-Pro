...
class SV_VISION_3D_API scCircleFitTool
{
public:
	scCircleFitTool();
	~scCircleFitTool();
	...
	//根据点云数据拟合圆，输出是否参与拟合的点
	// vFitPoint为拟合点投影至圆所在平面的三维投影点
	bool Execute(const std::vector<sc3Vector>& vPoints, scCircle3D& resCircle, double& dRms, std::vector<bool>& vbUsed);
	bool Execute(const std::vector<sc3Vector>& vPoints, scCircle3D& resCircle, double& dRms, std::vector<bool>& vbUsed, std::vector<sc3Vector>& vFitPoint);
