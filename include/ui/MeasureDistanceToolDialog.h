/**
 * @file MeasureDistanceToolDialog.h
 * @brief 距离测量工具参数编辑对话框
 * @author VisionForge Team
 * @date 2025-12-18
 */

#pragma once

#include <QDialog>
#include <QComboBox>
#include <QDoubleSpinBox>
#include <QSpinBox>
#include <QPushButton>
#include <QLabel>
#include <QGroupBox>
#include <QCheckBox>
#include <QLineEdit>
#include <QVBoxLayout>

#include "base/ImageData.h"
#include "ui/PreviewHelper.h"

namespace VisionForge {

namespace Algorithm {
    class MeasureDistanceTool;
}

namespace UI {

class ImageViewer;

/**
 * @class MeasureDistanceToolDialog
 * @brief 距离测量工具参数编辑对话框
 *
 * 支持的测量模式:
 * - 点到点距离
 * - 点到线距离
 * - 线到线距离
 */
class MeasureDistanceToolDialog : public QDialog
{
    Q_OBJECT

public:
    explicit MeasureDistanceToolDialog(Algorithm::MeasureDistanceTool* tool, QWidget* parent = nullptr);
    ~MeasureDistanceToolDialog() override;

    /**
     * @brief 设置显示图像
     */
    void setImage(const Base::ImageData::Ptr& image);

    /**
     * @brief 更新UI显示
     */
    void updateUI();

signals:
    /**
     * @brief 参数已改变
     */
    void parameterChanged();

    /**
     * @brief 请求执行测量
     */
    void executeMeasureRequested();

    /**
     * @brief 请求采集图像信号
     */
    void captureImageRequested();

protected:
    void showEvent(QShowEvent* event) override;

private slots:
    // 参数槽
    void onMeasureModeChanged(int index);
    void onPixelToMmChanged(double value);
    void onInputSourceChanged(int index);
    void onPoint1Changed();
    void onPoint2Changed();
    void onLine1Changed();
    void onLine2Changed();

    // 对话框按钮槽
    void onPreviewClicked();
    void onOkClicked();
    void onCancelClicked();
    void onApplyClicked();
    void onAutoCalc();

    // 图像操作槽
    void onLoadImageClicked();
    void onCaptureImageClicked();

private:
    void createUI();
    void createMeasureModeGroup(QVBoxLayout* layout);
    void createInputSourceGroup(QVBoxLayout* layout);
    void createPointInputGroup(QVBoxLayout* layout);
    void createLineInputGroup(QVBoxLayout* layout);
    void createPixelConversionGroup(QVBoxLayout* layout);
    void createResultGroup(QVBoxLayout* layout);
    void createButtonGroup(QVBoxLayout* layout);
    void connectSignals();
    void applyParameters();
    void updateInputVisibility();
    void updateResultDisplay();

private:
    Algorithm::MeasureDistanceTool* tool_;
    Base::ImageData::Ptr currentImage_;

    // 图像显示
    ImageViewer* imageViewer_;

    // 测量模式
    QComboBox* measureModeCombo_;

    // 输入源选择
    QComboBox* inputSourceCombo_;
    QCheckBox* useToolChainCheck_;

    // 点输入
    QGroupBox* pointInputGroup_;
    QDoubleSpinBox* point1XSpin_;
    QDoubleSpinBox* point1YSpin_;
    QDoubleSpinBox* point2XSpin_;
    QDoubleSpinBox* point2YSpin_;

    // 线输入
    QGroupBox* lineInputGroup_;
    QDoubleSpinBox* line1X1Spin_;
    QDoubleSpinBox* line1Y1Spin_;
    QDoubleSpinBox* line1X2Spin_;
    QDoubleSpinBox* line1Y2Spin_;
    QDoubleSpinBox* line2X1Spin_;
    QDoubleSpinBox* line2Y1Spin_;
    QDoubleSpinBox* line2X2Spin_;
    QDoubleSpinBox* line2Y2Spin_;

    // 像素转换
    QDoubleSpinBox* pixelToMmSpin_;

    // 结果显示
    QLabel* distancePixelLabel_;
    QLabel* distanceMmLabel_;
    QLabel* point1ResultLabel_;
    QLabel* point2ResultLabel_;

    // 对话框按钮
    QPushButton* previewBtn_;
    QPushButton* okBtn_;
    QPushButton* cancelBtn_;
    QPushButton* applyBtn_;

    // 图像操作按钮
    QPushButton* loadImageBtn_;
    QPushButton* captureImageBtn_;

    // 实时计算
    PreviewHelper* previewHelper_;
    QCheckBox* autoCalcCheck_;
};

} // namespace UI
} // namespace VisionForge
